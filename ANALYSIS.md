...
**Hephaestus** (oh-my-opencode) and **Deep-Executor** (oh-my-copilot) are both autonomous deep worker agents designed for complex, goal-oriented software engineering tasks. Deep-Executor is explicitly ported from Hephaestus (PR #1287) but has been adapted to fit the oh-my-copilot framework.
...
| Aspect | Hephaestus | Deep-Executor |
|--------|------------|---------------|
| **Model** | GPT 5.2 Codex Medium | Copilot Opus 4.6 |
...
- Survives conversation compaction (per copilot-instructions.md)
...
- OMP/Coordinator as primary orchestrators
...
**Integration:**
- Receives delegated tasks from OMP/Coordinator
- Works alongside other specialized agents
- Part of larger workflow orchestration

### Deep-Executor (OMP - oh-my-copilot)
**Framework:** Copilot CLI plugin
**Ecosystem:**
...
### Comparison
| Aspect | Hephaestus (OMO) | Deep-Executor (OMP) |
|--------|------------------|---------------------|
| **Primary usage** | Delegated deep work | Direct OR delegated |
| **Orchestration** | OMP/Coordinator invoke | Multiple modes can invoke |
| **Agent collaboration** | Can delegate to explore/document-specialist | Fully isolated |
| **Framework scope** | Part of OpenCode ecosystem | Part of Copilot CLI ecosystem |
...
- Follows existing OMP patterns (ultrawork, pipeline)
...
- All exploration done by Copilot Opus 4.6
- **Cost**: High (expensive model for all exploration)

**Execution Phase:**
- Copilot Opus 4.6 for all implementation
...
## Appendix A: Implementation Checklist for OMP
...
2. **Supporting Documentation**:
   ...
   - OMP prompt - Orchestrator patterns
...
2. **Supporting Context**:
   ...
   - Oh-my-copilot-cli framework documentation (copilot-instructions.md)
   - Agent definitions and orchestration patterns

3. **Key Insights**:
   - Explicitly ported from Hephaestus (PR #1287)
   - Adapted for Copilot CLI ecosystem
   - Designed as standalone OR delegated agent
   - Uses Copilot Opus 4.6 exclusively
