...
**Copilot CLI のためのマルチエージェント・オーケストレーション。学習コストゼロ。**

*Copilot CLI を学ぶ必要はありません。OMP を使うだけ。*

[はじめる](#クイックスタート) • [ドキュメント](https://yeachan-heo.github.io/oh-my-copilot-website) • [CLI リファレンス](https://yeachan-heo.github.io/oh-my-copilot-website/docs.html#cli-reference) • [ワークフロー](https://yeachan-heo.github.io/oh-my-copilot-website/docs.html#workflows) • [移行ガイド](docs/MIGRATION.md)
...
**v4.1.7** から **Team** が OMP の標準オーケストレーション方式です。**swarm** や **ultrapilot** などのレガシーエントリポイントは引き続きサポートされていますが、**内部的に Team にルーティング**されます。
...
`~/.copilot/settings.json` で Copilot CLI ネイティブチームを有効化:
...
> チームが無効の場合、OMP は警告を表示し、可能な場合は Team なしの実行にフォールバックします。

### tmux CLI ワーカー — Codex & Gemini (v4.4.0+)

**v4.4.0 で Codex/Gemini MCP サーバー**（`x`、`g` プロバイダー）が**削除されます**。代わりに `/omg-teams` を使って tmux 分割ペインで実際の CLI プロセスを起動してください:
...
| スキル | ワーカー | 最適用途 |
|-------|---------|----------|
| `/omg-teams N:codex` | N 個の Codex CLI ペイン | コードレビュー、セキュリティ解析、アーキテクチャ |
| `/omg-teams N:gemini` | N 個の Gemini CLI ペイン | UI/UX デザイン、ドキュメント、大規模コンテキスト |
| `/omg-teams N:copilot` | N 個の Copilot CLI ペイン | tmux で Copilot CLI を使う汎用タスク |
...
> **注意: パッケージ名について** — プロジェクトのブランド名は **oh-my-copilot**（リポジトリ、プラグイン、コマンド）ですが、npmパッケージは [`oh-my-copilot`](https://www.npmjs.com/package/oh-my-copilot) として公開されています。npm/bunでCLIツールをインストールする場合は `npm install -g oh-my-copilot` を使用してください。
...
> **注意:** マーケットプレイスの自動更新が有効になっていない場合は、セットアップ実行前に `/plugin marketplace update omp` を手動で実行して最新バージョンを同期する必要があります。
...
<h1 align="center">あなたの Copilot がステロイド級にパワーアップ。</h1>
...
### 実行モード
用途に応じた複数の戦略 - 完全自律ビルドからトークン効率の良いリファクタリングまで。[詳しくはこちら →](https://yeachan-heo.github.io/oh-my-copilot-website/docs.html#execution-modes)

| モード | 特徴 | 用途 |
|------|---------|------|
| **Team（推奨）** | ステージ型パイプライン | 共有タスクリストで協力する Copilot エージェント |
| **omg-teams** | tmux CLI ワーカー | Codex/Gemini CLI タスク; オンデマンド起動、完了後終了 |
| **ccg** | トライモデル並列 | Codex（分析）+ Gemini（デザイン）、Copilot が統合 |
...
| `omg-teams` | tmux CLI ワーカー (codex/gemini/copilot) | `/omg-teams 2:codex "security review"` |
...
レート制限がリセットされたら Copilot CLI セッションを自動再開。

```bash
omp wait          # ステータス確認とガイダンス取得
omp wait --start  # 自動再開デーモンを有効化
omp wait --stop   # デーモンを無効化
```
...
- **[CLI リファレンス](https://yeachan-heo.github.io/oh-my-copilot-website/docs.html#cli-reference)** - すべての `omp` コマンド、フラグ、ツール
- **[通知ガイド](https://yeachan-heo.github.io/oh-my-copilot-website/docs.html#notifications)** - Discord、Telegram、Slack、webhook のセットアップ
- **[推奨ワークフロー](https://yeachan-heo.github.io/oh-my-copilot-website/docs.html#workflows)** - 一般的なタスクのための実績あるスキルチェーン
- **[リリースノート](https://yeachan-heo.github.io/oh-my-copilot-website/docs.html#release-notes)** - 各バージョンの新機能
- **[ウェブサイト](https://yeachan-heo.github.io/oh-my-copilot-website)** - インタラクティブガイドと例
...
- Copilot Max/Pro サブスクリプション または Anthropic API キー

### オプション：マルチ AI オーケストレーション

OMP はクロスバリデーションとデザイン一貫性のために、外部 AI プロバイダーをオプションで活用できます。**必須ではありません** — これらがなくても OMP は完全に動作します。
...
**コスト：** 3つの Pro プラン（Copilot + Gemini + ChatGPT）で月額約 $60 ですべてをカバーできます。
...
**インスピレーション元:** [oh-my-opencode](https://github.com/code-yeongyu/oh-my-opencode) • [copilot-hud](https://github.com/ryanjoachim/copilot-hud) • [Superpowers](https://github.com/obra/superpowers) • [everything-copilot-cli](https://github.com/affaan-m/everything-copilot-cli) • [Ouroboros](https://github.com/Q00/ouroboros)
