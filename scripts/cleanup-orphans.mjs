 ...
 * When omitted, scans for ALL orphan copilot agent processes.
 *
...
/**
 * Find copilot agent processes that match team patterns.
...
function getWindowsProcessListOutput() {
  try {
    ...
    return execSync(
      'wmic process where "name like \'%node%\' or name like \'%copilot%\'" get processid,commandline /format:csv',
    ...
    try {
      return execSync(
        'powershell -NoProfile -NonInteractive -Command "$procs = Get-CimInstance Win32_Process -ErrorAction Stop | Where-Object { $_.Name -like \'*node*\' -or $_.Name -like \'*copilot*\' }; $procs | ForEach-Object { [string]$_.CommandLine + \',\' + [string]$_.ProcessId }"',
        { encoding: 'utf-8', timeout: 10000 }
      ).trim();
    } catch {
      return '';
...
function main() {
  ...
  // Explicit exit: any pending .unref()ed SIGKILL timers should not pin the
  // event loop, but exit(0) here makes the completion intent unambiguous.
  process.exit(0);
}
