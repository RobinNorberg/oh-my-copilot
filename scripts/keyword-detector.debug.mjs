 ...
 * 5. cccg: Copilot-Claude-Codex-Gemini quad-model orchestration
...
Skill: oh-my-copilot:${skillName}${argsSection}
...
function resolveConflicts(matches) {
  ...
  const priorityOrder = ['cancel','ralph','autopilot','ultrawork',
    'cccg','ralplan','deep-interview','ai-slop-cleaner','tdd','code-review','security-review','ultrathink','deepsearch','analyze'];
...
async function main() {
  ...
  try {
    const input = await readStdin(); process.stderr.write("[DBG] stdin len=" + input.length + "
");
    if (!input.trim()) {
      console.log(JSON.stringify({ continue: true, suppressOutput: true }));
      return;
    ...
    process.stderr.write("[DBG] prompt=" + JSON.stringify(prompt.substring(0,30)) + "
"); const cleanPrompt = sanitizeForKeywordDetection(prompt).toLowerCase();
    ...
    if (/\b(cccg|copilot-claude-codex-gemini)\b/i.test(cleanPrompt)) {
      matches.push({ name: 'cccg', args: '' });
    }
    ...
    process.stderr.write("[DBG] matches=" + JSON.stringify(matches) + "
"); // Deduplicate matches
    const seen = new Set();
    const uniqueMatches = [];
