...
function fixture(kind) {
  ...
  const statePath = join(project, '.omg', 'state', 'sessions', sessionId, 'autopilot-state.json');
...
function invoke(f, input = {}, extraEnv = {}) {
  const stdout = execFileSync(process.execPath, [f.hook], {
    ...
    encoding: 'utf8',
    env: { ...process.env, HOME: f.home, USERPROFILE: f.home, COPILOT_CONFIG_DIR: f.claudeConfigDir, OMC_STATE_DIR: '', OMC_PERSISTENT_MODE_TIMEOUT_MS: '3000', ...extraEnv },
  });
...
describe.each(['plugin', 'installed-template'])('workflow profile stop transition (%s)', (kind) => {
  ...
  it('honors cancellation beside an already-loaded global fallback without recreating it', async () => {
    ...
    const globalPath = join(f.home, '.omg', 'state', 'autopilot-state.json');
