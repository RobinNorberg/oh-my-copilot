...
- "Right now, when you use Copilot CLI or similar tools, you're essentially getting a very smart intern. They can do anything you ask, but YOU have to manage the workflow."
...
**The Reveal**
- "That's exactly what oh-my-copilot does. It transforms Copilot CLI from a single generalist assistant into a coordinated team of 28 specialists."
- "Today, I'm going to show you how this changes everything about AI-assisted development."

### Transition
"Let me start by showing you exactly what OMP is and how it thinks differently about AI assistance."
...
## Section 2: What is OMP? (8 min, Slides 4-8)

### Opening Line
"The fundamental insight behind OMP is simple: your AI should be a conductor, not a performer."
...
- "That's the shift OMP makes. Copilot becomes the conductor, and 28 specialized agents are the orchestra."
- "When you say 'build a feature,' Copilot doesn't do it all personally. It delegates: Explorer finds relevant code, Architect designs the solution, Executor implements, QA Tester verifies."

**Before vs After** (Slide 5)
- "BEFORE: 'Copilot, search for auth code.' [wait] 'Now analyze it.' [wait] 'Now write tests.' [wait] You're the project manager micromanaging every step."
- "AFTER: 'Build authentication for the API.' OMP automatically: explores codebase, analyzes requirements, generates plan, implements in parallel, writes tests, verifies. You're the product owner stating the goal."
...
- "2. Copilot detects intent: This is a parallel execution task"
...
### Opening Line
"Execution modes are where OMP's power becomes concrete. Each mode is optimized for a specific type of work. Let's explore all five."
...
### Transition
"These modes are powered by OMP's agent system. Let's look at how those 28 specialists are organized."

### Audience Engagement
- "Quick question: Which mode sounds most useful for YOUR daily work?" (Take 2-3 responses)
- "The beauty is you don't have to memorize this. Say 'fast parallel fixes' and OMP activates ultrawork. Say 'efficient batch fixes' and it activates ."
...
- "OMP organizes agents into 13 specializations:"
...
- "Each domain has up to three tiers corresponding to Copilot model versions:"
...
- "How does OMP decide which agent to use? Three factors:"
...
### Opening Line
"Enough talking about it. Let's see OMP in action across five different scenarios."

### Demo 1: Autopilot - Full Feature Build (5 min, Slide 29)

**Setup**
- Have terminal ready with OMP installed
...
- Could be the OMP codebase itself with intentional errors
...
### Opening Line
"Powerful technology is useless if it's hard to use. OMP is designed for zero learning curve."
...
```
[OMP] Mode: ultrawork | Agents: 3 active | executor-1: fixing auth.ts | executor-2: fixing api.ts | architect: reviewing
```
- "At a glance you know:"
- "  Which mode is active"
- "  How many agents are working"
- "  What each agent is doing"
- "Installation: `/oh-my-copilot:hud setup` - adds to your shell prompt"
...
- "These persist across sessions. Your OMP gets smarter over time."
...
### Key Points
- Requires Copilot CLI CLI installed
...
- "You need Copilot CLI installed: `npm install -g copilot-cli`"
- "You need a Copilot subscription (Pro or Team) or an API key"
...
### Transition
"That's OMP. Let's recap and open for questions."
...
### Opening Line
"Let's recap what makes OMP transformative."
...
- "OMP transforms Copilot CLI from a single assistant into a coordinated team."
...
- "If you like it, share it with your team. OMP shines with real-world complexity."
...
**Answer:**
"OMP itself is completely free - it's MIT licensed open-source. What you pay for is the Copilot API usage.

You need either:
- A Copilot Pro subscription ($20/month) which includes API access
- Or a Copilot Team subscription with API credits
- Or direct API access via Anthropic

The key cost benefit: OMP's smart model routing saves you 30-50% on token costs compared to manually using Copilot. For example, simple searches use Haiku (super cheap), complex debugging uses Opus (expensive but necessary). Without OMP, everything might default to Opus.
...
**Answer:**
"Currently OMP is designed specifically for Copilot CLI and leverages Copilot's three model tiers - Haiku, Sonnet, and Opus.

The architecture relies on Copilot's specific capabilities for the multi-agent orchestration. We don't support GPT-4, Gemini, or other models at this time.
...
### 3. "How is this different from just using Copilot CLI?"
...
Real-world impact: Tasks that took 30 minutes of back-and-forth with Copilot CLI now take 5 minutes of autonomous execution with OMP."

### 4. "What about security? Is my code safe?"

**Answer:**
"OMP runs entirely locally via Copilot CLI. Your code never leaves your machine except through the normal Copilot API calls that you'd be making anyway.
...
Place custom agent definitions in `~/.copilot/agents/{agent-name}.md` and they'll override the defaults.

For example, if you want a specialized Python testing agent:
```markdown
# ~/.copilot/agents/pytest-specialist.md
...
**Answer:**
"Yes. OMP works with any language that Copilot CLI supports - which is basically all mainstream languages.
...
**Answer:**
"Honestly? You don't need to think about it. Just describe what you want in natural language and OMP auto-detects the right mode.
...
But more seriously - OMP has built-in verification at multiple levels:
...
In practice, failures happen - maybe a test fails, maybe there's a linting error. OMP catches these and fixes them before claiming completion.
...
**Answer:**
"OMP is designed for interactive development, not CI/CD automation.
...
For CI/CD, you'd use Copilot CLI's built-in capabilities or traditional CI tools.

That said, some teams use OMP-generated tests in their CI pipeline. The tests themselves are standard - jest, pytest, etc. - they just happened to be generated via OMP.
...
3. Feel confident they could install and use OMP today
4. Be excited about the paradigm shift from interactive to autonomous

**Your Success Metric:**
"How many people install OMP in the next week?"
