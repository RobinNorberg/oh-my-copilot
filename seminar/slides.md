...
---
# oh-my-copilot: Multi-Agent Orchestration for Copilot CLI
...
Note: This is a 60-minute seminar covering the complete oh-my-copilot system. We'll focus on practical usage patterns.
...
Note: These are real problems I faced building production applications with Copilot CLI. OMP was born from frustration with manually orchestrating AI-assisted development.
...
**A multi-agent orchestration system for Copilot CLI**
...
Note: OMP transforms Copilot from a single performer into a conductor of an orchestra of specialized AI agents.
...
```
User -> Copilot -> [Does everything itself]
```

**OMP Workflow:**
```
User -> Copilot (Conductor) -> [Delegates to specialists]
                                    |
                    +---------------+---------------+
                    |               |               |
...
**Copilot becomes an intelligent orchestrator** that delegates to the right specialist for each task.

Note: This is the core mental model. Copilot stops being a generalist trying to do everything and becomes a smart coordinator.

---

## Before vs After OMP

| Aspect | Before OMP | After OMP |
|--------|-----------|-----------|
...
/oh-my-copilot:autopilot Add OAuth2 authentication
...
Note: The tier system is central to OMP's cost efficiency. Always start low and escalate only when needed.

---

## Smart Delegation

**OMP automatically picks the right agent:**
...
```javascript
Task(
  subagent_type="oh-my-copilot:executor-low",
  model="haiku",
  prompt="Add validation to the login form"
)
```

Note: The model parameter is always passed explicitly - Copilot CLI doesn't auto-apply model from agent definitions.
...
Note: Composition is where OMP really shines - combine behaviors for exactly the workflow you need.
...
**Real-time visibility into OMP state:**

```
+------------------------------------------------------------+
| OMP | autopilot:exec | 3 agents | 5/12 tasks | ctx:45% | $2.34 |
+------------------------------------------------------------+
      ^               ^          ^            ^         ^
      |               |          |            |         |
...
Note: The HUD integrates with Copilot CLI's statusLine API to show real-time orchestration state.
...
**Requirements:**
- Copilot CLI CLI
- Copilot Max/Pro subscription OR Anthropic API key
- Node.js 20+
...
Note: Zero learning curve means you can start using OMP immediately after installation.

---

## Configuration

**Project-level:** `copilot-instructions.md` in project root
...
Note: Matching the right mode to the task type is key to getting the most out of OMP.
...
| Question | Answer |
|----------|--------|
| Does OMP work with Copilot API keys? | Yes, both Max/Pro subscription and API keys work |
| Can I use OMP with other AI models? | No, OMP is specifically for Copilot CLI |
| How do I stop a runaway autopilot? | Say "stop", "cancel", or `/oh-my-copilot:cancel` |
| Why is my HUD not showing? | Run `/oh-my-copilot:hud setup` |
...
|----------|--------------|--------|
| `autopilot:` | `/oh-my-copilot:autopilot` | Full autonomous mode |
| `ralph:` | `/oh-my-copilot:ralph` | Persistence mode |
| `ulw` | `/oh-my-copilot:ultrawork` | Parallel execution |
| `eco:` | `/oh-my-copilot:` | Token-efficient mode |
| `plan` | `/oh-my-copilot:plan` | Planning interview |
