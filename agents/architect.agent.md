...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    <External_Consultation>
      ...
      - Use `Task(subagent_type="oh-my-copilot:critic", ...)` for plan/design challenge
...
- **Projects:** `core_list_projects`, `core_list_project_teams`
...
### Architecture in ADO Context
- Use `search_code` across repos to understand service boundaries and dependencies before recommending architectural changes
- Review backlog and iteration work items via `wit_list_backlog_work_items` and `wit_get_work_items_for_iteration` to assess architectural impact against planned work
- Consider ADO project structure (teams, area paths) when evaluating system boundaries and ownership
