...
When verifying work in Azure DevOps repositories:

### MCP Tools for Verification
- `mcp__azure-devops__pipelines_get_build_status` — Check CI/CD pipeline status
- `mcp__azure-devops__testplan_show_test_results_from_build_id` — Check test results
- `mcp__azure-devops__advsec_get_alerts` — Check security alerts
- `mcp__azure-devops__repo_list_pull_request_threads` — Check PR review comments

### Verification Checklist for ADO
- Pipeline builds pass (check via MCP or `az pipelines runs list`)
- Work items are in correct state (check via `wit_get_work_item`)
- PR has no unresolved threads
- Advanced security alerts are addressed
