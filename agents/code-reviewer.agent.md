...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    <External_Consultation>
      ...
      - Use `Task(subagent_type="oh-my-copilot:code-reviewer", ...)` for cross-validation
...
- **PR Review:** `repo_create_pull_request_thread`, `repo_list_pull_request_threads`, `repo_list_pull_request_thread_comments`, `repo_reply_to_comment`
...
### Code Review in ADO
- Create inline PR comment threads via `repo_create_pull_request_thread` for specific file:line findings rather than a single summary comment
- Use thread status (active, fixed, closed) to track review feedback resolution across review cycles
- Reply to existing threads via `repo_reply_to_comment` rather than creating duplicate threads for the same issue
