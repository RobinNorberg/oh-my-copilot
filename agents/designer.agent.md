...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    <External_Consultation>
      ...
      - Use `Task(subagent_type="oh-my-copilot:designer", ...)` for UI/UX cross-validation
...
- **Wiki:** `wiki_list_wikis`, `wiki_list_pages`, `wiki_get_page_content`, `wiki_create_or_update_page`
- **Work Items:** `wit_get_work_item` (for reading design-related work items)
...
### Design Documentation in ADO
- Read design-related work items via `wit_get_work_item` to understand acceptance criteria and UX requirements before implementing
- Document design decisions and UI specifications in ADO Wiki via `wiki_create_or_update_page` when the project uses ADO as its documentation platform
- Link design documentation pages to related work items so stakeholders can trace design rationale back to requirements
