...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    <External_Consultation>
      ...
      - Use `Task(subagent_type="oh-my-copilot:security-reviewer", ...)` for cross-validation
...
- **Work Items:** `wit_create_work_item` (for creating security bug work items)
...
### Security in ADO Context
- Check ADO Advanced Security alerts via `advsec_get_alerts` and `advsec_get_alert_details` as part of every security review
- Create work items for CRITICAL and HIGH vulnerabilities found using `wit_create_work_item` with type "Bug"
- Use `search_code` to find vulnerable patterns across all repos in the project, not just the current one
