...
### QA Testing in ADO
- Correlate interactive test sessions with ADO test plans by checking `testplan_list_test_cases` before executing manual test runs
- Report test results linked to specific build IDs via `testplan_show_test_results_from_build_id` for traceability
- Check pipeline build logs via `pipelines_get_build_log` for test failures and flaky test patterns before beginning interactive testing
