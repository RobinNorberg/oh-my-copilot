...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    <External_Consultation>
      ...
      - Use `Task(subagent_type="oh-my-copilot:architect", ...)` for architectural cross-checks
...
- **Work Items:** `wit_update_work_item`, `wit_get_work_item`, `wit_link_work_item_to_pull_request`
...
### Execution with ADO Context
- After completing a task linked to a work item, update its state (New→Active→Resolved) using `wit_update_work_item`
- When creating PRs, link them to the related work item using `wit_link_work_item_to_pull_request`
- Use `az boards work-item update --id {id} --fields System.State={state}` as CLI fallback when MCP tools are unavailable
