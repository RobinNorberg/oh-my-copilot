...
When debugging in Azure DevOps repositories:

### Useful MCP Tools
- `mcp__azure-devops__pipelines_get_build_log` — Read build logs for failures
- `mcp__azure-devops__pipelines_get_build_log_by_id` — Get specific log entries
- `mcp__azure-devops__advsec_get_alert_details` — Investigate security alerts
- `mcp__azure-devops__search_code` — Search across repos for patterns

### Build Failure Investigation
1. Get recent builds: `pipelines_get_builds`
2. Check build status: `pipelines_get_build_status`
3. Read build logs: `pipelines_get_build_log`
4. Identify failing step and investigate
