...
- **Iterations:** `work_list_iterations`, `work_list_team_iterations`, `work_get_iteration_capacities`
- **Capacity:** `work_get_team_capacity`, `work_update_team_capacity`
...
### Data Analysis in ADO
- Analyze sprint velocity by querying iteration work items via `wit_get_work_items_for_iteration` across multiple sprints and applying statistical measures
- Track team capacity trends across iterations using `work_get_iteration_capacities` to identify allocation patterns
- Correlate pipeline success rates with sprint metrics by combining `pipelines_get_builds` data with iteration work item completion rates
