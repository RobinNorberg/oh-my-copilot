...
import { writeFileSync, existsSync as existsSync3, mkdirSync, renameSync, unlinkSync, openSync, writeSync, closeSync, realpathSync, constants } from "fs";
...
async function markLeaderPaneMissingDeferred(params) {
  ...
  const logTransitionFailure = (err) => {
    try {
      process.stderr.write(`[omc] team.mcp-comm.markLeaderPaneMissingDeferred transitionDispatchRequest failed: ${err instanceof Error ? err.message : String(err)}
`);
    } catch {
    }
  };
...
function paneCurrentCommandLooksReady(command) {
  const normalized = basename3(command.replace(/\\/g, "/")).replace(/\.(exe|cmd|bat)$/i, "").toLowerCase();
...
function loadAgentPrompt(agentName) {
  ...
  try {
    ...
    const agentPath = join7(agentsDir, `${agentName}.agent.md`);
...
function getConfigDir() {
  ...
  if (!configured) {
    return stripTrailingSep(normalize(join8(home, ".copilot")));
  }
  if (configured === "~") {
    return stripTrailingSep(normalize(home));
  }
  if (configured.startsWith("~/") || configured.startsWith("~\\")) {
    return stripTrailingSep(normalize(join8(home, configured.slice(2))));
  }
...
function getConfigPaths() {
  ...
  const copilotProject = join11(process.cwd(), ".copilot", "omg.jsonc");
  const claudeProject = join11(process.cwd(), ".claude", "omc.jsonc");
  const project = existsSync7(claudeProject) && !existsSync7(copilotProject) ? claudeProject : copilotProject;
  return {
    user: join11(copilotConfigDir, "omg", "config.jsonc"),
    project
  };
}
...
import { existsSync as existsSync8, readFileSync as readFileSync4, readdirSync as readdirSync2, statSync, unlinkSync as unlinkSync2, rmSync, symlinkSync } from "fs";
import { homedir as homedir2 } from "os";
function getConfigDir2() {
  if (process.platform === "win32") {
    return process.env.APPDATA || join12(homedir2(), "AppData", "Roaming");
  }
  return process.env.XDG_CONFIG_HOME || join12(homedir2(), ".config");
}
...
function loadSecurityFromConfigFiles() {
  const paths = [
    join13(process.cwd(), ".copilot", "omc.jsonc"),
    join13(getConfigDir2(), "copilot-omc", "config.jsonc")
  ];
...
async function composeInitialInbox(teamName, workerName, content, cwd, cliOutputContract) {
  const inboxPath = join14(cwd, `.omcp/state/team/${teamName}/workers/${workerName}/inbox.md`);
...
async function appendToInbox(teamName, workerName, message, cwd) {
  ...
  const inboxPath = join14(cwd, `.omcp/state/team/${safeTeam}/workers/${safeWorker}/inbox.md`);
...
async function ensureWorkerStateDir(teamName, workerName, cwd) {
  const workerDir = join14(cwd, `.omcp/state/team/${teamName}/workers/${workerName}`);
  await mkdir3(workerDir, { recursive: true });
  const mailboxDir = join14(cwd, `.omcp/state/team/${teamName}/mailbox`);
  await mkdir3(mailboxDir, { recursive: true });
  const tasksDir = join14(cwd, `.omcp/state/team/${teamName}/tasks`);
  await mkdir3(tasksDir, { recursive: true });
}
async function writeWorkerOverlay(params) {
  const { teamName, workerName, cwd } = params;
  const overlay = generateWorkerOverlay(params);
  const overlayPath = join14(cwd, `.omcp/state/team/${teamName}/workers/${workerName}/AGENTS.md`);
...
var init_worker_bootstrap = __esm({
  "src/team/worker-bootstrap.ts"() {
    ...
    init_fs_utils();
    init_model_contract();
    DEFAULT_INSTRUCTION_STATE_ROOT = ".omcp/state";
  }
});

// src/lib/worktree-cleanup-safety.ts
import { existsSync as existsSync10, lstatSync, realpathSync as realpathSync2 } from "node:fs";
import { homedir as homedir3 } from "node:os";
import { isAbsolute as isAbsolute6, join as join15, parse as parse2, relative as relative3, resolve as resolve3 } from "node:path";
...
import { existsSync as existsSync12, readFileSync as readFileSync8, readdirSync as readdirSync3, rmSync as rmSync2, unlinkSync as unlinkSync5, writeFileSync as writeFileSync2 } from "node:fs";
...
function getWorktreePath(repoRoot, teamName, workerName) {
  return join17(repoRoot, ".omcp", "team", sanitizeName(teamName), "worktrees", sanitizeName(workerName));
}
...
function git(repoRoot, args, cwd = repoRoot) {
  return execFileSync2("git", args, { cwd, encoding: "utf-8", stdio: "pipe" }).trim();
}
...
function getRegisteredWorktreeBranch(repoRoot, wtPath) {
  try {
    ...
    const resolvedWtPath = resolve4(wtPath);
    let currentMatches = false;
    for (const line of output2.split("\n")) {
      if (line.startsWith("worktree ")) {
        currentMatches = resolve4(line.slice("worktree ".length).trim()) === resolvedWtPath;
        continue;
      }
...
function isRegisteredWorktreePath(repoRoot, wtPath) {
  try {
    const output2 = git(repoRoot, ["worktree", "list", "--porcelain"]);
    const resolvedWtPath = resolve4(wtPath);
    return output2.split("\n").some((line) => line.startsWith("worktree ") && resolve4(line.slice("worktree ".length).trim()) === resolvedWtPath);
  } catch {
    return false;
  }
}
function isDetached(wtPath) {
  try {
    const branch = execFileSync2("git", ["branch", "--show-current"], { cwd: wtPath, encoding: "utf-8", stdio: "pipe" }).trim();
...
function isWorktreeDirtyExcept(wtPath, ignoredRootPaths = []) {
  try {
    ...
    const entries = execFileSync2("git", ["status", "--porcelain"], { cwd: wtPath, encoding: "utf-8", stdio: "pipe" }).split("\n").filter((line) => line.trim().length > 0);
...
function getMetadataPath(repoRoot, teamName) {
  return join17(repoRoot, ".omcp", "state", "team", sanitizeName(teamName), "worktrees.json");
}
function getLegacyMetadataPath(repoRoot, teamName) {
  return join17(repoRoot, ".omcp", "state", "team-bridge", sanitizeName(teamName), "worktrees.json");
}
function getWorkerStateDir(repoRoot, teamName, workerName) {
  return join17(repoRoot, ".omcp", "state", "team", sanitizeName(teamName), "workers", sanitizeName(workerName));
}
...
function listRootAgentsBackupIssues(repoRoot, teamName, entries) {
  const workersDir = join17(repoRoot, ".omcp", "state", "team", sanitizeName(teamName), "workers");
...
function writeMetadata(repoRoot, teamName, entries) {
  ...
  ensureDirWithMode(join17(repoRoot, ".omcp", "state", "team", sanitizeName(teamName)));
...
function ensureWorkerWorktree(teamName, workerName, repoRoot, options = {}) {
  ...
  const wtDir = join17(repoRoot, ".omcp", "team", sanitizeName(teamName), "worktrees");
  ...
  execFileSync2("git", args, { cwd: repoRoot, stdio: "pipe" });
  const info = {
    path: wtPath,
    branch,
    workerName,
...
function checkWorkerWorktreeRemovalSafety(teamName, workerName, repoRoot, worktreePath) {
  ...
  validateWorktreeRemovalTarget({
    ...
    expectedRoots: [join17(repoRoot, ".omcp", "team", sanitizeName(teamName), "worktrees")],
...
function removeWorkerWorktree(teamName, workerName, repoRoot) {
  ...
  withFileLockSync(metaLockPath, () => {
    ...
    try {
      execFileSync2("git", ["worktree", "remove", wtPath], { cwd: repoRoot, stdio: "pipe" });
    } catch (err) {
...
function checkMergeConflicts(workerBranch, baseBranch, repoRoot) {
  ...
  const mergeBase = execFileSync3(
    "git",
    ["merge-base", baseBranch, workerBranch],
    { cwd: repoRoot, encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }
  ).trim();
  const baseDiff = execFileSync3(
    "git",
    ["diff", "--name-only", mergeBase, baseBranch],
    { cwd: repoRoot, encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }
  ).trim();
  const workerDiff = execFileSync3(
    "git",
...
function mergeWorkerBranch(workerBranch, baseBranch, repoRoot) {
  ...
  try {
    try {
      execFileSync3("git", ["diff-index", "--quiet", "HEAD", "--"], {
        cwd: repoRoot,
        stdio: "pipe"
      });
    } catch {
    ...
    execFileSync3("git", ["checkout", baseBranch], {
      cwd: repoRoot,
      stdio: "pipe"
    });
    execFileSync3("git", ["merge", "--no-ff", "-m", `Merge ${workerBranch} into ${baseBranch}`, workerBranch], {
      cwd: repoRoot,
      stdio: "pipe"
    });
    ...
    try {
      execFileSync3("git", ["merge", "--abort"], { cwd: repoRoot, stdio: "pipe" });
    } catch {
    }
...
function leaderInboxPath(teamName, cwd) {
  ...
  return join20(cwd, `.omcp/state/team/${safe}/leader/inbox.md`);
}
...
import { join as join21, dirname as dirname13 } from "path";
...
function mergerWorktreePathFor(repoRoot, teamName) {
  return join22(repoRoot, ".omcp", "team", sanitizeName(teamName), "merger");
}
...
function gitRevParseHead(repoRoot, branch) {
  return execFileSync4("git", ["rev-parse", `refs/heads/${branch}`], {
    cwd: repoRoot,
    encoding: "utf-8",
    stdio: "pipe"
  }).trim();
...
function gitPath(worktreePath, gitPathName) {
  try {
    const resolved = execFileSync4("git", ["rev-parse", "--git-path", gitPathName], {
      cwd: worktreePath,
      encoding: "utf-8",
      stdio: "pipe"
    }).trim();
...
function ensureMergerWorktree(repoRoot, mergerPath, leaderBranch) {
  ...
  execFileSync4("git", ["worktree", "add", "--force", mergerPath, leaderBranch], {
    cwd: repoRoot,
    stdio: "pipe"
  });
}
function preflightMergerWorktree(mergerPath, leaderBranch) {
  try {
    execFileSync4("git", ["fetch", "--no-tags", "origin", leaderBranch], {
      cwd: mergerPath,
      stdio: "pipe"
    });
  } catch {
  }
  execFileSync4("git", ["reset", "--hard", leaderBranch], {
    cwd: mergerPath,
    stdio: "pipe"
  });
}
...
async function startMergeOrchestrator(config) {
  ...
  async function fanOutRebase(triggeringWorker) {
    for (const other of workers.values()) {
      ...
      try {
        execFileSync4("git", ["fetch", "--no-tags", "origin", config.leaderBranch], {
          cwd: wtPath,
          stdio: "pipe"
        });
      } catch {
      ...
      try {
        execFileSync4("git", ["rebase", config.leaderBranch], {
          cwd: wtPath,
          stdio: "pipe"
        });
        ...
        const baseSha = (() => {
          try {
            return execFileSync4("git", ["rev-parse", `refs/heads/${config.leaderBranch}`], {
              cwd: config.repoRoot,
              encoding: "utf-8",
              stdio: "pipe"
            }).trim();
...
function resolveLeaderBranch(cwd) {
  const out = execFileSync5("git", ["branch", "--show-current"], {
    cwd,
    encoding: "utf-8",
    stdio: ["pipe", "pipe", "pipe"]
  }).trim();
...
async function startTeamV2(config) {
  ...
  await mkdir10(join23(leaderCwd, ".omcp", "state", "team", sanitized, "mailbox"), { recursive: true });
...
async function findActiveTeamsV2(cwd) {
  const root = join23(cwd, ".omcp", "state", "team");
...
import { readFile as readFile11, readdir as readdir3, rm as rm5 } from "fs/promises";
...
init_platform();
init_state_paths();
import { readFileSync as readFileSync9, readdirSync as readdirSync4, existsSync as existsSync13, openSync as openSync4, closeSync as closeSync4, unlinkSync as unlinkSync6, writeSync as writeSync4, statSync as statSync3, constants as fsConstants2 } from "fs";
...
async function monitorTeam(teamName, cwd, workerPaneIds) {
  ...
  try {
    const { readdir: readdir4 } = await import("fs/promises");
    const taskFiles = await readdir4(join19(root, "tasks"));
...
function teamStateExists(teamName, candidateCwd) {
  ...
  const teamRoot = join24(candidateCwd, ".omcp", "state", "team", teamName);
...
import { readdirSync as readdirSync5, readFileSync as readFileSync11, existsSync as existsSync22 } from "fs";
...
var VALID_CLI_AGENT_TYPES = /* @__PURE__ */ new Set(["claude", "copilot", "codex", "gemini", "cursor", "grok"]);
...
function resolveJobsDir(env = process.env) {
  return env.OMC_JOBS_DIR || join26(homedir4(), ".omcp", "team-jobs");
}
...
function teamStateRoot2(cwd, teamName) {
  return join26(cwd, ".omcp", "state", "team", teamName);
}
