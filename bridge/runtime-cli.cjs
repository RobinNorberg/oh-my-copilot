...
function getDefaultShell() {
  ...
  const name = (0, import_path10.basename)(shell.replace(/\\/g, "/")).replace(/\.(exe|cmd|bat)$/i, "");
...
function pathCandidateNames(candidatePath) {
  const base = (0, import_path10.basename)(candidatePath.replace(/\\/g, "/"));
...
function paneCurrentCommandLooksReady(command) {
  const normalized = (0, import_path10.basename)(command.replace(/\\/g, "/")).replace(/\.(exe|cmd|bat)$/i, "").toLowerCase();
...
function shellNameFromPath(shellPath) {
  const shellName = (0, import_path10.basename)(shellPath.replace(/\\/g, "/"));
...
function isAbsoluteLaunchBinaryPath(value) {
  return (0, import_path10.isAbsolute)(value) || import_path10.win32.isAbsolute(value);
}
...
async function killWorkerPanes(opts) {
  ...
  const shutdownPath = (0, import_path10.join)(cwd, ".omc", "state", "team", teamName, "shutdown.json");
  try {
...
var import_fs7, import_crypto, import_path10, import_promises, sleep, TMUX_SESSION_PREFIX, SUPPORTED_POSIX_SHELLS, ZSH_CANDIDATES, BASH_CANDIDATES, DANGEROUS_LAUNCH_BINARY_CHARS;
...
function loadAgentPrompt(agentName) {
  ...
  try {
    ...
    const agentPath = (0, import_path2.join)(agentsDir, `${agentName}.agent.md`);
...
function getConfigDir() {
  ...
  if (!configured) {
    return stripTrailingSep((0, import_path3.normalize)((0, import_path3.join)(home, ".copilot")));
  }
  if (configured === "~") {
...
function getConfigPaths() {
  ...
  const copilotProject = (0, import_path4.join)(process.cwd(), ".copilot", "omg.jsonc");
  const claudeProject = (0, import_path4.join)(process.cwd(), ".claude", "omc.jsonc");
  const project = (0, import_fs2.existsSync)(claudeProject) && !(0, import_fs2.existsSync)(copilotProject) ? claudeProject : copilotProject;
  return {
    user: (0, import_path4.join)(copilotConfigDir, "omg", "config.jsonc"),
    project
  };
}
function loadJsoncFile(path4) {
  if (!(0, import_fs2.existsSync)(path4)) {
    return null;
  }
  try {
    const content = (0, import_fs2.readFileSync)(path4, "utf-8");
...
function validateTeamConfig(config) {
  ...
  if (ops && typeof ops === "object") {
    if (ops.defaultAgentType !== void 0) {
      if (typeof ops.defaultAgentType !== "string" || !TEAM_ROLE_PROVIDERS.has(ops.defaultAgentType)) {
        throw new Error(
          `[OMCP] team.ops.defaultAgentType: invalid value "${String(ops.defaultAgentType)}". Allowed: ${[...TEAM_ROLE_PROVIDERS].join(", ")}`
        );
      }
    }
  }
  ...
  for (const [rawRoleKey, rawSpec] of Object.entries(roleRouting)) {
    ...
    if (!CANONICAL_TEAM_ROLE_SET.has(normalized)) {
      throw new Error(
        `[OMCP] team.roleRouting: unknown role "${rawRoleKey}". Allowed roles: ${[...CANONICAL_TEAM_ROLE_SET].join(", ")}`
      );
    }
    if (!rawSpec || typeof rawSpec !== "object" || Array.isArray(rawSpec)) {
      throw new Error(
        `[OMCP] team.roleRouting.${rawRoleKey}: must be an object, got ${Array.isArray(rawSpec) ? "array" : typeof rawSpec}`
      );
    }
    const spec = rawSpec;
    if (normalized === "orchestrator") {
      for (const key of Object.keys(spec)) {
        if (key !== "model") {
          throw new Error(
            `[OMCP] team.roleRouting.orchestrator: key "${key}" is not allowed (orchestrator is pinned to claude; only "model" is configurable)`
          );
        }
      }
      if (spec.model !== void 0 && !isValidModelValue(spec.model)) {
        throw new Error(
          `[OMCP] team.roleRouting.orchestrator.model: must be a tier name (HIGH|MEDIUM|LOW) or model ID string, got ${typeof spec.model}`
        );
      }
      continue;
    }
    if (spec.provider !== void 0) {
      if (typeof spec.provider !== "string" || !TEAM_ROLE_PROVIDERS.has(spec.provider)) {
        throw new Error(
          `[OMCP] team.roleRouting.${rawRoleKey}.provider: invalid value "${String(spec.provider)}". Allowed: ${[...TEAM_ROLE_PROVIDERS].join(", ")}`
        );
      }
    }
    if (spec.model !== void 0 && !isValidModelValue(spec.model)) {
      throw new Error(
        `[OMCP] team.roleRouting.${rawRoleKey}.model: must be a tier name (HIGH|MEDIUM|LOW) or a non-empty model ID string`
      );
    }
    if (spec.agent !== void 0) {
      if (typeof spec.agent !== "string" || !KNOWN_AGENT_NAME_SET.has(spec.agent)) {
        throw new Error(
          `[OMCP] team.roleRouting.${rawRoleKey}.agent: unknown agent "${String(spec.agent)}". Allowed: ${[...KNOWN_AGENT_NAME_SET].join(", ")}`
        );
      }
    }
  }
...
function parseTeamRoleOverridesFromEnv() {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      console.warn(
        "[OMCP] OMCP_TEAM_ROLE_OVERRIDES: expected a JSON object; ignoring."
      );
      return void 0;
    }
    return parsed;
    ...
    console.warn(
      `[OMCP] OMCP_TEAM_ROLE_OVERRIDES: invalid JSON, ignoring (${err.message})`
    );
    return void 0;
  }
}
...
var DOCUMENT_SPECIALIST_PROMPT_METADATA = {
  ...
  triggers: [
    { domain: "Project documentation", trigger: "README, docs/, migration guides, local references" },
    { domain: "External documentation", trigger: "API references, official docs" },
    { domain: "API/framework correctness", trigger: "Context Hub / chub first when available; curated backend fallback otherwise" },
...
function loadSecurityFromConfigFiles() {
  const paths = [
    (0, import_path7.join)(process.cwd(), ".copilot", "omc.jsonc"),
    (0, import_path7.join)(getConfigDir2(), "copilot-omc", "config.jsonc")
  ];
...
function isTrustedPrefix(resolvedPath) {
  const normalized = (0, import_path8.normalize)(resolvedPath);
  return getTrustedPrefixes().some((prefix) => {
    const p = (0, import_path8.normalize)(prefix);
    if (normalized === p) return true;
    const withSep = p.endsWith(import_path8.sep) ? p : p + import_path8.sep;
...
function resolveCliBinaryPath(binary) {
  ...
  const resolvedPath = (0, import_path8.normalize)(firstLine);
  if (!(0, import_path8.isAbsolute)(resolvedPath)) {
...
function validateBinaryRef(binary) {
  if ((0, import_path8.isAbsolute)(binary)) return;
...
function resolveBinaryPath(binary) {
  ...
  if ((0, import_path8.isAbsolute)(binary)) return binary;
  try {
    ...
    const isResolvedAbsolute = !!firstPath && ((0, import_path8.isAbsolute)(firstPath) || import_path8.win32.isAbsolute(firstPath));
...
function atomicWriteJson(filePath, data, mode = 384) {
  ...
  (0, import_fs8.writeFileSync)(tmpPath, serialized, { encoding: "utf-8", mode });
  try {
    (0, import_fs8.renameSync)(tmpPath, filePath);
  } catch (err) {
    if (process.platform === "win32" && err && typeof err === "object" && "code" in err && err.code === "EPERM") {
      try {
        (0, import_fs8.writeFileSync)(filePath, serialized, { encoding: "utf-8", mode });
      } finally {
        try {
...
async function composeInitialInbox(teamName, workerName2, content, cwd, cliOutputContract) {
  const inboxPath = (0, import_path12.join)(cwd, `.omcp/state/team/${teamName}/workers/${workerName2}/inbox.md`);
...
async function appendToInbox(teamName, workerName2, message, cwd) {
  ...
  const inboxPath = (0, import_path12.join)(cwd, `.omcp/state/team/${safeTeam}/workers/${safeWorker}/inbox.md`);
...
async function ensureWorkerStateDir(teamName, workerName2, cwd) {
  const workerDir = (0, import_path12.join)(cwd, `.omcp/state/team/${teamName}/workers/${workerName2}`);
  await (0, import_promises2.mkdir)(workerDir, { recursive: true });
  const mailboxDir = (0, import_path12.join)(cwd, `.omcp/state/team/${teamName}/mailbox`);
  await (0, import_promises2.mkdir)(mailboxDir, { recursive: true });
  const tasksDir = (0, import_path12.join)(cwd, `.omcp/state/team/${teamName}/tasks`);
...
async function writeWorkerOverlay(params) {
  ...
  const overlayPath = (0, import_path12.join)(cwd, `.omcp/state/team/${teamName}/workers/${workerName2}/AGENTS.md`);
...
function getWorktreePath(repoRoot, teamName, workerName2) {
  return (0, import_node_path2.join)(repoRoot, ".omcp", "team", sanitizeName(teamName), "worktrees", sanitizeName(workerName2));
}
...
function getRegisteredWorktreeBranch(repoRoot, wtPath) {
  try {
    ...
    const resolvedWtPath = (0, import_node_path2.resolve)(wtPath);
    let currentMatches = false;
    for (const line of output.split("\n")) {
      if (line.startsWith("worktree ")) {
        currentMatches = (0, import_node_path2.resolve)(line.slice("worktree ".length).trim()) === resolvedWtPath;
        continue;
      }
...
function isRegisteredWorktreePath(repoRoot, wtPath) {
  try {
    const output = git(repoRoot, ["worktree", "list", "--porcelain"]);
    const resolvedWtPath = (0, import_node_path2.resolve)(wtPath);
    return output.split("\n").some((line) => line.startsWith("worktree ") && (0, import_node_path2.resolve)(line.slice("worktree ".length).trim()) === resolvedWtPath);
  } catch {
    return false;
  }
}
...
function getMetadataPath(repoRoot, teamName) {
  return (0, import_node_path2.join)(repoRoot, ".omcp", "state", "team", sanitizeName(teamName), "worktrees.json");
}
function getLegacyMetadataPath(repoRoot, teamName) {
  return (0, import_node_path2.join)(repoRoot, ".omcp", "state", "team-bridge", sanitizeName(teamName), "worktrees.json");
}
function getWorkerStateDir(repoRoot, teamName, workerName2) {
  return (0, import_node_path2.join)(repoRoot, ".omcp", "state", "team", sanitizeName(teamName), "workers", sanitizeName(workerName2));
}
...
function listRootAgentsBackupIssues(repoRoot, teamName, entries) {
  const workersDir = (0, import_node_path2.join)(repoRoot, ".omcp", "state", "team", sanitizeName(teamName), "workers");
...
function writeMetadata(repoRoot, teamName, entries) {
  ...
  ensureDirWithMode((0, import_node_path2.join)(repoRoot, ".omcp", "state", "team", sanitizeName(teamName)));
...
function ensureWorkerWorktree(teamName, workerName2, repoRoot, options = {}) {
  ...
  const wtDir = (0, import_node_path2.join)(repoRoot, ".omcp", "team", sanitizeName(teamName), "worktrees");
...
function checkWorkerWorktreeRemovalSafety(teamName, workerName2, repoRoot, worktreePath) {
  ...
  validateWorktreeRemovalTarget({
    ...
    expectedRoots: [(0, import_node_path2.join)(repoRoot, ".omcp", "team", sanitizeName(teamName), "worktrees")],
...
var TeamPaths = {
  ...
  workerDir: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}`,
  heartbeat: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/heartbeat.json`,
  inbox: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/inbox.md`,
  outbox: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/outbox.jsonl`,
  ready: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/.ready`,
  overlay: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/AGENTS.md`,
  shutdownAck: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/shutdown-ack.json`,
  mailbox: (teamName, workerName2) => `.omcp/state/team/${teamName}/mailbox/${workerName2}.json`,
  mailboxLockDir: (teamName, workerName2) => `.omcp/state/team/${teamName}/mailbox/.lock-${workerName2}`,
  ...
  workerStatus: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/status.json`,
  workerIdleNotify: (teamName) => `.omcp/state/team/${teamName}/worker-idle-notify.json`,
  workerPrevNotifyState: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/prev-notify-state.json`,
  ...
  workerIdentity: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/identity.json`,
  workerAgentsMd: (teamName) => `.omcp/state/team/${teamName}/worker-agents.md`,
  shutdownRequest: (teamName, workerName2) => `.omcp/state/team/${teamName}/workers/${workerName2}/shutdown-request.json`
};
...
function canonicalTasksDir(teamName, cwd) {
  ...
  validateResolvedPath(dir, (0, import_path14.join)(root, ".omcp", "state", "team"));
  return dir;
}
...
function stateRoot(cwd, teamName) {
  validateTeamName(teamName);
  return (0, import_path15.join)(cwd, `.omcp/state/team/${teamName}`);
}
...
function buildInitialTaskInstruction(teamName, workerName2, task, taskId) {
  const donePath = `.omcp/state/team/${teamName}/workers/${workerName2}/done.json`;
  return [
...
function expandTokens(value, workspace) {
  ...
  return value.replace(/\$\{HOME\}/g, home).replace(/\$\{WORKSPACE\}/g, ws).replace(/\$\{COPILOT_CONFIG_DIR\}/g, getConfigDir());
}
...
function normalizeTeamManifest(manifest) {
  return {
    ...manifest,
    policy: { ...normalizeTeamTransportPolicy(manifest.policy), ...normalizeTeamGovernance(manifest.governance, manifest.policy) },
...
async function markImmediateDispatchFailure(params) {
  ...
  const logTransitionFailure = (err) => {
    try {
      process.stderr.write(`[omc] team.mcp-comm.markImmediateDispatchFailure transitionDispatchRequest failed: ${err instanceof Error ? err.message : String(err)}
`);
    } catch {
    }
  };
...
function leaderInboxPath(teamName, cwd) {
  ...
  return (0, import_path20.join)(cwd, `.omcp/state/team/${safe}/leader/inbox.md`);
}
...
function extendLeaderBootstrapPrompt(teamName) {
  const safe = sanitizeName(teamName);
  const path4 = `.omcp/state/team/${safe}/leader/inbox.md`;
...
function mergerWorktreePathFor(repoRoot, teamName) {
  return (0, import_node_path3.join)(repoRoot, ".omcp", "team", sanitizeName(teamName), "merger");
}
...
async function startTeamV2(config) {
  ...
  await (0, import_promises10.mkdir)((0, import_path22.join)(leaderCwd, ".omcp", "state", "team", sanitized, "mailbox"), { recursive: true });
...
`);
  ...
  const stateRoot2 = (0, import_path23.join)(cwd, `.omcp/state/team/${teamName}`);
  const config = {
    teamName,
    workerCount,
    agentTypes,
