...
function extractCommandStrings(entry: HookCommandConfig | HooksJson['hooks'] extends infer T ? unknown : never): string[] {
  const commands: string[] = [];
  const cfg = entry as HookCommandConfig;
  if (typeof cfg.command === 'string') commands.push(cfg.command);
  if (typeof cfg.bash === 'string') commands.push(cfg.bash);
  if (typeof cfg.powershell === 'string') commands.push(cfg.powershell);
  return commands;
}
...
function listHookScriptEntries(): string[] {
  ...
  for (const eventHooks of Object.values(hooksJson.hooks ?? {})) {
    for (const matcherEntry of eventHooks) {
      ...
      const nestedHooks = (matcherEntry as { hooks?: HookCommandConfig[] }).hooks ?? [];
      const directCommands = extractCommandStrings(matcherEntry as HookCommandConfig);
      const candidateCommands: string[] = [...directCommands];
