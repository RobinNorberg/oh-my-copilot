...
describe('auto-update reconciliation', () => {
  ...
  it('does not persist metadata when reconciliation fails', async () => {
    ...
    delete process.env.OMC_UPDATE_RECONCILE;
    if (savedEntrypoint2 !== undefined) process.env.CLAUDE_CODE_ENTRYPOINT = savedEntrypoint2;
    else delete process.env.CLAUDE_CODE_ENTRYPOINT;
  });
  ...
  it('preserves non-OMC hooks when refreshing plugin hooks during reconciliation', () => {
    ...
    mockedExistsSync.mockImplementation((path) => {
      ...
      if (normalized.endsWith('/.copilot/hud')) {
        return false;
      }
    ...
    process.env.CLAUDE_PLUGIN_ROOT = join(homedir(), '.copilot', 'plugins', 'cache', 'omg', 'oh-my-copilot', '4.1.5');
    ...
    if (settingsWrite) {
      ...
      expect(writtenSettings.hooks.UserPromptSubmit[0].hooks[0].command).toBe('node $HOME/.copilot/hooks/other-plugin.mjs');
    }
