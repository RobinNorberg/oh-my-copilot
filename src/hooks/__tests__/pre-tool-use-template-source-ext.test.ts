...
describe('pre-tool-use template source extension detection', () => {
  ...
  describe('throwaway scratchpad and temporary file writes stay quiet (Class 1)', () => {
    it.each([
      ...
      expect(output.continue).toBe(true);
      // POSIX temp shapes are only recognized on POSIX hosts; on Windows the
      // bounded temp check rejects them and the delegation notice fires.
      expect(hasDelegationNotice(output)).toBe(process.platform === 'win32');
    });
  ...
  describe('bounded path allowance agrees with the TypeScript helper', () => {
    it.each([
      ...
      ['/tmp/omc-fixture.ts', '/home/project', process.platform !== 'win32'],
    ...
    it('keeps absolute project metadata and COPILOT_CONFIG_DIR decisions in parity', () => {
      ...
      try {
        ...
        if (previousConfig === undefined) delete process.env.COPILOT_CONFIG_DIR;
        else process.env.COPILOT_CONFIG_DIR = previousConfig;
  ...
  describe('quote-aware Bash mutation matrix', () => {
    it.each([
      ...
      // /tmp is only a recognized temp root on POSIX hosts; Windows warns.
      ['copy source into an explicit temp target directory', 'cp -t /tmp src/app.ts', process.platform === 'win32'],
      ['install source into an explicit temp target directory', 'install --target-directory=/tmp src/app.ts', process.platform === 'win32'],
