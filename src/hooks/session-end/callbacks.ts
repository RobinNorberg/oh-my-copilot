...
export function interpolatePath(pathTemplate: string, sessionId: string): string {
  ...
  const interpolated = pathTemplate
    .replace(/~/g, homedir())
    .replace(/\{session_id\}/g, safeSessionId)
    .replace(/\{date\}/g, date)
    .replace(/\{time\}/g, time);
  // Normalize path separators to forward slashes for cross-platform consistency.
  // path.normalize() converts to backslashes on Windows which breaks tests that
  // use POSIX-style paths like /tmp/... as template strings.
