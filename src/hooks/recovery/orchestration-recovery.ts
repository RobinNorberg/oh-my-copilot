...
import type { OrchestrationFailureType, RecoveryAction, RecoveryActionResult } from './types.js';
...
const FAILURE_PATTERNS: Array<{ type: OrchestrationFailureType; patterns: RegExp[] }> = [
  {
    type: 'rate_limited',
    patterns: [
      /429/,
 ...
 */
export function classifyOrchestrationFailure(error: string): OrchestrationFailureType {
  if (!error || !error.trim()) {
    return 'unknown';
  }
...
const ACTION_MAP: Record<OrchestrationFailureType, RecoveryAction> = {
...
export function determineRecoveryAction(
  ...
  if (baseAction === 'retry_with_backoff') {
    const backoffMs = Math.min(
      ORCHESTRATION_RECOVERY_CONFIG.initialBackoffMs * Math.pow(ORCHESTRATION_RECOVERY_CONFIG.backoffMultiplier, attemptCount),
      ORCHESTRATION_RECOVERY_CONFIG.maxBackoffMs,
    );
    return {
      action: baseAction,
