...
import type { AttemptRecord, AttemptHistory, OrchestrationFailureType, RecoveryAction } from './types.js';
import { ORCHESTRATION_RECOVERY_CONFIG } from './types.js';

/**
 * Read attempt history from session state
 */
export function readAttemptHistory(
  directory: string,
  sessionId?: string,
): AttemptHistory | null {
  return readModeState<AttemptHistory>('attempt-history', directory, sessionId);
}
...
export function recordAttempt(
  ...
  history.attempts[taskId] = history.attempts[taskId].filter(
...
export function getAttemptCount(
  ...
  return history.attempts[taskId].filter(
    (a) => new Date(a.timestamp).getTime() > cutoff,
  ).length;
}
