 ...
 *
 * Error history stored in .omg/state/sessions/{sessionId}/error-history.json
 */

import { writeModeState, readModeState } from '../../lib/mode-state-io.js';
import type { ErrorRecord, CircularFixState } from './types.js';
 ...
 */
export function readErrorHistory(
  directory: string,
  sessionId?: string,
): CircularFixState | null {
  return readModeState<CircularFixState>('error-history', directory, sessionId);
}

/**
 * Write error history to session state
 */
export function writeErrorHistory(
  directory: string,
  state: CircularFixState,
  sessionId?: string,
): boolean {
...
export function recordError(
  ...
  state.errors[taskId] = state.errors[taskId].filter(
...
export function isCircularFix(
  ...
  const recentErrors = state.errors[taskId].filter(
    (e) => new Date(e.timestamp).getTime() > cutoff,
  );
 ...
 */
function getMostRecurringError(
  errors: ErrorRecord[],
  cutoff: number,
): { hash: string; count: number; message: string } | null {
  const recentErrors = errors.filter(
    (e) => new Date(e.timestamp).getTime() > cutoff,
  );

  const hashCounts = new Map<string, { count: number; message: string }>();
  for (const error of recentErrors) {
  ...
  let maxEntry: { hash: string; count: number; message: string } | null = null;
 ...
 */
export function getEscalationReport(
  directory: string,
  sessionId: string,
  taskId: string,
): string {
  ...
  lines.push(
    '## Recommended Actions',
    '',
    '1. **Stop automated retries** — the same fix is being attempted repeatedly',
    '2. **Review the root cause** — the error pattern suggests a fundamental issue that automated fixes cannot resolve',
    '3. **Consider manual intervention** — examine the affected files and error context',
    '4. **Check for environmental issues** — missing dependencies, configuration, or permissions',
    '',
    '## Error Timeline',
    '',
    '| Time | Hash | Message |',
