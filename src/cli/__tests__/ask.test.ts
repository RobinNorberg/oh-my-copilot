...
function writeAdvisorStub(dir: string): string {
  ...
  writeFileSync(
    ...
    [
      ...
      '// Read prompt from temp file (new protocol) or fall back to argv (legacy/wrapper)',
      'let prompt = process.argv[3] ?? null;',
      'const promptFile = process.env.OMC_ASK_PROMPT_FILE;',
      'if (promptFile) { try { prompt = readFileSync(promptFile, "utf-8").trim(); } catch {} }',
      'const payload = {',
  ...
  it('writes artifact to .omcp/artifacts/ask/{provider}-{slug}-{timestamp}.md', () => {
    ...
    try {
      ...
      const result = runAdvisorScript(
        ['claude', '--print', 'artifact-contract'],
        wd,
...
describe('resolveAskAdvisorScriptPath', () => {
  it('resolves canonical env and supports package-root relative paths', () => {
    ...
    expect(resolveAskAdvisorScriptPath(packageRoot, { OMC_ASK_ADVISOR_SCRIPT: optCustomJs } as NodeJS.ProcessEnv))
      .toBe(optCustomJs);
  });
});
