...
function readJsonMaybePath(raw: string | undefined): unknown | undefined {
...
function parseCriticalIntegrations(
  raw: string | undefined,
...
export async function aliasRetirementCommand(args: string[]): Promise<void> {
  const sub = args[0];
  ...
  if (hasFlag(verifyArgs, '--check-eligible')) {
    if (summary.anyEligible) process.exitCode = 2;
