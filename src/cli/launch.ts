 ...
 */
export function extractTeamsFlag(args: string[]): { teamsEnabled: boolean | undefined; remainingArgs: string[] } {
  let teamsEnabled: boolean | undefined = undefined;
  const remainingArgs: string[] = [];
  for (const arg of args) {
    if (arg === TEAMS_FLAG) { teamsEnabled = true; continue; }
 ...
 */
export function runCopilot(cwd: string, args: string[], sessionId: string): void {
