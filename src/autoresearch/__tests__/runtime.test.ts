  ...
  it.skipIf(process.platform === 'win32')('keeps improved candidates and resets discarded candidates back to the last kept commit', async () => {
    ...
    try {
      ...
      const decisionLog = await readFile(
        join(repo, '.omcp', 'autoresearch', 'missions-demo', 'runs', runtime.runId, 'decision-log.md'),
        'utf-8',
      );
      ...
      const evaluationOne = JSON.parse(await readFile(
        join(repo, '.omcp', 'autoresearch', 'missions-demo', 'runs', runtime.runId, 'evaluations', 'iteration-0001.json'),
        'utf-8',
