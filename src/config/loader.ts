...
export function loadEnvConfig(): Partial<PluginConfig> {
  ...
  if (process.env.OMC_DELEGATION_ROUTING_DEFAULT_PROVIDER) {
    ...
    if (['claude', 'copilot', 'codex', 'gemini'].includes(provider)) {
      config.delegationRouting = {
        ...config.delegationRouting,
        defaultProvider: provider as 'claude' | 'copilot' | 'codex' | 'gemini',
      };
    }
  }
...
function parseTeamRoleOverridesFromEnv(): Record<string, TeamRoleAssignmentSpec> | undefined {
  ...
  try {
    ...
    console.warn(
      `[OMCP] OMCP_TEAM_ROLE_OVERRIDES: invalid JSON, ignoring (${(err as Error).message})`,
    );
    return undefined;
  }
}
...
export function loadConfig(): PluginConfig {
  ...
  // Triggers for: CC Switch / LiteLLM (non-Copilot model IDs), custom
  // ANTHROPIC_BASE_URL, AWS Bedrock (CLAUDE_CODE_USE_BEDROCK=1), and
  // Google Vertex AI (CLAUDE_CODE_USE_VERTEX=1). Passing Copilot-specific
        ...
        properties: {
          ...
          roles: {
            ...
            additionalProperties: {
              type: 'object',
              properties: {
                provider: { type: 'string', enum: ['claude', 'copilot', 'codex', 'gemini'] },
