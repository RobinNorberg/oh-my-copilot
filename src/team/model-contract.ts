...
export function resolveDefaultWorkerModel(
  ...
  // copilot is this fork's native provider and shares the claude resolution
  // path: no external-models default, Bedrock/Vertex-only env resolution.
  if (agentType === 'claude' || agentType === 'copilot') return resolveClaudeWorkerModel(env);
  const providerConfigKeys: Record<Exclude<CliAgentType, 'claude' | 'copilot'>, keyof ExternalModelsDefaults> = {
    codex: 'codexModel',
    gemini: 'geminiModel',
