 ...
 *
 * See: https://github.com/anthropics/copilot-cli/issues/1000
 */
...
describe('spawnWorkerForTask – model passthrough from environment variables', () => {
  ...
  it('copilot worker does not pass model flag (not supported)', async () => {
    process.env.OMC_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL = 'gpt-4o';
    const runtime = makeRuntime(cwd, 'claude');
    ...
    // Copilot worker should not have --model flag
    expect(launchCmd).not.toContain("'--model'");
  });

  it('copilot worker propagates ANTHROPIC_MODEL into the pane startup env', async () => {
  ...
  it('copilot worker propagates custom provider env needed for inherited model selection', async () => {
  ...
  it('copilot worker propagates tiered Bedrock/env model selection variables', async () => {
