...
describe('runtime v2 startup inbox dispatch', () => {
  ...
  beforeEach(() => {
    ...
    modelContractMocks.buildWorkerArgv.mockImplementation((agentType?: string) => [`/usr/bin/${agentType ?? 'copilot'}`]);
    modelContractMocks.resolveValidatedBinaryPath.mockImplementation((agentType?: string) => `/usr/bin/${agentType ?? 'copilot'}`);
