...
export function validateConfigPath(configPath: string, homeDir: string, claudeConfigDir: string): boolean {
  ...
  // Normalize to forward slashes for cross-platform string comparison
  const n = (p: string) => p.replace(/\\/g, '/');
  const resolved = n(resolve(configPath));
  ...
  const isTrustedSubpath =
    resolved === normConfigDir ||
    resolved.startsWith(normConfigDir + '/') ||
    resolved === normOmcDir ||
    resolved.startsWith(normOmcDir + '/') ||
    hasOmcComponent;
  ...
  try {
    ...
    const realParent = n(realpathSync(parentDir));
...
function validateBridgeWorkingDirectory(workingDirectory: string): void {
  ...
  // Resolve symlinks and verify under homedir (normalize separators for cross-platform comparison)
  const n = (p: string) => p.replace(/\\/g, '/');
