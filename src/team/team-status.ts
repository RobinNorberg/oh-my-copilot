...
function peekRecentOutboxMessages(
  ...
  const outboxPath = join(getCopilotConfigDir(), 'teams', safeName, 'outbox', `${safeWorker}.jsonl`);
...
export interface WorkerStatus {
  workerName: string;
  provider: 'claude' | 'copilot' | 'codex' | 'gemini' | 'grok' | 'cursor' | 'antigravity';
...
export function getTeamStatus(
  ...
  const workers: WorkerStatus[] = mcpWorkers.map(w => {
    ...
    const provider = w.agentType.replace(/^(?:mcp|tmux)-/, '') as 'claude' | 'copilot' | 'codex' | 'gemini' | 'grok' | 'cursor' | 'antigravity';
