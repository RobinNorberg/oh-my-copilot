...
if [ "$SKIP_VANILLA" = false ]; then
    log_step "Step 1/4: Running vanilla Copilot CLI benchmark..."
    echo ""
...
# Step 2: Run OMP benchmark
if [ "$SKIP_OMC" = false ]; then
    log_step "Step 2/4: Running OMP-enhanced benchmark..."
    echo ""
    "$SCRIPT_DIR/run_omc.sh" $ARGS
    if [ $? -ne 0 ]; then
        log_error "OMP benchmark failed. Aborting."
        exit 1
    fi
    echo ""
else
    log_warn "Skipping OMP benchmark (--skip-omp)"
    echo ""
fi
...
if [ "$SKIP_EVAL" = false ]; then
    ...
    log_step "Step 4/4: Evaluating OMP predictions..."
    echo ""
    if [ -f "$SCRIPT_DIR/evaluate.py" ]; then
        python3 "$SCRIPT_DIR/evaluate.py" \
            --predictions "$SCRIPT_DIR/predictions/omp" \
            --output "$SCRIPT_DIR/results/omc_results.json"
        if [ $? -ne 0 ]; then
            log_warn "OMP evaluation had issues (continuing...)"
        fi
    else
        log_warn "evaluate.py not found, skipping evaluation"
    fi
...
if [ -f "$SCRIPT_DIR/compare_results.py" ]; then
    python3 "$SCRIPT_DIR/compare_results.py" \
        ...
        --omp "$SCRIPT_DIR/predictions/omp/predictions.jsonl" \
...
log_info "  OMP predictions:     $SCRIPT_DIR/predictions/omp/"
