...
and understand differences between vanilla and OMP runs.

Usage:
    python analyze_failures.py --results results/vanilla/ --predictions predictions.json
    python analyze_failures.py --vanilla results/vanilla/ --omp results/omp/ --compare
"""
...
def compare_failures(
    ...
    """Compare failure patterns between vanilla and OMP."""
    comparison = {
        "timestamp": datetime.now().isoformat(),
    ...
    if len(comparison["unique_to_vanilla"]) > len(comparison["unique_to_omc"]):
        insights.append({
            "type": "positive",
            "message": f"OMP fixed {len(comparison['unique_to_vanilla'])} failures that vanilla couldn't solve."
        })
    elif len(comparison["unique_to_omc"]) > len(comparison["unique_to_vanilla"]):
        insights.append({
            "type": "negative",
            "message": f"OMP introduced {len(comparison['unique_to_omc'])} new failures compared to vanilla."
        })
    ...
    for category, counts in comparison["category_comparison"].items():
        if counts["delta"] < -2:
            insights.append({
                "type": "positive",
                "message": f"OMP reduced '{category}' failures by {abs(counts['delta'])}."
            })
        elif counts["delta"] > 2:
            insights.append({
                "type": "negative",
                "message": f"OMP increased '{category}' failures by {counts['delta']}."
            })
...
def generate_failure_report(
    ...
    if comparison:
        lines.extend([
            "",
            "## Vanilla vs OMP Comparison",
            "",
            f"- **Vanilla Failures:** {comparison['vanilla_failures']}",
            f"- **OMP Failures:** {comparison['omc_failures']}",
            f"- **Fixed by OMP:** {len(comparison['unique_to_vanilla'])}",
            f"- **New in OMP:** {len(comparison['unique_to_omc'])}",
            f"- **Common Failures:** {len(comparison['common_failures'])}",
            "",
            "### Category Changes",
            "",
            "| Category | Vanilla | OMP | Delta |",
            "|----------|---------|-----|-------|",
        ])
        ...
        for category, counts in sorted(
            ...
            lines.append(f"| {category} | {counts['vanilla']} | {counts['omp']} | {delta_str} |")
...
Examples:
    ...
    python analyze_failures.py --results results/omp/ --predictions predictions.json

    # Compare vanilla vs OMP failures
    python analyze_failures.py --vanilla results/vanilla/ --omp results/omp/ --compare
        """
    )
    ...
    parser.add_argument(
        "--omp",
        type=Path,
        help="Path to OMP results for comparison"
    )

    parser.add_argument(
        "--compare",
        action="store_true",
        help="Compare vanilla vs OMP (requires --vanilla and --omp)"
    )
    ...
    # Validate arguments
    if args.compare:
        if not args.vanilla or not args.omp:
            parser.error("--compare requires both --vanilla and --omp")
    elif not args.results:
        parser.error("Either --results or (--vanilla, --omp, --compare) required")
    ...
    if args.compare:
        ...
        for pred_path in [args.omp / "predictions.json", args.omp.parent / "omc_predictions.json"]:
            if pred_path.exists():
        ...
        print(f"OMP Failures:     {omc_analysis['total_failures']}")
        print(f"Fixed by OMP:     {len(comparison['unique_to_vanilla'])}")
        print(f"New in OMP:       {len(comparison['unique_to_omc'])}")
