...
Compare evaluation results between vanilla Copilot CLI and OMP-enhanced runs.
Generates detailed comparison reports in multiple formats.

Usage:
    python compare_results.py --vanilla results/vanilla/ --omp results/omp/
...
def compare_results(
    ...
    """
    Compare vanilla and OMP results.
    ...
    comparison = {
        ...
        "regressions": {},
        "per_instance": {},
        "categories": defaultdict(lambda: {"vanilla": 0, "omp": 0})
    }
    ...
    for instance_id in all_instances:
        ...
        if "__" in instance_id:
            ...
            if omc_passed:
                comparison["categories"][repo]["omp"] += 1
...
def generate_markdown_report(comparison: dict[str, Any]) -> str:
    ...
    lines = [
        "# SWE-bench Comparison Report: Vanilla vs OMP",
        "",
    ...
    if delta["pass_rate"] > 0:
        lines.append(f"OMP improved pass rate by **{delta['pass_rate']:.1f} percentage points** "
                    f"({vanilla['pass_rate']:.1f}% -> {omp['pass_rate']:.1f}%).")
    elif delta["pass_rate"] < 0:
        lines.append(f"OMP decreased pass rate by **{abs(delta['pass_rate']):.1f} percentage points** "
                    f"({vanilla['pass_rate']:.1f}% -> {omp['pass_rate']:.1f}%).")
    else:
        lines.append("Pass rates are identical between vanilla and OMP.")

    lines.extend([
        "",
        f"- **Improvements:** {comparison['improvements']['count']} instances that vanilla failed but OMP passed",
        f"- **Regressions:** {comparison['regressions']['count']} instances that vanilla passed but OMP failed",
        "",
        "## Overall Metrics",
        "",
        "| Metric | Vanilla | OMP | Delta |",
        "|--------|---------|-----|-------|",
        f"| Total Instances | {vanilla['total']} | {omp['total']} | - |",
        f"| Passed | {vanilla['passed']} | {omp['passed']} | {delta['passed']:+d} |",
        f"| Failed | {vanilla['failed']} | {omp['failed']} | {omp['failed'] - vanilla['failed']:+d} |",
        f"| **Pass Rate** | **{vanilla['pass_rate']:.2f}%** | **{omp['pass_rate']:.2f}%** | **{delta['pass_rate']:+.2f}pp** |",
        "",
        "## Resource Usage",
        "",
        "| Metric | Vanilla | OMP | Change |",
        "|--------|---------|-----|--------|",
    ])
    ...
    lines.append(f"| Avg Tokens/Instance | {vanilla['avg_tokens']:,.0f} | {omp['avg_tokens']:,.0f} | {token_change_str} |")

    # Time comparison
    time_change = delta.get("time_change_pct", 0)
    time_change_str = f"{time_change:+.1f}%" if time_change else "N/A"
    lines.append(f"| Avg Time/Instance | {vanilla['avg_time_seconds']:.1f}s | {omp['avg_time_seconds']:.1f}s | {time_change_str} |")

    # Cost comparison
    lines.append(f"| Total Cost | ${vanilla['total_cost_usd']:.2f} | ${omp['total_cost_usd']:.2f} | ${omp['total_cost_usd'] - vanilla['total_cost_usd']:+.2f} |")

    lines.extend([
        "",
        "## Improvements (Vanilla FAIL -> OMP PASS)",
        "",
    ])
    ...
    lines.extend([
        "",
        "## Regressions (Vanilla PASS -> OMP FAIL)",
        "",
    ])
    ...
    if comparison["categories"]:
        lines.extend([
            ...
            "",
            "| Repository | Vanilla Passed | OMP Passed | Delta |",
            "|------------|----------------|------------|-------|",
        ])

        for repo, counts in sorted(comparison["categories"].items()):
            delta_count = counts["omp"] - counts["vanilla"]
            lines.append(f"| {repo} | {counts['vanilla']} | {counts['omp']} | {delta_count:+d} |")
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare SWE-bench results between vanilla and OMP runs",
...
Examples:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to vanilla Copilot CLI results directory"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to OMP-enhanced results directory"
    )
    ...
    if not args.omp.exists():
        logger.error(f"OMP results directory not found: {args.omp}")
        return 1
    ...
    logger.info(f"Loading OMP results from {args.omp}")
    omc_results = load_results(args.omp)
    ...
    print(f"OMP Pass Rate:     {comparison['overall']['omp']['pass_rate']:.2f}%")
