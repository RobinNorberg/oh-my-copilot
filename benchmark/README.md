...
Automated benchmark comparison between vanilla Copilot CLI and OMP-enhanced Copilot CLI.
...
- Tests both vanilla and OMP modes
...
### run_vanilla.sh
Run vanilla Copilot CLI benchmark:
- Standard Copilot CLI without OMP
...
- `--model MODEL` - Copilot model to use (default: claude-sonnet-4.5-20250929)
...
./run_full_comparison.sh --skip-vanilla     # Only run OMP (reuse vanilla results)
```
...
│   ├── omc_results.json      # OMP evaluation
...
2. **Parallel Runs**: Don't run vanilla and OMP in parallel (share API rate limits)
