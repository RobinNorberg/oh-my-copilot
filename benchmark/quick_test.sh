...
# Step 2: Run quick OMP test
log_step "Step 2/2: Quick OMP test ($TEST_LIMIT instances)..."
echo ""
...
if [ $VANILLA_STATUS -eq 0 ] && [ $OMP_STATUS -eq 0 ]; then
    ...
    [ $OMP_STATUS -ne 0 ] && log_error "  OMP test: FAILED (exit code $OMP_STATUS)"
    echo ""
