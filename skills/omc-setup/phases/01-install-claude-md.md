...
For `local` installs inside a git repository, the script also seeds `.git/info/exclude` with an OMC block that re-includes `.omcp/`, ignores local `.omcp/*` artifacts by default, and preserves `.omcp/skills/` for project skills you intend to commit.

**FALLBACK** if curl fails:
Tell user to manually download from:
https://raw.githubusercontent.com/RobinNorberg/oh-my-copilot/main/docs/CLAUDE.md
...
- Git excludes: Added local `.omcp/*` ignore rules to `.git/info/exclude` (keeps `.omcp/skills/` trackable for committed project skills)
