...
- New runs default to `.omg/self-improve/topics/default/`.
- When the user provides a topic or slug, use `.omg/self-improve/topics/{topic_slug}/`.
- Legacy single-track state at `.omg/self-improve/` remains valid only as a compatibility fallback when no explicit topic/slug is supplied and that flat layout already exists.
...
OMC mode lifecycle: `.omg/state/sessions/{sessionId}/self-improve-state.json`
...
| Architecture Review | 6-point plan review | oh-my-copilot:architect | opus |
| Critic Review | Harness rule enforcement | oh-my-copilot:critic | opus |
| Execution | Implement plan + run benchmark | oh-my-copilot:executor | opus |
| Git Operations | Atomic merge/tag/PR | oh-my-copilot:git-master | sonnet |
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `oh-my-copilot:git-master`:
  ```
...
Spawn N `oh-my-copilot:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).
...
**6a. Architecture Review**: Spawn `oh-my-copilot:architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `oh-my-copilot:critic` with the plan + harness rules:
...
For each approved plan, spawn `oh-my-copilot:executor`(model=opus) in parallel.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `oh-my-copilot:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
