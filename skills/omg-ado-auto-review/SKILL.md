...
description: >
  ...
  DO NOT USE FOR: Manual interactive PR review (use omg-ado-review), creating PRs (use git-master agent), sprint planning (use omg-ado-sprint), or non-ADO repositories.
---
...
If config is NOT_FOUND or missing the `ado` key: tell the user to run `/oh-my-copilot:omg-ado-setup` first and stop.
...
**If invoked with a specific PR ID argument** (e.g. `omg-ado-auto-review 42`): skip discovery and process that PR directly, regardless of reviewer assignment.
...
| Error | Action |
|-------|--------|
| `.omg/config.json` missing | Tell user to run `/oh-my-copilot:omg-ado-setup` first |
