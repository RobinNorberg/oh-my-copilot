---
name: ultragoal
description: Durable multi-goal workflow that persists plan/ledger artifacts under .omcp/ultragoal and prints Claude /goal handoff text for the active session
...
- The user wants the assistant to literally invoke `/goal` itself from the shell — that is not possible; `omcp ultragoal` only writes artifacts and prints handoff text
...
- The shell cannot invoke or mutate Claude Code `/goal` state. `omcp ultragoal` only persists durable artifacts and prints instructions that the active Claude agent reads and acts on in-session.
