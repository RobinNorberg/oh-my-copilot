...
Copilot CLI does NOT automatically apply model parameters from agent definitions. When you invoke the `Task` tool (or `Agent` tool), you must manually specify the `model` parameter every time, even though each agent has a default model defined in its configuration.
...
```typescript
import { enforceModel } from 'oh-my-copilot';
...
```typescript
import { getModelForAgent } from 'oh-my-copilot';
...
```typescript
import { isAgentCall } from 'oh-my-copilot';
...
```typescript
import { processHook } from 'oh-my-copilot';
