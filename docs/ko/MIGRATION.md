...
이 가이드는 oh-my-copilot의 모든 마이그레이션 경로를 다룹니다. 아래에서 현재 사용 중인 버전을 찾아주세요.
...
8개의 폐기된 스킬이 제거되었습니다. 통합된 `/cancel` 및 `/omg-setup` 명령어가 이를 대체합니다.
...
```bash
/oh-my-copilot:cancel-ralph      # ralph만 취소
/oh-my-copilot:omg-default       # 로컬 프로젝트 설정
/oh-my-copilot:planner "task"    # 플래닝 시작
```

**v3.5.3 이후:**

```bash
/oh-my-copilot:cancel            # 활성 모드를 자동 감지하여 취소
/oh-my-copilot:omg-setup --local # 로컬 프로젝트 설정
/oh-my-copilot:plan "task"       # 플래닝 시작 (인터뷰 모드 포함)
```
...
```bash
/oh-my-copilot:plan --consensus "task"  # critic 리뷰가 포함된 반복적 플래닝
/oh-my-copilot:ralplan "task"           # plan --consensus의 별칭
```
...
3. copilot-instructions.md 설정을 업데이트하려면 **`/omg-setup`을 재실행**하세요
...
- **프로젝트/브랜드명**: `oh-my-copilot` (GitHub 저장소, 플러그인명, 명령어)
- **npm 패키지명**: `oh-my-copilot` (변경 없음)

> **왜 이름이 다른가요?** npm 패키지명 `oh-my-copilot`는 기존 설치와의 하위 호환성을 위해 유지되었습니다. 프로젝트, GitHub 저장소, 플러그인 및 모든 명령어는 `oh-my-copilot`를 사용합니다.
...
/oh-my-copilot:ralph "implement user authentication"       # 지속성 모드
/oh-my-copilot:ultrawork "refactor the API layer"          # 최대 병렬 처리
/oh-my-copilot:planner "plan the new dashboard"            # 플래닝 인터뷰
/oh-my-copilot:deepsearch "find database schema files"     # 딥 서치
/oh-my-copilot:git-master "commit these changes"           # Git 전문가
/oh-my-copilot:deepinit ./src                              # 코드베이스 인덱싱
/oh-my-copilot:analyze "why is this test failing?"         # 심층 분석
```
...
- **이전**: `~/.copilot/skills/sisyphus-learned/`
- **이후**: `~/.copilot/skills/omg-learned/`
...
| -------------------------------------- | ---------------------------------------------------------- | ------------------- |
| `/oh-my-copilot:ralph "task"`       | "don't stop until done"이라고 말하거나 `ralph` 키워드 사용 | ✅ 예 (양쪽 모두)   |
| `/oh-my-copilot:ultrawork "task"`   | "fast" 또는 "parallel"이라고 말하거나 `ulw` 키워드 사용    | ✅ 예 (양쪽 모두)   |
| `/oh-my-copilot:ultrawork-ralph`    | "ralph ulw:" 접두사 사용                                   | ✅ 예 (키워드 조합) |
| `/oh-my-copilot:planner "task"`     | "plan this"라고 말하거나 `plan` 키워드 사용                | ✅ 예 (양쪽 모두)   |
| `/oh-my-copilot:plan "description"` | 자연스럽게 플래닝 시작                                     | ✅ 예               |
| `/oh-my-copilot:review [path]`      | 기존과 동일하게 호출                                       | ✅ 예 (변경 없음)   |
| `/oh-my-copilot:deepsearch "query"` | "find" 또는 "search"라고 말하기                            | ✅ 예 (자동 감지)   |
| `/oh-my-copilot:analyze "target"`   | "analyze"라고 말하기 — debugger/architect 에이전트로 라우팅 | ✅ 예 (키워드 라우트) |
| `/oh-my-copilot:deepinit [path]`    | 기존과 동일하게 호출                                       | ✅ 예 (변경 없음)   |
| `/oh-my-copilot:git-master`         | "git", "commit", "atomic commit"이라고 말하기              | ✅ 예 (자동 감지)   |
| `/oh-my-copilot:frontend-ui-ux`     | "UI", "styling", "component", "design"이라고 말하기        | ✅ 예 (자동 감지)   |
| `/oh-my-copilot:note "content"`     | "remember this" 또는 "save this"라고 말하기                | ✅ 예 (자동 감지)   |
| `/oh-my-copilot:cancel-ralph`       | "stop", "cancel" 또는 "abort"라고 말하기                   | ✅ 예 (자동 감지)   |
| `/oh-my-copilot:omg-doctor`         | 기존과 동일하게 호출                                       | ✅ 예 (변경 없음)   |
...
더 이상 `/oh-my-copilot:cancel-ralph`이 필요 없습니다 - 그냥 "cancel"이라고 말하세요!
...
- 최신 copilot-instructions.md 다운로드
- 32개 에이전트 설정
- 자동 동작 감지 활성화
- 연속 실행 강제 활성화
- 스킬 조합 설정
...
2. **디렉토리 존재 확인**:
   ...
   ```bash
   ls -la .omg/  # 프로젝트 디렉토리에서
   ls -la ~/.omg/  # 글로벌 디렉토리
   ```

3. **간단한 명령어 테스트**:
   Copilot CLI에서 `/oh-my-copilot:omg-help`를 실행하여 플러그인이 올바르게 로드되었는지 확인하세요.
...
```
이전: "OK, ultrawork를 사용하려면 /oh-my-copilot:ultrawork를 써야지..."
이후: "빨리 해줘!"
      ↓
      Copilot: "ultrawork 모드를 활성화합니다..."
```
...
#### 3. 학습된 스킬 (`/oh-my-copilot:learner`)
...
- **프로젝트 레벨**: `.omg/skills/` (버전 관리됨)
- **사용자 레벨**: `~/.copilot/skills/omg-learned/` (이식 가능)
...
설치하려면 `/oh-my-copilot:hud setup`을 실행하세요. 프리셋: minimal, focused, full.
...
또는 `/oh-my-copilot:note`를 사용하여 발견한 것을 수동으로 저장할 수 있습니다:
...
사용자: "사용자 대시보드 구현해줘"
    ↓
Copilot: "완료를 보장하기 위해 ralph-loop을 활성화합니다"
    ↓
...
- 충돌을 방지하는 파일 소유권 조정
- 지능적 조정을 통한 병렬 실행
- 상태 파일: `.omg/state/ultrapilot-state.json`, `.omg/state/ultrapilot-ownership.json`
...
```bash
/oh-my-copilot: "refactor the authentication system"
```
...
- `/oh-my-copilot:cancel-ralph` (폐기됨)
- `/oh-my-copilot:cancel-ultraqa` (폐기됨)
- `/oh-my-copilot:cancel-ultrawork` (폐기됨)
- `/oh-my-copilot:cancel-` (폐기됨)
- `/oh-my-copilot:cancel-autopilot` (폐기됨)

대신 `/oh-my-copilot:cancel`을 사용하세요.
...
- 로컬: `.omg/state/{name}.json`
- 글로벌: `~/.omg/state/{name}.json`
...
사용자는 `/oh-my-copilot:omg-setup`을 통해 기본 모드 선호도를 설정할 수 있습니다.
...
`~/.copilot/.omg-config.json`에서 선호하는 실행 모드를 설정하세요:
...
4. **상태 디렉토리 확인**:
   ```bash
   ls -la .omg/state/  # ultrapilot 실행 후 ultrapilot-state.json이 보여야 합니다
   ```
...
"implement the todo list feature quickly"
    ↓
Copilot: "최대 병렬 처리를 위해 ultrawork를 활성화합니다"
```
...
oh-my-copilot를 현재 프로젝트에만 적용합니다:

```
/oh-my-copilot:omg-default
```

생성 파일: `./.copilot/copilot-instructions.md`

### 글로벌 설정

모든 Copilot CLI 세션에 적용합니다:
...
A: 계속 사용하세요! `/oh-my-copilot:ralph`, `/oh-my-copilot:ultrawork`, `/oh-my-copilot:plan`이 작동합니다. 참고: `/oh-my-copilot:planner`는 이제 `/oh-my-copilot:plan`으로 리다이렉트됩니다.

**Q: Claude가 무엇을 하고 있는지 어떻게 알 수 있나요?**
A: Claude가 주요 동작을 안내합니다: "ralph-loop을 활성화합니다..." 또는 실시간 상태를 위해 `/oh-my-copilot:hud`를 설정하세요.
...
- **이슈 진단**: `/oh-my-copilot:omg-doctor` 실행
...
2. **전체 기능 활용을 위해**: [docs/copilot-instructions.md](../copilot-instructions.md)를 읽고 오케스트레이션을 이해하세요
3. **고급 사용을 위해**: [docs/ARCHITECTURE.md](../ARCHITECTURE.md)에서 심층 분석을 확인하세요
4. **팀 온보딩을 위해**: 이 가이드를 팀원들과 공유하세요

oh-my-copilot에 오신 것을 환영합니다!
