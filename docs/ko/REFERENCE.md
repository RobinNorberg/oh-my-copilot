...
oh-my-copilot의 전체 레퍼런스입니다. 빠른 시작은 메인 [README.md](../../README.md)를 참조하세요.
...
**Copilot CLI 플러그인 방식만 지원됩니다.** 다른 설치 방법 (npm, bun, curl)은 폐기되었으며 올바르게 작동하지 않을 수 있습니다.
...
이 방법은 Copilot CLI의 플러그인 시스템과 직접 통합되며 Node.js 훅을 사용합니다.

> **참고**: npm/bun 글로벌 직접 설치는 **지원되지 않습니다**. 플러그인 시스템이 모든 설치 및 훅 설정을 자동으로 처리합니다.

### 요구 사항

- [Copilot CLI](https://docs.github.com/copilot-cli) 설치됨
- 다음 중 하나:
  - **Copilot Max/Pro 구독** (개인 사용자에게 권장)
...
- 현재 프로젝트에 `./.copilot/copilot-instructions.md`를 생성합니다
- 설정은 이 프로젝트에만 적용됩니다
- 다른 프로젝트나 글로벌 설정에는 영향을 주지 않습니다
- **안전**: 글로벌 copilot-instructions.md를 보존합니다

### 글로벌 설정

모든 Copilot CLI 세션에 omc를 설정합니다:

```
/oh-my-copilot:omg-setup
```

- 글로벌로 `~/.copilot/copilot-instructions.md`를 생성합니다
- 설정은 모든 프로젝트에 적용됩니다
- **주의**: 기존 `~/.copilot/copilot-instructions.md`를 완전히 덮어씁니다
...
```
./.copilot/copilot-instructions.md  (프로젝트)   →  덮어씀  →  ~/.copilot/copilot-instructions.md  (글로벌)
```
...
- **다른 머신**: Copilot CLI를 사용하는 각 머신에서 실행
- **새 프로젝트**: omc가 필요한 각 프로젝트에서 `/oh-my-copilot:omg-setup --local` 실행

> **참고**: 플러그인 업데이트 후 (`npm update`, `git pull` 또는 Copilot CLI의 플러그인 업데이트를 통해), 최신 copilot-instructions.md 변경사항을 적용하려면 반드시 `/oh-my-copilot:omg-setup`을 재실행**해야 합니다**.

### 에이전트 커스터마이징

`~/.copilot/agents/`의 에이전트 파일을 편집하여 동작을 커스터마이징할 수 있습니다:
...
프로젝트별 지침을 위해 프로젝트에 `.copilot/copilot-instructions.md`를 생성하세요:
...
`omp config-stop-callback`으로 Telegram/Discord stop callback 태그를 설정합니다.
...
Task 도구를 통해 호출할 때는 항상 `oh-my-copilot:` 접두사를 사용하세요.
...
| `autopilot`   | 아이디어에서 작동하는 코드까지 완전 자율 실행 | `/oh-my-copilot:autopilot`  |
| `ultrawork`   | 병렬 에이전트를 통한 최대 성능                | `/oh-my-copilot:ultrawork`  |
| `ultrapilot`  | 3-5배 빠른 병렬 autopilot                     | `/oh-my-copilot:ultrapilot` |
| `swarm`       | 작업 클레이밍을 가진 N개의 조정된 에이전트    | `/oh-my-copilot:swarm`      |
| `pipeline`    | 순차적 에이전트 체이닝                        | `/oh-my-copilot:pipeline`   |
| ``            | 토큰 효율적 병렬 실행                         | `/oh-my-copilot:`           |
| `ralph`       | 완료될 때까지 자기 참조적 개발                | `/oh-my-copilot:ralph`      |
| `ralph-init`  | 구조화된 작업 추적을 위한 PRD 초기화          | `/oh-my-copilot:ralph-init` |
| `ultraqa`     | 자율 QA 사이클링 워크플로우                   | `/oh-my-copilot:ultraqa`    |
| `plan`        | 플래닝 세션 시작                              | `/oh-my-copilot:plan`       |
| `ralplan`     | 반복적 플래닝 (planner+architect+critic)      | `/oh-my-copilot:ralplan`    |
| `review`      | critic을 통한 작업 플랜 리뷰                  | `/oh-my-copilot:review`     |

### 향상 스킬

| 스킬              | 설명                                      | 수동 명령어                         |
| ----------------- | ----------------------------------------- | ----------------------------------- |
| `deepinit`        | 계층적 AGENTS.md 코드베이스 문서화        | `/oh-my-copilot:deepinit`        |
| `deepsearch`      | 다중 전략 코드베이스 검색                 | `/oh-my-copilot:deepsearch`      |
| `sciomc`          | 병렬 scientist 오케스트레이션             | `/oh-my-copilot:sciomc`          |
| `frontend-ui-ux`  | 디자이너 출신 개발자의 UI/UX 전문성       | (자동 활성화)                       |
| `git-master`      | 원자적 커밋 및 히스토리를 위한 Git 전문가 | (자동 활성화)                       |
| `learner`         | 세션에서 재사용 가능한 스킬 추출          | `/oh-my-copilot:learner`         |

### 유틸리티 스킬

| 스킬                      | 설명                                          | 수동 명령어                                 |
| ------------------------- | --------------------------------------------- | ------------------------------------------- |
| `note`                    | 컨텍스트 압축에 강한 노트패드에 메모 저장     | `/oh-my-copilot:note`                    |
| `cancel`                  | 모든 모드에 대한 통합 취소                    | `/oh-my-copilot:cancel`                  |
| `omg-setup`               | 최초 설정 마법사                              | `/oh-my-copilot:omg-setup`               |
| `omg-doctor`              | 설치 문제 진단 및 수정                        | `/oh-my-copilot:omg-doctor`              |
| `omg-help`                | OMP 사용 가이드 표시                          | `/oh-my-copilot:omg-help`                |
| `hud`                     | HUD 상태 표시줄 설정                          | `/oh-my-copilot:hud`                     |
| `release`                 | 자동 릴리스 워크플로우                        | `/oh-my-copilot:release`                 |
| `mcp-setup`               | MCP 서버 설정                                 | `/oh-my-copilot:mcp-setup`               |
| `writer-memory`           | 작성자를 위한 에이전틱 메모리 시스템          | `/oh-my-copilot:writer-memory`           |
| `project-session-manager` | 격리된 개발 환경 관리 (git worktrees + tmux)  | `/oh-my-copilot:project-session-manager` |
| `skill`                   | 로컬 스킬 관리 (목록, 추가, 제거, 검색, 편집) | `/oh-my-copilot:skill`                   |

---

## 슬래시 명령어

모든 스킬은 `/oh-my-copilot:` 접두사가 붙은 슬래시 명령어로 사용할 수 있습니다.

| 명령어                                       | 설명                                      |
| -------------------------------------------- | ----------------------------------------- |
| `/oh-my-copilot:orchestrate <task>`       | 멀티 에이전트 오케스트레이션 모드 활성화  |
| `/oh-my-copilot:autopilot <task>`         | 완전 자율 실행                            |
| `/oh-my-copilot:ultrawork <task>`         | 병렬 에이전트를 통한 최대 성능 모드       |
| `/oh-my-copilot:ultrapilot <task>`        | 병렬 autopilot (3-5배 빠름)               |
| `/oh-my-copilot:swarm <N>:<agent> <task>` | 조정된 에이전트 swarm                     |
| `/oh-my-copilot:pipeline <stages>`        | 순차적 에이전트 체이닝                    |
| `/oh-my-copilot: <task>`                  | 토큰 효율적 병렬 실행                     |
| `/oh-my-copilot:ralph-init <task>`        | 구조화된 작업 추적을 위한 PRD 초기화      |
| `/oh-my-copilot:ralph <task>`             | 작업 완료까지 자기 참조 루프              |
| `/oh-my-copilot:ultraqa <goal>`           | 자율 QA 사이클링 워크플로우               |
| `/oh-my-copilot:plan <description>`       | 플래닝 세션 시작                          |
| `/oh-my-copilot:ralplan <description>`    | 합의를 통한 반복적 플래닝                 |
| `/oh-my-copilot:review [plan-path]`       | critic을 통한 플랜 리뷰                   |
| `/oh-my-copilot:deepsearch <query>`       | 다중 전략 코드베이스 검색                 |
| `/oh-my-copilot:deepinit [path]`          | 계층적 AGENTS.md 파일로 코드베이스 인덱싱 |
| `/oh-my-copilot:sciomc <topic>`           | 병렬 리서치 오케스트레이션                |
| `/oh-my-copilot:learner`                  | 세션에서 재사용 가능한 스킬 추출          |
| `/oh-my-copilot:note <content>`           | notepad.md에 메모 저장                    |
| `/oh-my-copilot:cancel`                   | 통합 취소                                 |
| `/oh-my-copilot:omg-setup`                | 최초 설정 마법사                          |
| `/oh-my-copilot:omg-doctor`               | 설치 문제 진단 및 수정                    |
| `/oh-my-copilot:omg-help`                 | OMP 사용 가이드 표시                      |
| `/oh-my-copilot:hud`                      | HUD 상태 표시줄 설정                      |
| `/oh-my-copilot:release`                  | 자동 릴리스 워크플로우                    |
| `/oh-my-copilot:mcp-setup`                | MCP 서버 설정                             |

---

## 훅 시스템

oh-my-copilot에는 Copilot CLI의 동작을 향상시키는 31개의 라이프사이클 훅이 포함되어 있습니다.
...
| `omg-orchestrator`   | 오케스트레이터 동작 및 위임 강제           |
...
| `OMC_MCP_OUTPUT_REDIRECT_DIR`    | 경로 (기본값: `.omg/outputs`)        | 정책이 `redirect_output`일 때 리다이렉트할 디렉토리 |
...
.omg/outputs/response.md      -> /home/user/project/.omg/outputs/response.md
```
...
| 출력 파일이 예상 위치에 없음                        | `redirect_output` 정책 활성 상태              | `OMC_MCP_OUTPUT_REDIRECT_DIR` 확인 (기본값: `.omg/outputs`)                                     |
...
oh-my-copilot에는 에이전트 성능, 토큰 사용량 및 병렬 워크플로우 디버깅을 위한 종합 모니터링이 포함되어 있습니다.
...
| **Token Analytics**     | 비용 추적, 사용량 보고서, 예산 경고     | HUD(`analytics` 프리셋), `omp cost` |
| **Session Replay**      | 세션 후 분석을 위한 이벤트 타임라인     | `.omg/state/agent-replay-*.jsonl` |
...
```bash
omp                # 기본 분석 대시보드
omp cost daily     # 일일 비용 보고서
omp cost weekly    # 주간 비용 보고서
omp backfill       # 과거 트랜스크립트 데이터 가져오기
...
- **[MarginLab.ai](https://marginlab.ai)** - Copilot 모델 성능 저하를 감지하기 위한 통계적 유의성 테스트가 포함된 SWE-Bench-Pro 성능 추적
...
`~/.copilot/settings.json`에서 HUD 요소를 설정합니다:
...
| 문제                     | 해결 방법                                                                            |
| ------------------------ | ------------------------------------------------------------------------------------ |
| 명령어를 찾을 수 없음    | `/oh-my-copilot:omg-setup` 재실행                                                 |
| 훅이 실행되지 않음       | 훅 권한 확인: `chmod +x ~/.copilot/hooks/**/*.sh`                                     |
| 에이전트가 위임하지 않음 | copilot-instructions.md가 로드되었는지 확인: `./.copilot/copilot-instructions.md` 또는 `~/.copilot/copilot-instructions.md` 확인 |
| LSP 도구가 작동하지 않음 | 언어 서버 설치: `npm install -g typescript-language-server`                          |
| 토큰 제한 오류           | 토큰 효율적 실행을 위해 `/oh-my-copilot:` 사용                                    |

### 자동 업데이트

oh-my-copilot에는 백그라운드에서 업데이트를 확인하는 무음 자동 업데이트 시스템이 포함되어 있습니다.
...
수동으로 업데이트하려면 플러그인 설치 명령어를 재실행하거나 Copilot CLI의 내장 업데이트 메커니즘을 사용하세요.
