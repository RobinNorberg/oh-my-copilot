...
> oh-my-copilot가 멀티 에이전트 워크플로우를 오케스트레이션하는 방법.

## 개요

oh-my-copilot는 스킬 기반 라우팅 시스템을 통해 Copilot CLI가 전문 에이전트를 오케스트레이션할 수 있도록 합니다.
...
**로컬 프로젝트 상태:**
- `.omg/state/{name}.json` - 세션 상태 (ultrapilot, swarm, pipeline)
...
oh-my-copilot는 `src/hooks/`에 라이프사이클 이벤트를 위한 31개의 훅을 포함합니다:
...
- **스킬 레퍼런스**: 프로젝트의 copilot-instructions.md 참조