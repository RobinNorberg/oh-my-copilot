...
The Compatibility Layer enables oh-my-copilot to discover, register, and use external plugins, MCP servers, and tools. It provides a unified interface for managing external tools while maintaining security through an integrated permission system.
...
Plugins are discovered from `~/.copilot/plugins/` and `~/.copilot/installed-plugins/`:
...
**Agents** are discovered from `.md` files in the agents directory with similar frontmatter structure. Supported runtime fields depend on Copilot CLI; OMC's bundled agent files currently rely on `name`, `description`, `model`, optional tool restrictions, and prompt body guidance. They do not currently ship an `effort:` frontmatter override, so effort inherits from the parent Copilot CLI session unless a custom agent explicitly adds one.

## MCP Server Discovery

### Copilot Desktop Config

Located at `~/.copilot/claude_desktop_config.json`:
...
Located at `~/.copilot/settings.json`:
...
const result = await initializeCompatibility({
  pluginPaths: ['~/.copilot/plugins'],
  mcpConfigPath: '~/.copilot/claude_desktop_config.json',
...
**Checklist:**
- Plugins are in `~/.copilot/plugins/` or `~/.copilot/installed-plugins/`
- Each plugin has a `plugin.json` in the root or `.copilot-plugin/` subdirectory
- Plugin name doesn't conflict with reserved names (e.g., 'oh-my-copilot')
