...
When `/oh-my-copilot:cancel` is invoked, it clears mode state files for
...
2. Before the skill completes, invoke `/oh-my-copilot:cancel`
...
```bash
rm .omg/state/sessions/<sessionId>/skill-active-state.json
```
