...
This document describes known issues with CJK (Chinese, Japanese, Korean) IME input in Copilot CLI CLI and provides workarounds for affected users.
...
Copilot CLI CLI uses React Ink for terminal UI rendering. Due to limitations in how terminal raw mode handles IME (Input Method Editor) composition events, CJK users experience various input issues ranging from invisible characters to mispositioned composition text.
...
Compose your text in an external editor that handles IME correctly, then paste into Copilot CLI.

1. Open any text editor (VS Code, Notes, TextEdit, Notepad)
2. Type your CJK text there
3. Copy (`Cmd+C` / `Ctrl+C`)
4. Paste into Copilot CLI (`Cmd+V` / `Ctrl+V`)
...
Create a script that reads from clipboard and sends to Copilot CLI:

```bash
# macOS
pbpaste | copilot --stdin

# Linux (requires xclip)
xclip -selection clipboard -o | copilot --stdin
```

### Workaround 4: Use IDE Integration

Use Copilot CLI through IDE integrations (VS Code extension) which may handle IME better than raw terminal.
...
- [#18291](https://github.com/anthropics/copilot-cli/issues/18291) - Korean IME composition: jamo not displayed until syllable completion
- [#16322](https://github.com/anthropics/copilot-cli/issues/16322) - [CRITICAL] Korean IME: Composing characters display at wrong position
- [#15705](https://github.com/anthropics/copilot-cli/issues/15705) - Korean input characters disappear on iOS mobile SSH
- [#1547](https://github.com/anthropics/copilot-cli/issues/1547) - IME input causes performance issues
- [#3045](https://github.com/anthropics/copilot-cli/issues/3045) - Investigation: Fixing IME Issues by Patching React Ink
