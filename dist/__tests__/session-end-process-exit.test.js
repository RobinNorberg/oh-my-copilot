...
function expectPromptExit(result, ceilingMs = COMMAND_CEILING_MS) {
    expect(result.timedOut).toBe(false);
    expect(result.signal).toBeNull();
    expect(result.code).toBe(0);
    expect(result.elapsedMs).toBeLessThanOrEqual(ceilingMs);
}
function validSessionEndInput(cwd, sessionId) {
...
function configureDeferredAdapters(cwd) {
...
describe('SessionEnd run.cjs process exit regressions (#3477)', () => {
    ...
    it.skipIf(!HAS_GENERATED_DIST)('uses the generated dist closure: the shipped worker imports and can execute', async () => {
        ...
        const { prepareCoreManifest, sealCoreManifest, sealWikiManifest, mutateSessionEndJob, readSessionEndJob } = await import(pathToFileURL(distManifest).href);
        const { processSessionEndWorker } = await import(pathToFileURL(distWorker).href);
        const cwd = createProject();
    ...
    it.skipIf(!HAS_GENERATED_DIST)('keeps configured callbacks, proxies, and custom CA out of the foreground process', async () => {
        ...
        const result = await runUntilClose(join(REPO_ROOT, 'scripts', 'session-end.mjs'), cwd, validSessionEndInput(cwd, 'configured-network-routing'), COMMAND_CEILING_MS, {
...
//# sourceMappingURL=session-end-process-exit.test.js.map