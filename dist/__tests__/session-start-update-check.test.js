...
describe('session-start.mjs update check', () => {
    ...
    beforeEach(() => {
        ...
        pluginRoot = join(fakeHome, '.claude', 'plugins', 'cache', 'omc', 'oh-my-copilot', '1.0.0');
        ...
        writeFileSync(join(marketplaceDir, 'marketplace.json'), JSON.stringify({ plugins: [{ name: 'oh-my-copilot', version: '9.9.9' }] }));
        // Fresh Claude Code entry so the registry fetch is skipped.
        cachePath = join(fakeHome, '.claude', '.omg', 'update-check.json');
        mkdirSync(join(fakeHome, '.claude', '.omg'), { recursive: true });
        writeFileSync(cachePath, JSON.stringify({ claudeCodeLatestVersion: '2.1.240', claudeCodeCheckedAt: Date.now() }));
    });
    afterEach(() => {
        rmSync(tempDir, { recursive: true, force: true });
    });
    function runRefreshChild() {
...
//# sourceMappingURL=session-start-update-check.test.js.map