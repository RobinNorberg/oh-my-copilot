...
const CONCURRENT_RESTORE_HELPERS = [
...
].filter(([label]) => label !== 'dist' || process.env.OMC_PRECOMPACT_DIST_INTERLEAVINGS === '1');
...
function writeCheckpoint(project, createdAt, extra = {}) {
    const dir = join(project, '.omg', 'state', 'checkpoints');
...
function runHook(payload, project, home, extraEnv = {}) {
...
function writeAncestorRedirectPreload(tempDir) {
...
function runHookWithPlugin(payload, project, home) {
    const stdout = execFileSync(NODE, [SCRIPT_PATH], {
        input: JSON.stringify(payload),
        encoding: 'utf-8',
        env: {
...
function runTemplateHook(payload, project, home) {
...
describe('session-start.mjs PreCompact checkpoint restore (issue #3730)', () => {
    ...
    it('restores the newest checkpoint when source=compact', () => {
        ...
        writeCheckpoint(project, new Date().toISOString(), {
            plan_refs: {
                boulder: {
                    active_plan: '/repo/.omg/plans/epic.md',
                    plan_name: 'epic',
    ...
    it('advances the installed SessionStart marker from checkpoint A to newer B', () => {
        ...
        const first = runHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'marker-advance-installed', cwd: project }, project, home);
        expect(parseContext(first.stdout)).toContain(t1);
        const markerPath = join(project, '.omg', 'state', 'checkpoints-restored', 'marker-advance-installed', 'restored.json');
        ...
        const second = runHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'marker-advance-installed', cwd: project }, project, home);
        ...
        const replay = runHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'marker-advance-installed', cwd: project }, project, home);
    ...
    it('does not commit a restore marker when oversized Priority Context evicts the sentinel', () => {
        ...
        const notepad = join(project, '.omg', 'notepad.md');
        writeFileSync(notepad, `## Priority Context\n${'P'.repeat(7000)}\n`, 'utf-8');
        const first = runHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'budgeted-restore-installed', cwd: project }, project, home);
        expect(parseContext(first.stdout)).not.toContain('PRECOMPACT CHECKPOINT RESTORED');
        expect(existsSync(join(project, '.omg', 'state', 'checkpoints-restored', 'budgeted-restore-installed', 'restored.json'))).toBe(false);
        rmSync(notepad, { force: true });
        const retry = runHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'budgeted-restore-installed', cwd: project }, project, home);
        expect(parseContext(retry.stdout)).toContain('PRECOMPACT CHECKPOINT RESTORED');
        expect(existsSync(join(project, '.omg', 'state', 'checkpoints-restored', 'budgeted-restore-installed', 'restored.json'))).toBe(true);
    });
    it('does not consume a checkpoint when the aggregate budget truncates the restore closing sentinel', () => {
        ...
        const first = runHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: sessionId, cwd: project }, project, home);
        const context = parseContext(first.stdout);
        expect(context).not.toContain('PRECOMPACT CHECKPOINT RESTORED');
        expect(context).not.toContain('</session-restore>');
        expect(existsSync(join(project, '.omg', 'state', 'checkpoints-restored', sessionId, 'restored.json'))).toBe(false);
        rmSync(notepad, { force: true });
        const retry = runHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: sessionId, cwd: project }, project, home);
    ...
    it('template SessionStart also delivers A then newer B and suppresses B replay', () => {
        ...
        const first = runTemplateHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'marker-advance-template', cwd: project }, project, home);
        expect(parseContext(first.stdout)).toContain(t1);
        const markerPath = join(project, '.omg', 'state', 'checkpoints-restored', 'marker-advance-template', 'restored.json');
        expect(JSON.parse(readFileSync(markerPath, 'utf-8')).checkpoint).toBe(realpathSync(checkpointA));
        const t2 = new Date().toISOString();
        const checkpointB = writeCheckpoint(project, t2, { session_id: 'marker-advance-template' });
        const second = runTemplateHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'marker-advance-template', cwd: project }, project, home);
        expect(parseContext(second.stdout)).toContain(t2);
        expect(JSON.parse(readFileSync(markerPath, 'utf-8')).checkpoint).toBe(realpathSync(checkpointB));
        const replay = runTemplateHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'marker-advance-template', cwd: project }, project, home);
        expect(parseContext(replay.stdout)).not.toContain('PRECOMPACT CHECKPOINT RESTORED');
    });
    it('template SessionStart defers marker commit when Priority Context evicts restore', () => {
        ...
        const notepad = join(project, '.omg', 'notepad.md');
        writeFileSync(notepad, `## Priority Context\n${'P'.repeat(7000)}\n`, 'utf-8');
        const first = runTemplateHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'budgeted-restore-template', cwd: project }, project, home);
        expect(parseContext(first.stdout)).not.toContain('PRECOMPACT CHECKPOINT RESTORED');
        expect(existsSync(join(project, '.omg', 'state', 'checkpoints-restored', 'budgeted-restore-template', 'restored.json'))).toBe(false);
        rmSync(notepad, { force: true });
        const retry = runTemplateHook({ hook_event_name: 'SessionStart', source: 'compact', session_id: 'budgeted-restore-template', cwd: project }, project, home);
        expect(parseContext(retry.stdout)).toContain('PRECOMPACT CHECKPOINT RESTORED');
        expect(existsSync(join(project, '.omg', 'state', 'checkpoints-restored', 'budgeted-restore-template', 'restored.json'))).toBe(true);
    });
    ;
    ;
    it.each([
        ...
        const helper = await import(`${pathToFileURL(helperPath).href}?prepare-replace-${_label}`);
        const root = join(project, '.omg');
        const prepared = helper.preparePreCompactCheckpointRestore(root, sessionId);
        ...
        expect(helper.claimPreCompactCheckpointRestore(root, sessionId, prepared.path, prepared.created_at, prepared.mtime_ms, prepared.checkpoint_sha256)).toBe('contended');
        const refreshed = helper.preparePreCompactCheckpointRestore(root, sessionId);
        expect(refreshed.created_at).toBe(replacementCreatedAt);
    });
    it.each(CONCURRENT_RESTORE_HELPERS)('keeps marker advancement monotonic across delayed %s helper processes', async (_label, helperPath, usesOmcRoot) => {
        if (!SECURE_MARKER_SUPPORTED)
            return;
        const createdAt = new Date().toISOString();
        const checkpointDir = join(project, '.omg', 'state', 'checkpoints');
...
process.stdout.write(JSON.stringify(result));`;
        const delayed = spawn(NODE, ['--import', pathToFileURL(preload).href, '--input-type=module', '-e', code], {
            env: {
                ...process.env,
                OMC_ROOT: usesOmcRoot ? join(project, '.omg') : project,
        ...
        const delayedStatus = await new Promise((resolve) => delayed.on('close', resolve));
        expect(delayedStatus, delayedStderr).toBe(0);
        expect(JSON.parse(delayedStdout)).toBeNull();
        const markerPath = join(project, '.omg', 'state', 'checkpoints-restored', `marker-process-${_label}`, 'restored.json');
    ...
    it.each(CONCURRENT_RESTORE_HELPERS)('delivers only one restore for concurrent duplicate %s claims', async (_label, helperPath, usesOmcRoot) => {
        if (!SECURE_MARKER_SUPPORTED)
            return;
...
process.stdout.write(JSON.stringify(restorePreCompactCheckpoint(process.env.OMC_ROOT, process.env.MARKER_SESSION)));`;
        const inputRoot = usesOmcRoot ? join(project, '.omg') : project;
        const delayed = spawn(NODE, ['--import', pathToFileURL(preload).href, '--input-type=module', '-e', code], {
            env: {
                ...process.env,
                OMC_ROOT: inputRoot,
        ...
        writeFileSync(release, 'release');
        expect(await new Promise((resolve) => delayed.on('close', resolve))).toBe(0);
        expect(JSON.parse(delayedStdout)).toBeNull();
    }, 30_000);
    ;
    it.each([
...
process.stdout.write(JSON.stringify(restorePreCompactCheckpoint(process.env.OMC_ROOT, process.env.MARKER_SESSION)));`;
    ...
    it('writes the replay marker under the session-scoped restore directory', () => {
        ...
        const markerPath = join(project, '.omg', 'state', 'checkpoints-restored', 'session-3730', 'restored.json');
    ...
    it('installed SessionStart helper rejects an in-directory symlink to external JSON', () => {
        ...
        expect(existsSync(join(project, '.omg', 'state', 'checkpoints-restored', 'installed-symlink-session', 'restored.json'))).toBe(false);
    });
    it('installed SessionStart helper rejects a symlinked .omg/state ancestor', () => {
        const omcRoot = join(project, '.omg');
        ...
        expect(parseContext(stdout)).not.toContain('PRECOMPACT CHECKPOINT RESTORED');
        expect(parseContext(stdout)).not.toContain(marker);
        expect(existsSync(join(externalState, 'checkpoints-restored', 'installed-state-symlink-session', 'restored.json'))).toBe(false);
    });
    it('installed SessionStart rejects an ancestor redirect between verification and open', async () => {
        ...
        const statePath = join(project, '.omg', 'state');
        ...
        expect(parseContext(stdout)).not.toContain('PRECOMPACT CHECKPOINT RESTORED');
        expect(parseContext(stdout)).not.toContain(marker);
        expect(existsSync(join(project, '.omg', 'state', 'checkpoints-restored', 'installed-ancestor-redirect-session', 'restored.json'))).toBe(false);
        expect(existsSync(join(externalState, 'checkpoints-restored', 'installed-ancestor-redirect-session', 'restored.json'))).toBe(false);
    });
});
...
describe('precompact-restore helper parity (issue #3730 security)', () => {
    ...
    beforeEach(() => {
        ...
        mkdirSync(join(project, '.omg', 'state', 'checkpoints'), { recursive: true });
        writeFileSync(join(project, '.omg', 'state', 'checkpoints', 'checkpoint-now.json'), JSON.stringify({ created_at: new Date().toISOString(), session_id: 'valid-session-3730', trigger: 'auto', active_modes: {}, todo_summary: { pending: 1, in_progress: 0, completed: 0 }, wisdom_exported: false }), 'utf-8');
    });
    afterEach(() => {
    ...
    it('rejects an in-directory symlink to external JSON without restoring or marking it', async () => {
        ...
        expect(existsSync(join(omcRoot, 'state', 'checkpoints-restored', 'template-symlink-session', 'restored.json'))).toBe(false);
    });
    it('rejects externally hard-linked checkpoint in both helper copies', async () => {
        const checkpointPath = join(project, '.omg', 'state', 'checkpoints', 'checkpoint-now.json');
        ...
        for (const [helper, suffix] of [
            ...
            const result = mod.restorePreCompactCheckpoint(join(project, '.omg'), suffix);
            expect(result).toBeNull();
    ...
    it('does not write replay markers outside .omc through a symlinked marker parent', async () => {
        const markerRoot = join(project, '.omg', 'state', 'checkpoints-restored');
        ...
        for (const [helper, suffix] of [
            ...
            const result = mod.restorePreCompactCheckpoint(join(project, '.omg'), suffix);
            expect(result).toBeNull();
            const repeated = mod.restorePreCompactCheckpoint(join(project, '.omg'), suffix);
    ...
    it('does not read or overwrite an external file through a symlinked marker file', async () => {
        ...
        for (const [helper, suffix] of [
            ...
            writeFileSync(join(project, '.omg', 'state', 'checkpoints', 'checkpoint-now.json'), JSON.stringify({ created_at: new Date().toISOString(), session_id: suffix, trigger: 'auto', active_modes: {}, todo_summary: { pending: 1, in_progress: 0, completed: 0 }, wisdom_exported: false }));
            ...
            writeFileSync(externalMarker, JSON.stringify({ checkpoint: join(project, '.omg', 'state', 'checkpoints', 'checkpoint-now.json') }), 'utf-8');
...
`);
            ...
            try {
                ...
                const result = mod.restorePreCompactCheckpoint(join(project, '.omg'), suffix);
                expect(result).toBeNull();
                expect(mod.restorePreCompactCheckpoint(join(project, '.omg'), suffix)).toBeNull();
                if (process.platform !== 'win32')
                    expect(existsSync(signalPath)).toBe(true);
                expect(existsSync(join(externalMarkerParent, 'restored.json'))).toBe(false);
            }
            finally {
                if (previousPublisherPreload === undefined)
                    delete process.env.OMC_PRECOMPACT_PUBLISHER_IMPORT;
                else
                    process.env.OMC_PRECOMPACT_PUBLISHER_IMPORT = previousPublisherPreload;
    ...
    it('rejects a symlinked .omg/state ancestor in the template helper', async () => {
        const omcRoot = join(project, '.omg');
        const statePath = join(omcRoot, 'state');
        rmSync(statePath, { recursive: true, force: true });
        ...
        expect(result).toBeNull();
        expect(result?.text ?? '').not.toContain(marker);
        expect(existsSync(join(externalState, 'checkpoints-restored', 'template-state-symlink-session', 'restored.json'))).toBe(false);
    });
    it('rejects a template ancestor redirect between verification and open', async () => {
        ...
        try {
            ...
            expect(result).toBeNull();
            expect(result?.text ?? '').not.toContain(marker);
            expect(existsSync(join(omcRoot, 'state', 'checkpoints-restored', 'template-ancestor-redirect-session', 'restored.json'))).toBe(false);
            expect(existsSync(join(externalState, 'checkpoints-restored', 'template-ancestor-redirect-session', 'restored.json'))).toBe(false);
        }
        finally {
            openSpy.mockRestore();
    ...
    it('keeps the template helper bound to the opened file when its pathname is swapped during read', async () => {
        ...
        try {
            ...
            const result = mod.restorePreCompactCheckpoint(join(project, '.omg'), 'template-mutation-session');
    ...
    it('keeps the installed helper bound to the opened file when its pathname is swapped during read', async () => {
        ...
        try {
            ...
            const result = mod.restorePreCompactCheckpoint(join(project, '.omg'), 'installed-mutation-session');
            expect(swapped).toBe(true);
            expect(result).toBeNull();
            expect(result?.text ?? '').not.toContain(marker);
        }
    ...
    it.each([
        ...
        const mod = await import(`${pathToFileURL(helper).href}?cross-session`);
        const root = usesOmcRoot ? join(project, '.omg') : project;
        expect(mod.restorePreCompactCheckpoint(root, 'different-session')).toBeNull();
    });
});
//# sourceMappingURL=session-start-precompact-restore.test.js.map