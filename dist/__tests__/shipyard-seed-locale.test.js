...
function documentLanguageContract() {
    ...
    if (!block)
        throw new Error('missing document language contract');
...
function isScriptAmbiguousChinese(input) {
...
function resolveDocumentLanguage(input) {
    ...
    if (input.explicit !== undefined) {
        if (isScriptAmbiguousChinese(input.explicit))
            return { kind: 'ask', reason: 'script-ambiguous' };
    ...
    if (input.persisted !== undefined) {
        if (isScriptAmbiguousChinese(input.persisted))
            return { kind: 'ask', reason: 'script-ambiguous' };
        const tag = normalizeLanguageTag(input.persisted);
        if (tag)
            return { kind: 'resolved', tag, authority: 'persisted' };
    }
    const evidence = contract.canonicalSources.map((path) => input.sources?.[path]).filter((entry) => entry !== undefined);
    if (evidence.length === 0)
        return { kind: 'ask', reason: 'missing' };
    if (evidence.some((entry) => entry.mixed))
        return { kind: 'ask', reason: 'mixed' };
    if (evidence.some((entry) => entry.confidence !== 'high'))
        return { kind: 'ask', reason: 'low-confidence' };
    if (evidence.some((entry) => isScriptAmbiguousChinese(entry.tag)))
        return { kind: 'ask', reason: 'script-ambiguous' };
...
function renderSeed(seed, tag) {
    const block = DRYDOCK.match(new RegExp(`<!-- shipyard-seed-${seed}:${tag}:start -->` +
        '\\s*```markdown\\s*([\\s\\S]*?)\\s*```\\s*' +
        `<!-- shipyard-seed-${seed}:${tag}:end -->`))?.[1];
    if (!block)
        throw new Error(`missing seed ${seed}:${tag}`);
    return block;
}
function companionForTag(tag) {
    const contract = documentLanguageContract();
    const prefix = Object.keys(contract.seedCompanionPrefixes)
        .sort((left, right) => right.length - left.length)
        .find((candidate) => tag === candidate || tag.startsWith(`${candidate}-`));
    if (!prefix)
        throw new Error(`no seed companion for ${tag}`);
    return contract.seedCompanionPrefixes[prefix];
}
function renderResolvedSeed(seed, tag) {
...
function projectSkillSeed() {
    const block = DRYDOCK.match(/```markdown\n(---\nid: project-release-check[\s\S]*?)\n```/)?.[1];
    if (!block)
        throw new Error('missing project-skill seed');
    return block;
}
describe('shipyard document-language behavior contract', () => {
    ...
    ])('asks once instead of guessing from ambiguous evidence %#', (input, expected) => {
    ...
    it('renders exactly one companion while preserving byte-stable tokens', () => {
        ...
        for (const [tag, output] of Object.entries(rendered)) {
            expect(output).toContain(headings[tag]);
            for (const [otherTag, otherHeading] of Object.entries(headings)) {
                if (otherTag !== tag)
                    expect(output).not.toContain(otherHeading);
            }
            expect(output).toContain('# <Project> — Agent & Human Shipyard');
            expect(output).toContain('/oh-my-copilot:launch');
            expect(output).toContain('plan → execute → review → verify');
            expect(output).toContain('CONTEXT.md');
            expect(renderSeed('b', tag)).toContain(`documentLanguage: ${tag}`);
            expect(renderSeed('b', tag)).toContain('## <term>');
        }
    });
    ...
    it('loads and matches a CRLF non-Latin project skill by stable ASCII id', () => {
        ...
        try {
            const skillsDir = join(projectRoot, '.omg', 'skills');
...
//# sourceMappingURL=shipyard-seed-locale.test.js.map