...
const SUPPORTED_CLI_ALIASES = ["oh-my-copilot", "omg"];
...
let committedSnapshotCache = null;
...
function createIsolatedPackWorkspace(workspacePath, snapshotPath) {
    mkdirSync(workspacePath, { recursive: true });
    mkdirSync(snapshotPath, { recursive: true });
    execFileSync("git", ["checkout-index", "--all", `--prefix=${snapshotPath}/`], {
        cwd: PACKAGE_ROOT,
        stdio: "pipe",
    });
...
function getPackedPackage() {
    ...
    try {
        ...
        const startedWithoutGeneratedBundles = [...GENERATED_RUNTIME_ENTRYPOINTS].every((file) => !existsSync(join(packWorkspaceCache, file)));
...
describe("npm package bin surface regression", () => {
    ...
    it("keeps the committed plugin runtime closure as a byte-identical npm package subset", () => {
        const surface = collectPluginRuntimeClosure(committedSnapshotCache);
        const extractedPackageRoot = join(packDirCache, "package");
        for (const relativePath of surface.requiredPaths) {
            expect(packedPackageFixture.files.has(relativePath), relativePath).toBe(true);
            if (relativePath.startsWith("dist/") || relativePath.startsWith("bridge/"))
                continue;
            expect(sha256(join(extractedPackageRoot, relativePath)), relativePath).toBe(sha256(join(committedSnapshotCache, relativePath)));
        }
    });
    it("typechecks the packed team declaration closure with skipLibCheck disabled", () => {
        ...
        symlinkSync(packedPackageFixture.extractedPackageRoot, join(consumerModules, packageName), process.platform === "win32" ? "junction" : "dir");
        symlinkSync(join(PACKAGE_ROOT, "node_modules"), join(packedPackageFixture.extractedPackageRoot, "node_modules"), process.platform === "win32" ? "junction" : "dir");
        writeFileSync(join(consumerRoot, "package.json"), JSON.stringify({ type: "module" }));
        writeFileSync(join(consumerRoot, "index.ts"), `import { recoverDeadWorkerV2 } from ${JSON.stringify(`./node_modules/${packageName}/dist/team/index.js`)};\nvoid recoverDeadWorkerV2;\n`);
        ...
        expect(() => execFileSync(process.execPath, [join(PACKAGE_ROOT, "node_modules", "typescript", "bin", "tsc"), "-p", join(consumerRoot, "tsconfig.json")], { cwd: consumerRoot, stdio: "pipe" })).not.toThrow();
    });
    it("typechecks the supported team export from a clean tarball install", () => {
        ...
        writeFileSync(join(consumerRoot, "index.ts"), `import { recoverDeadWorkerV2 } from ${JSON.stringify(`${packedPackageFixture.packageJson.name}/team`)};\nvoid recoverDeadWorkerV2;\n`);
        writeFileSync(join(consumerRoot, "tsconfig.json"), JSON.stringify({
            compilerOptions: {
                target: "ES2022",
                module: "NodeNext",
        ...
        expect(() => execFileSync(process.execPath, [join(consumerRoot, "node_modules", "typescript", "bin", "tsc"), "-p", join(consumerRoot, "tsconfig.json")], { cwd: consumerRoot, stdio: "pipe" })).not.toThrow();
    });
...
describe("packed project-memory learner command-harvest regression (#3494)", () => {
    function projectMemoryDistDir() {
        return join(packDirCache, "package", "dist", "hooks", "project-memory");
    }
    async function importPackedLearner() {
    ...
    function createBaseMemory(projectRoot, schemaVersion) {
        return {
    ...
    ])("does not harvest shell text into durable build/test commands: %s", async (_name, command) => {
        getPackedPackage();
        const { learnFromToolOutput, saveProjectMemory, loadProjectMemory, SCHEMA_VERSION, } = await importPackedLearner();
