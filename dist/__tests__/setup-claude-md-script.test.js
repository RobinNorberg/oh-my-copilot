...
const isScriptMissing = !require('node:fs').existsSync(SETUP_SCRIPT) || !require('node:fs').existsSync(CONFIG_DIR_HELPER);
describe.skipIf(isWindows || isScriptMissing)('setup-claude-md.sh (issue #1572)', () => {
...
`);
        ...
        const installedPath = join(fixture.projectRoot, '.copilot', 'CLAUDE.md');
        ...
        const installedSkillPath = join(fixture.projectRoot, '.copilot', 'skills', 'omc-reference', 'SKILL.md');
...
describe.skipIf(isWindows || isScriptMissing)('setup-claude-md.sh stale CLAUDE_PLUGIN_ROOT resolution', () => {
    it('uses docs/CLAUDE.md from the active version in installed_plugins.json, not the stale script location', () => {
        ...
        tempRoots.push(root);
        const cacheBase = join(root, '.copilot', 'plugins', 'cache', 'omc', 'oh-my-copilot');
        ...
        mkdirSync(join(homeRoot, '.copilot', 'plugins'), { recursive: true });
        writeFileSync(join(homeRoot, '.copilot', 'plugins', 'installed_plugins.json'), JSON.stringify({
            'oh-my-copilot@omc': [
                {
        ...
        mkdirSync(projectRoot, { recursive: true });
        mkdirSync(join(homeRoot, '.copilot'), { recursive: true });
        writeFileSync(join(homeRoot, '.copilot', 'settings.json'), JSON.stringify({ plugins: ['oh-my-copilot'] }));
        ...
        const result = spawnSync('bash', [join(oldVersion, 'scripts', 'setup-claude-md.sh'), 'local'], {
            ...
            env: {
                ...process.env,
                HOME: homeRoot,
                CLAUDE_CONFIG_DIR: join(homeRoot, '.copilot'),
            },
            encoding: 'utf-8',
        });
        expect(result.status).toBe(0);
        const installed = readFileSync(join(projectRoot, '.copilot', 'CLAUDE.md'), 'utf-8');
