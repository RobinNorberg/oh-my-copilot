...
describe.skipIf(process.platform === 'win32')('project-session-manager session filtering', () => {
    ...
    it('returns only active review/fix sessions for cleanup scans', () => {
        ...
        writeFileSync(join(psmRoot, 'sessions.json'), JSON.stringify({
            version: 1,
            sessions: {
                'repo:pr-101': {
                    id: 'repo:pr-101',
        ...
        const reviews = runShell(`source "${CONFIG_LIB}"; source "${SESSION_LIB}"; psm_get_review_sessions`, home);
        const fixes = runShell(`source "${CONFIG_LIB}"; source "${SESSION_LIB}"; psm_get_fix_sessions`, home);
        expect(reviews).toBe('repo:pr-101|101|repo');
        expect(fixes).toBe('repo:issue-201|201|repo');
    });
});
//# sourceMappingURL=project-session-manager-session-filter.test.js.map