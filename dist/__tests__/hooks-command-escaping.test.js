...
describe('hooks.json command escaping', () => {
    ...
    it('keeps Windows-style plugin roots with spaces intact when bash expands the command', () => {
        const pluginRoot = '/c/Users/First Last/.copilot/plugins/cache/omc/oh-my-copilot/4.7.10';
        for (const command of getHookCommands()) {
            ...
            expect(argv).not.toContain('Last/.copilot/plugins/cache/omc/oh-my-copilot/4.7.10/scripts/run.cjs');
        }
    });
});
