...
describe('resolveTranscriptPath', () => {
    ...
    it('resolves worktree-encoded transcript path to original project path', () => {
        // Simulate: ~/.copilot/projects/-Users-user-project/<session>.jsonl (real)
        ...
        // ~/.copilot/projects/-Users-user-project--copilot-worktrees-refactor/<session>.jsonl
        const worktreeDir = join(tempDir, 'projects', '-Users-user-project--copilot-worktrees-refactor');
    ...
    it('resolves worktree paths with simple single-word names', () => {
        ...
        writeFileSync(realTranscript, '{}');
        const worktreePath = join(tempDir, 'projects', '-Users-dev-app--copilot-worktrees-feature', 'sess.jsonl');
        const resolved = resolveTranscriptPath(worktreePath);
        expect(resolved).toBe(realTranscript);
    });
    it('returns original path when resolved path also does not exist', () => {
        // Both worktree and original paths don't exist
        const worktreePath = join(tempDir, 'projects', '-missing-project--copilot-worktrees-wt', 'transcript.jsonl');
        const resolved = resolveTranscriptPath(worktreePath);
    ...
    describe('native git worktree fallback', () => {
        ...
        beforeEach(() => {
            ...
            // Simulate ~/.copilot/projects/ with a transcript at the main repo's encoded path
            fakeClaudeDir = join(tempDir, 'fake-copilot');
