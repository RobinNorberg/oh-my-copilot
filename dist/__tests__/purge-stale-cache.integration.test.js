...
import { mkdtempSync, mkdirSync, writeFileSync, symlinkSync, renameSync, rmSync, existsSync, lstatSync, readdirSync, readlinkSync, utimesSync, } from 'fs';
...
describe('purgeStalePluginCacheVersions on a real filesystem', () => {
    it('demotes a stale version to a redirect and keeps every pinned path resolving', () => {
        ...
        for (const v of LEGACY)
            symlinkSync(stale, join(pluginDir, v), 'dir');
        makeStale(stale);
        ...
        for (const v of [STALE, ...LEGACY])
            expect(hookResolves(v)).toBe(true);
...
describe('invariant across every cache shape', () => {
    ...
    const OCCUPANTS = ['missing', 'empty-dir', 'dotfile-only', 'junk-only', 'partial-scripts', 'payload', 'live-symlink', 'dangling-link', 'file'];
    const BACKUPS = ['none', 'payload-dead', 'payload-live', 'empty-dead', 'dotfile-dead'];
    const DEAD_PID = 999997;
    function shape(occupant, backup) {
        const V = join(pluginDir, STALE);
        if (occupant === 'empty-dir')
            mkdirSync(V, { recursive: true });
        else if (occupant === 'dotfile-only') {
            mkdirSync(V, { recursive: true });
            writeFileSync(join(V, '.DS_Store'), 'x');
        }
        else if (occupant === 'junk-only') {
            mkdirSync(V, { recursive: true });
            writeFileSync(join(V, 'desktop.ini'), 'x');
        }
        else if (occupant === 'partial-scripts') {
            mkdirSync(join(V, 'scripts'), { recursive: true });
            writeFileSync(join(V, 'scripts', 'partial.txt'), 'x');
        }
        else if (occupant === 'payload')
            writeVersion(STALE);
        else if (occupant === 'live-symlink')
            symlinkSync(join(pluginDir, ACTIVE), V, 'dir');
        else if (occupant === 'dangling-link')
            symlinkSync(join(pluginDir, 'gone-away'), V, 'dir');
        else if (occupant === 'file')
            writeFileSync(V, 'not a directory');
        let aside = null;
        if (backup !== 'none') {
            ...
            else if (backup.startsWith('empty'))
                mkdirSync(aside, { recursive: true });
            else {
                mkdirSync(aside, { recursive: true });
                writeFileSync(join(aside, '.DS_Store'), 'x');
            }
        }
        for (const p of [V, aside].filter(Boolean)) {
            try {
                makeStale(p);
            }
            catch { /* symlinks and plain files cannot be aged */ }
        }
        return { V, aside };
    }
    /** A backup only counts as intact if it is a root the runner would load. */
    const loadableRoot = (dir) => existsSync(join(dir, 'hooks', 'hooks.json'))
        && existsSync(join(dir, 'scripts', 'run.cjs'))
        && existsSync(join(dir, 'scripts'));
    const intactBackupSurvives = () => readdirSync(pluginDir).filter(n => n.includes('.omc-stale-'))
    ...
    it('never ends with a broken pinned path and nothing to show for it', () => {
        ...
        for (const occupant of OCCUPANTS) {
            for (const backup of BACKUPS) {
                ...
                if (!held) {
                    violations.push(`${occupant} + ${backup}: resolves=${hookResolves(STALE)} ` +
                        `backup=${intactBackupSurvives()} errors=${result?.errors.length ?? '-'} ` +
                        `skipped=${result?.skipped ?? '-'} threw=${threw?.code ?? '-'}`);
                }
            }
        }
...
//# sourceMappingURL=purge-stale-cache.integration.test.js.map