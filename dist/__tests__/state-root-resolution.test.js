 ...
 */
function getCentralizedOmcRoot(projectDir, stateDir) {
...
describe('OMC_STATE_DIR state-root resolution (issue #2532)', () => {
    ...
    it('session-start does NOT restore state when OMC_STATE_DIR is set but state is only in default .omc', () => {
        ...
        writeFileSync(join(defaultStateDir, 'ralph-state.json'), JSON.stringify({
            active: true,
            session_id: sessionId,
    ...
    it('stop hook blocks when active ralph state is in centralized OMC_STATE_DIR path', () => {
        ...
        const output = runHook(STOP_HOOK, { hook_event_name: 'Stop', session_id: sessionId, cwd: fakeProject }, { OMC_STATE_DIR: fakeStateDir });
        expect(output.decision).toBe('block');
        expect(String(output.reason)).toContain('[RALPH LOOP');
    });
    ...
    it('pre-tool-enforcer writes skill-active-state into the centralized OMC_STATE_DIR path', () => {
        ...
        runHook(PRE_TOOL_ENFORCER, {
            ...
            tool_input: { skill: 'oh-my-copilot:skill' },
            session_id: sessionId,
            cwd: fakeProject,
        }, { OMC_STATE_DIR: fakeStateDir });
        const centralizedPath = join(centralizedOmcRoot, 'state', 'sessions', sessionId, 'skill-active-state.json');
        const defaultPath = join(fakeProject, '.omg', 'state', 'sessions', sessionId, 'skill-active-state.json');
