...
describe('npm trusted publishing contract', () => {
    ...
    it('preserves the annotated-tag archive, smoke, registry, and GitHub Release boundary', () => {
        ...
        expect(releaseJob).toContain('node scripts/release-boundary.mjs assert-trigger --tag "$GITHUB_REF_NAME" --sha "$RELEASE_SHA"');
        expect(releaseJob).toContain('node scripts/release-boundary.mjs assert-npm-absent --package oh-my-copilot --version "$VERSION"');
        ...
        expect(releaseJob).toContain('node scripts/release-boundary.mjs prepare-stage --seed-tarball "$SEED_TARBALL" --stage "$STAGE" --git-head "$GITHUB_SHA"');
        expect(releaseJob).toContain('node scripts/release-boundary.mjs assert-archive --tarball "$FINAL_TARBALL" --version "$VERSION" --git-head "$GITHUB_SHA"');
        expect(releaseJob).toContain('node scripts/release-boundary.mjs write-evidence --tarball "$FINAL_TARBALL" --output "$EVIDENCE_JSON"');
        expect(releaseJob).toContain('require(process.argv[1]).gitHead');
        expect(releaseJob).toContain('require(process.argv[1]).sourceSha');
        expect(releaseJob).toContain('node scripts/release-boundary.mjs verify-registry --package oh-my-copilot --version "$VERSION" --tag "$GITHUB_REF_NAME" --sha "$GITHUB_SHA" --evidence "$EVIDENCE_JSON" --tarball "$FINAL_TARBALL" --provenance required --audit "$AUDIT_JSON"');
...
//# sourceMappingURL=trusted-publishing-contract.test.js.map