...
describe('session-start.mjs detached update-cache refresh', () => {
    ...
    async function startHangingRegistry() {
    ...
    async function startFakeRegistry(version) {
     ...
     */
    function runScriptAsync(args, registryBase) {
    ...
    describe('--refresh-update-cache child mode', () => {
        ...
        it('serializes concurrent refreshes and leaves a complete cache document', async () => {
            const registryBase = await startFakeRegistry('9.9.9');
            const statuses = await Promise.all(Array.from({ length: 8 }, () => runScriptAsync(['--refresh-update-cache'], registryBase)));
...
//# sourceMappingURL=session-start-update-refresh.test.js.map