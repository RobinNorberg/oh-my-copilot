...
describe('auto slash live-data security', () => {
    ...
    it('allows safe arguments on an explicitly authored live-data directive', async () => {
        ...
        expect(mockedExecSync).not.toHaveBeenCalled();
        expect(mockedExecFileSync).toHaveBeenCalledWith('git', ['status', '--short'], expect.objectContaining({ shell: false }));
    });
    it.each([
        ['line feed', '\n!git status'],
    ...
    it('blocks $ARGUMENTS interpolation inside an authored script block', async () => {
        ...
        expect(result.success).toBe(true);
        expect(result.replacementText).toContain('error="true"');
        expect(result.replacementText).toContain('blocked: arguments are not supported in live-data script blocks');
        expect(mockedExecSync).not.toHaveBeenCalled();
        expect(mockedExecFileSync).not.toHaveBeenCalled();
    });
...
//# sourceMappingURL=auto-slash-live-data-security.test.js.map