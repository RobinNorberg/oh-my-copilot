...
vi.mock('../../lib/file-lock.js', () => ({
    withFileLock: vi.fn((_lockPath, fn) => {
...
function stubCredentialFile() {
    vi.mocked(fs.existsSync).mockImplementation((p) => typeof p === 'string' && p.endsWith('.credentials.json'));
    vi.mocked(fs.readFileSync).mockImplementation((p) => (typeof p === 'string' && p.endsWith('.credentials.json') ? FAKE_CREDENTIALS : '{}'));
}
/** Answer the next https.request with a minimal 200 usage payload. */
function stubUsage200(httpsRequest) {
    httpsRequest.mockImplementationOnce((_options, callback) => {
...
function stubUsage429(httpsRequest) {
    httpsRequest.mockImplementationOnce((_options, callback) => {
        const req = new EventEmitter();
        req.destroy = vi.fn();
        req.end = () => {
...
describe('GET /api/oauth/usage request headers', () => {
    ...
    it('does not let an anonymous 429 suppress a concurrent versioned request', async () => {
        ...
        vi.mocked(fs.existsSync).mockImplementation((p) => String(p).endsWith('.credentials.json') || cacheFiles.has(String(p)));
        vi.mocked(fs.readFileSync).mockImplementation((p) => {
            const path = String(p);
            return (path.endsWith('.credentials.json') ? FAKE_CREDENTIALS : cacheFiles.get(path) ?? '{}');
        });
...
//# sourceMappingURL=usage-api-user-agent.test.js.map