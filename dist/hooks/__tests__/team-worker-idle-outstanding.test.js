...
import { maybeNotifyLeaderWorkerIdle, maybeNotifyLeaderAllWorkersIdle } from '../team-worker-hook.js';
...
function seed(options, teamDir, stateDir) {
    ...
    writeFileSync(join(teamDir, 'workers', WORKER, 'prev-notify-state.json'), JSON.stringify({ state: options.prevState ?? 'working', updated_at: nowIso }));
    // heartbeat.json (fresh)
    if (options.heartbeat !== false) {
        writeFileSync(join(teamDir, 'workers', WORKER, 'heartbeat.json'), JSON.stringify({ pid: process.pid, last_turn_at: nowIso, turn_count: 1, alive: true }));
        writeFileSync(join(teamDir, 'workers', OTHER_WORKER, 'heartbeat.json'), JSON.stringify({ pid: process.pid, last_turn_at: nowIso, turn_count: 1, alive: true }));
    }
    // mailbox: {worker}.json with an undelivered directed message
    const mailboxDir = join(stateDir, 'team', TEAM, 'mailbox');
    mkdirSync(mailboxDir, { recursive: true });
    const inboundMessage = (delivered) => ({
    ...
    if (options.malformedMailbox) {
        ...
        const messages = [inboundMessage(!!options.deliveredInbound)];
        writeFileSync(join(mailboxDir, `${WORKER}.json`), JSON.stringify({ worker: WORKER, messages }));
    }
    if (options.otherWorkerMailbox) {
        writeFileSync(join(mailboxDir, `${OTHER_WORKER}.json`), JSON.stringify({
            worker: OTHER_WORKER,
...
describe('team-worker-hook idle outstanding metadata (issue #3662)', () => {
    ...
    it('qualifies the all-workers-idle readiness claim when any worker has undelivered directed messages', async () => {
        ...
        const nowIso = new Date().toISOString();
        writeFileSync(join(teamDir, 'workers', OTHER_WORKER, 'status.json'), JSON.stringify({ state: 'idle', updated_at: nowIso }));
        const { tmux, sent } = makeTmux();
...
//# sourceMappingURL=team-worker-idle-outstanding.test.js.map