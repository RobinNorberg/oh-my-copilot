...
describe('ensureStdinSymlink', () => {
    ...
    beforeEach(() => {
        ...
        // Create a fake config directory and set COPILOT_CONFIG_DIR env var
    ...
    it('is a no-op when pluginRoot does not exist', () => {
        expect(() => ensureStdinSymlink(join(tmpdir(), 'nonexistent-plugin-root-xyz'))).not.toThrow();
    });
    ...
    it('uses COPILOT_CONFIG_DIR when set', () => {
        ...
        process.env.COPILOT_CONFIG_DIR = customConfigDir;
        try {
            ensureStdinSymlink(pluginRoot);
...
//# sourceMappingURL=stdin-symlink.test.js.map