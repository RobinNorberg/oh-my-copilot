 ...
 *   (`.omg/state/sessions/{id}/prd.json`) is NOT removed and survives into the
...
function resolveCheckPath(directory, checkPath) {
...
function runGit(args, directory) {
    try {
...
function checkFileExists(check, directory) {
    if (!check.path) {
...
function checkFileContains(check, directory) {
...
function checkGitGrep(check, directory) {
    if (!check.pattern) {
 ...
 */
export function runObservableCheck(check, directory) {
    switch (check.type) {
        case 'fileExists':
...
function isBranchMergedIntoHead(directory, branch) {
 ...
 */
export function detectStalePrd(directory, sessionId, options) {
    ...
    const ralphState = readModeState('ralph', directory, sessionId);
...
// Audit trail
// ============================================================================
function getAuditLogPath(directory, sessionId) {
    if (sessionId) {
...
function appendAuditEntry(directory, entry, sessionId) {
 ...
 */
export function formatStalePrdWarning(detection) {
...
// Reconciliation
// ============================================================================
function buildEvidence(checks, results) {
    return checks
        .map((check, i) => {
 ...
 */
export function reconcileStalePrd(directory, sessionId) {
    ...
    const warning = remaining.length > 0
 ...
 */
export function getSessionEndStalePrdWarning(directory, sessionId) {
    try {
        const detection = detectStalePrd(directory, sessionId);
 ...
 */
export function reconcileStalePrdForStartup(directory, sessionId) {
    try {
...
//# sourceMappingURL=stale-prd.js.map