...
import { checkMergeReadiness, cancelMergeReadiness, createInitialMergeReadinessState, formatMergeReadinessReport, formatMergeReadinessQuestionMessage, readMergeReadinessState, recordMergeReadinessMCQAnswer, recordMergeReadinessAskUserQuestionResult, overrideMergeReadiness, redactMergeReadinessState, setMergeReadinessContent, } from "../index.js";
...
vi.mock("../../../lib/mode-state-io.js", async (importOriginal) => {
    ...
    return {
        ...actual,
        writeModeState: (mode, state, directory, sessionId) => {
            if (persistFail.failWrites)
                return false;
            return actual.writeModeState(mode, state, directory, sessionId);
        },
    };
});
function makeQuestion(id, dimension, correctOptionId = "a") {
    return {
        id,
        dimension,
...
describe("merge-readiness runtime", () => {
    ...
    afterEach(() => {
        if (originalPrincipal === undefined)
            ...
            process.env.OMC_MERGE_READINESS_AUTHENTICATED_PRINCIPAL = originalPrincipal;
        if (originalMaintainers === undefined)
            delete process.env.OMC_MERGE_READINESS_MAINTAINERS;
        else
            process.env.OMC_MERGE_READINESS_MAINTAINERS = originalMaintainers;
        rmSync(tempDir, { recursive: true, force: true });
    });
    it("creates the authoritative audit state without writing a report artifact", () => {
        const state = createInitialMergeReadinessState(tempDir, "/merge-readiness --standard improve docs after review", sessionId);
    ...
    it("refuses to re-start while an active pending attempt exists (audit preservation)", () => {
        ...
        expect(() => createInitialMergeReadinessState(tempDir, "/merge-readiness --quick retry", sessionId)).toThrow(/active merge-readiness attempt is still in progress/);
    ...
    it("redacts prior-attempt answer keys on state_read while the current attempt is pending", () => {
        ...
        const redacted = redactMergeReadinessState(state);
    ...
    it("blocks when minimal evidence is missing (no diff/change signal)", () => {
        ...
        try {
            ...
            setMergeReadinessContent(emptyDir, {
                why: "Why",
                whatChanged: "What",
                tradeoffs: "Tradeoff",
                risksConsidered: "Risk",
    ...
    it("parses source flags exactly and rejects conflicting source modes", () => {
        ...
        const conflicting = createInitialMergeReadinessState(tempDir, "/merge-readiness --from-diff --from-artifacts change", "conflicting-source-session");
    ...
    describe("15-case failed-write durable-non-advance transition matrix", () => {
        ...
        it.each([
            {
                ...
                setup: (sid) => seedPending(sid),
                action: (sid) => setMergeReadinessContent(tempDir, {
            ...
            },
            {
                ...
                setup: (sid) => {
                    ...
                    for (const id of ["q1", "q2", "q3"])
                        recordMergeReadinessMCQAnswer(tempDir, id, "b", sid);
                },
                action: (sid) => setMergeReadinessContent(tempDir, {
            ...
            },
            {
                ...
                action: (sid) => recordMergeReadinessMCQAnswer(tempDir, "q3", "a", sid),
            },
            {
                ...
                action: (sid) => recordMergeReadinessMCQAnswer(tempDir, "q3", "b", sid),
            },
            {
                name: "maintainer override transition",
                setup: (sid) => seedPending(sid),
                action: (sid) => overrideMergeReadiness(tempDir, "Maintainer override.", sid),
            },
            {
            ...
            },
            {
                ...
                setup: (sid) => { createInitialMergeReadinessState(tempDir, "/merge-readiness --quick change", sid); },
                action: (sid) => cancelMergeReadiness(tempDir, sid),
            },
            {
            ...
            },
            {
                ...
                setup: (sid) => seedPending(sid),
                action: (sid) => recordMergeReadinessAskUserQuestionResult(tempDir, { question: "[MERGE READINESS:q1] choose" }, "[a]", sid),
            },
        ...
        it("history append (re-start) does not advance on-disk state on failed write", () => {
            ...
            persistFail.failWrites = true;
            try {
                expect(() => createInitialMergeReadinessState(tempDir, "/merge-readiness --quick retry", sessionId)).toThrow("could not create durable state");
            }
            finally {
                persistFail.failWrites = false;
            }
