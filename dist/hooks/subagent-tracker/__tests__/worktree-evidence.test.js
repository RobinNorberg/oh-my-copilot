...
import { collectWorktreeDirtyEvidence, buildDirtyWorktreeNotice, isAbnormalTermination, MAX_EVIDENCE_ENTRIES, } from "../worktree-evidence.js";
const tempDirs = [];
function makeTempDir(prefix = "omc-wt-evidence-") {
...
describe("collectWorktreeDirtyEvidence", () => {
    ...
    it("identifies linked git worktrees as isolated worktrees", () => {
        const repo = makeTempDir();
        initRepo(repo);
        const worktree = join(repo, ".omg", "team", "demo-team", "worktrees", "worker-1");
    ...
    it("is fail-closed and never throws", () => {
        ...
        const repo = makeTempDir();
        initRepo(repo);
        expect(() => collectWorktreeDirtyEvidence(repo, { gitCommand: "/nonexistent/git" })).not.toThrow();
    });
...
`, { mode: 0o755 });
        ...
        expect(buildDirtyWorktreeNotice(evidence, "agent-p1", "executor")).toContain("2 uncommitted file(s)");
    });
...
`, { mode: 0o755 });
        ...
        expect(buildDirtyWorktreeNotice(evidence, "agent-p2", "executor")).toContain("55 uncommitted file(s)");
    });
});
describe("buildDirtyWorktreeNotice", () => {
    ...
    it("returns null for non-dirty kinds", () => {
        ...
        expect(buildDirtyWorktreeNotice({ ...base, kind: "not_git" }, agentId, agentType)).toBeNull();
        expect(buildDirtyWorktreeNotice({ ...base, kind: "cwd_missing" }, agentId, agentType)).toBeNull();
        expect(buildDirtyWorktreeNotice({ ...base, kind: "git_unavailable", error: "git_unavailable" }, agentId, agentType)).toBeNull();
    });
...
describe("isAbnormalTermination", () => {
    ...
    it("detects structured failure envelopes when success is omitted (issue #3663 B6)", () => {
        ...
        expect(isAbnormalTermination({
            output: "<task-notification>\n<status>failed</status>\n</task-notification>",
        })).toBe(true);
        // Start-of-line API-error phrases.
        expect(isAbnormalTermination({ output: "Agent terminated early due to an API error" })).toBe(true);
        expect(isAbnormalTermination({ output: "API Error: Response stalled mid-stream" })).toBe(true);
    });
    ...
    it("treats normal completion and cancel-like stops as non-abnormal", () => {
        ...
        expect(isAbnormalTermination({ output: "Interrupted by user — cancelling task" })).toBe(false);
        expect(isAbnormalTermination({ output: "" })).toBe(false);
    });
});
//# sourceMappingURL=worktree-evidence.test.js.map