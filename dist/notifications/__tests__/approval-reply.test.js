...
describe("handleApprovalReply", () => {
    it("writes a decision file attributed to the reply channel", () => {
        ...
        const decision = handleApprovalReply({ runsRoot, runId: "run-1", activationId: "act-1" }, "approved", "telegram");
    ...
    it("returns null and writes nothing for non-decision text", () => {
        const runsRoot = makeRunsRoot();
        const decision = handleApprovalReply({ runsRoot, runId: "run-1", activationId: "act-1" }, "looks good but wait", "discord");
        expect(decision).toBeNull();
        expect(existsSync(join(runsRoot, "run-1", "approvals", "decisions", "act-1.json"))).toBe(false);
    });
    it("returns null instead of throwing for a stale approval ref", () => {
        ...
        expect(handleApprovalReply({ runsRoot, runId: "gone-run", activationId: "act-1" }, "approved", "telegram")).toBeNull();
    });
...
//# sourceMappingURL=approval-reply.test.js.map