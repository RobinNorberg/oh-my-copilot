...
import { formatSessionIdle, formatSessionEnd, formatAgentCall, formatNotification, parseTmuxTail, formatAskUserQuestion, formatTeamsAdaptiveCard, } from "../formatter.js";
...
describe("formatTeamsAdaptiveCard", () => {
    ...
    it("should include session-end facts", () => {
        ...
        const factSet = result.attachments[0].content.body.find((b) => b.type === "FactSet");
    ...
    it("should include heading text block with event title", () => {
        const result = JSON.parse(formatTeamsAdaptiveCard(basePayload));
        const heading = result.attachments[0].content.body.find((b) => b.type === "TextBlock" && b.style === "heading");
        expect(heading).toBeDefined();
        expect(heading.text).toBe("Session Ended");
    });
    it("should format session-start event", () => {
        const payload = { ...basePayload, event: "session-start" };
        const result = JSON.parse(formatTeamsAdaptiveCard(payload));
        const heading = result.attachments[0].content.body.find((b) => b.style === "heading");
    ...
    it("should format ask-user-question event with question", () => {
        ...
        const factSet = result.attachments[0].content.body.find((b) => b.type === "FactSet");
        const questionFact = factSet.facts.find((f) => f.title === "Question");
    ...
    it("should include context summary for session-end", () => {
        ...
        const result = JSON.parse(formatTeamsAdaptiveCard(payload));
        const summaryBlock = result.attachments[0].content.body.find((b) => b.type === "TextBlock" && b.text?.includes("Summary"));
    ...
    it("should include project in footer facts", () => {
        const result = JSON.parse(formatTeamsAdaptiveCard(basePayload));
        const factSet = result.attachments[0].content.body.find((b) => b.type === "FactSet");
        const projectFact = factSet.facts.find((f) => f.title === "Project");
