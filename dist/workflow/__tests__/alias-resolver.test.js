...
describe('alias-resolver — Tier-0 routing', () => {
    ...
    afterEach(() => {
        if (origQuiet === undefined)
            ...
            process.env.OMC_QUIET = origQuiet;
        if (origResolver === undefined)
            delete process.env.OMC_ALIAS_RESOLVER_ENABLED;
        else
            process.env.OMC_ALIAS_RESOLVER_ENABLED = origResolver;
        if (origDisable === undefined)
            delete process.env.OMC_DISABLE_ALIAS_RESOLVER;
        else
            process.env.OMC_DISABLE_ALIAS_RESOLVER = origDisable;
    });
    ...
    it.each(['team', 'autopilot', 'autoresearch', 'ai-slop-cleaner', 'visual-verdict', 'self-improve'])('kept skill %s resolves to itself, not an alias target', (name) => {
    ...
    it('adapter seam resolveWorkflowAliasViaRegistry honors injected registry', () => {
        const fake = { alias: 'my-alias', canonical: 'plan', tier0: 'plan', owner: 'test', description: 'x', removalMilestone: 'test', isWorkflowAlias: true };
        const lookup = (k) => (k === 'my-alias' ? fake : undefined);
...
describe('alias-resolver — telemetry and usage receipts', () => {
    ...
    it('registry covers expected aliases and excludes self-canonical entries', () => {
        ...
        for (const e of ALIAS_REGISTRY) {
            ...
            if (isSelfCanonical) {
                expect(TIER0_WORKFLOWS).toContain(e.canonical);
            }
            else {
                expect([
...
//# sourceMappingURL=alias-resolver.test.js.map