...
describe('findPosixShell on Windows', () => {
    it('prefers $SHELL when it names an existing POSIX shell', () => {
        ...
        vi.stubEnv('PATH', '');
        mockExistsSync.mockImplementation((p) => normalize(p) === 'D:/Tools/Git/bin/bash.exe');
    ...
    it('never selects the System32 WSL bash launcher', () => {
        ...
        mockExistsSync.mockImplementation((p) => normalize(p) === 'C:/Windows/System32/bash.exe');
    ...
    it('falls back to a common Git for Windows install location', () => {
        ...
        mockExistsSync.mockImplementation((p) => normalize(p) === 'C:/Program Files/Git/bin/bash.exe');
        expect(normalize(findPosixShell())).toBe('C:/Program Files/Git/bin/bash.exe');
    });
...
//# sourceMappingURL=posix-shell.test.js.map