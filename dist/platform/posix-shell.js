...
function isWslLauncher(candidate) {
    ...
    return (normalized.startsWith(`${systemRootNormalized}/system32/`) ||
        normalized.startsWith(`${systemRootNormalized}/sysnative/`));
}
function isPosixShellName(candidate) {
...
function findBashInInstallLocations() {
    const roots = [
...
export const NO_POSIX_SHELL_MESSAGE = 'No POSIX shell (bash) found on Windows. Install Git for Windows (which provides Git Bash) ' +
 ...
 */
export function resolvePosixCommandInvocation(command) {
    if (process.platform !== 'win32') {
