...
function makeFence(dir, staleGraceMs = 1000) {
...
describe("FileOwnershipFence", () => {
    ...
    it.each(["10garbage", "1garbage", "+10", " 10", "10 ", "1.0", "1e2", "0", "9007199254740992"])("rejects non-canonical owner.epoch text %j without issuing an epoch", async (sidecar) => {
        const dir = makeRunDir();
    ...
    it.each([1.5, Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER + 1])("rejects stale lock epoch %j instead of issuing an unsafe successor", async (epoch) => {
        const dir = makeRunDir();
        craftJsonLock(dir, {
            pid: spawnDeadPid(),
            epoch,
    ...
    it("accepts the largest sidecar that still has a safe successor", async () => {
        const dir = makeRunDir();
        writeFileSync(join(dir, EPOCH_FILE_NAME), String(Number.MAX_SAFE_INTEGER - 1), "utf8");
        await expect(makeFence(dir).acquire()).resolves.toEqual({
            outcome: "acquired",
    ...
    it("throws FenceError('fenced_out') for a wrong epoch claim", async () => {
        ...
        expect(caught.code).toBe("fenced_out");
...
//# sourceMappingURL=fence.test.js.map