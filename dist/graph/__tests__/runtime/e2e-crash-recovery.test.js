...
function distIsStale() {
    if (!existsSync(CLI_ENTRY) || !builtCliLooksCurrent())
        return true;
    ...
    if (distReady === null) {
        distReady = (async () => {
            ...
            if (built.status !== 0 || !existsSync(CLI_ENTRY)) {
                throw new Error(`dist build failed (exit ${String(built.status)}):\n${built.stderr?.toString() ?? "(no stderr)"}`);
            }
        })();
    }
    return distReady;
}
function spawnGraphRun(runsRoot, markerDir) {
    const env = {
        ...process.env,
    ...
    const child = spawn(process.execPath, [CLI_ENTRY, "graph", "run", FIXTURE_PATH, "--runs-root", runsRoot], {
        cwd: REPO_ROOT,
        env,
...
function journalRecords(runDir) {
...
describe("e2e crash recovery via spawned CLI (AC-2/AC-3)", () => {
    ...
    it("resumes a killed run without re-executing journaled nodes and converges to the direct-run projection", async () => {
        ...
        await waitFor(() => existsSync(join(markers, "n1.marker")), "n1 marker to appear", 60_000);
        await waitFor(() => journalRecords(runDir).some((record) => record.transition.node_id === "n1"), "n1 journal record", 15_000);
        const markersBeforeKill = Object.fromEntries(["n1.marker", "n2.marker", "n3.marker", "term.marker"].map((name) => [
            name,
        ...
        if (resumed.code !== 0) {
            throw new Error(`resume exited ${resumed.code}\nstdout:\n${resumed.stdout}\nstderr:\n${resumed.stderr}`);
        }
        ...
        const n1Records = records.filter((record) => record.transition.node_id === "n1");
        expect(n1Records).toHaveLength(1);
        expect(n1Records[0].epoch).toBe(1);
        // Takeover advanced the epoch: resumed commits carry epoch >= 2.
        const resumedCommits = records.filter((record) => record.transition.node_id !== "n1");
        ...
        expect(statSync(join(markers, "n1.marker")).mtimeMs).toBe(markersBeforeKill["n1.marker"]);
        ...
        if (direct.code !== 0) {
            throw new Error(`direct run exited ${direct.code}\nstdout:\n${direct.stdout}\nstderr:\n${direct.stderr}`);
        }
        expect(direct.stdout).toContain("[replay] 0 record(s)");
        const envelopeA = JSON.parse(readFileSync(join(runsRoot, RUN_ID, "projection.json"), "utf8"));
        const envelopeB = JSON.parse(readFileSync(join(directRunsRoot, RUN_ID, "projection.json"), "utf8"));
        ...
        expect(canonicalJson(normalizeVolatile(envelopeA.projection))).toBe(canonicalJson(normalizeVolatile(envelopeB.projection)));
    }, 300_000);
});
/**
...
function normalizeVolatile(value, key) {
    if (key === "request_fingerprint")
        return "<derived-from-measured-io>";
...
//# sourceMappingURL=e2e-crash-recovery.test.js.map