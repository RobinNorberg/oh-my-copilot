...
async function completeRunWithEpochs(runsRoot, epochForLine) {
    ...
    const rewritten = lines.map((line, index) => {
        ...
        const { journal_fingerprint: _journalFingerprint, ...withoutFingerprint } = record;
...
describe("journal epoch provenance (P1-4)", () => {
    it.each(["missing", "empty"])("fails closed before executing work when an existing descriptor has a %s journal", async (journalState) => {
    ...
    it("fails closed CORRUPT_JOURNAL when a record carries a forged future epoch", async () => {
        ...
        const { journal_fingerprint: _journalFingerprint, ...withoutFingerprint } = forged;
    ...
    it("folds legitimately increasing epochs across two takeovers (1 then 2)", async () => {
        ...
        const runsRoot = makeRunsRoot();
        const { sealed } = await completeRunWithEpochs(runsRoot, (index) => (index < 2 ? 1 : 2));
...
//# sourceMappingURL=journal-epoch.test.js.map