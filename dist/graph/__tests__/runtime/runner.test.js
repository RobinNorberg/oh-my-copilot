...
describe("runGraph", () => {
    it("runs a linear 3-command graph to success with journal + snapshot", async () => {
        ...
        expect(isGraphSucceeded(sealed, snapshot?.projection)).toBe(true);
    ...
    it("resumes a partial run without re-running completed nodes", async () => {
        ...
        const runOneExecutor = new ScriptedExecutor((nodeId) => nodeId === "b" ? new Promise(() => { }) : okOutput(nodeId));
    ...
    it("rebuilds from the journal instead of accepting a stale projection snapshot", async () => {
        ...
        const snapshot = JSON.parse(readFileSync(projectionPath, "utf8"));
        ...
        writeFileSync(projectionPath, canonicalJson({
            ...snapshot,
            saved_at_seq: -1,
    ...
    it("rejects descriptor drift on resume with exit code 21", async () => {
        ...
        writeFileSync(join(runsRoot, sealed.run_id, "descriptor.json"), canonicalJson(drifted), "utf8");
        // Act
        const result = await runGraph(sealed, {
            runsRoot,
            executors: [allSucceed()],
    ...
    it("surfaces a corrupt journal tail as exit code 20", async () => {
        ...
        appendFileSync(join(runsRoot, sealed.run_id, "journal.jsonl"), '{"seq":3,"epoch":1,"descriptor_hash":"', "utf8");
        // Act
        const result = await runGraph(sealed, {
            runsRoot,
            executors: [allSucceed()],
    ...
    it("drains other branches when one branch exhausts its retry budget (AC-9/AC-10)", async () => {
        ...
        const sealed = sealGraphDescriptor({
            ...descriptorInput,
            nodes: descriptorInput.nodes.map((node) => node.id === "b1" ? { ...node, max_attempts: 2 } : node),
        });
        const executor = new ScriptedExecutor((nodeId) => nodeId === "b1" ? failedOutput(nodeId) : okOutput(nodeId));
        const { reporter, events } = makeReporter();
        // Act
        const result = await runGraph(sealed, {
            runsRoot,
        ...
        const journal = new FileJournal(runsRoot, sealed.run_id);
        const records = await journal.readAll();
        const failedB1 = records.filter((r) => r.transition.node_id === "b1" && r.transition.outcome === "failed");
    ...
    it("runs fork/join to success with a join_resolved record (AC-13)", async () => {
        ...
        const journal = new FileJournal(runsRoot, sealed.run_id);
        const records = await journal.readAll();
        const joinRecord = records.find((r) => r.transition.outcome === "join_resolved");
    ...
    it("replays bit-for-bit: resume fold equals direct run snapshot (AC-3)", async () => {
        ...
        const controller = new AbortController();
        const partialExecutor = new ScriptedExecutor((nodeId) => nodeId === "b1" || nodeId === "b2"
            ? new Promise(() => { })
            : okOutput(nodeId));
        await expect(runGraph(sealed, {
            runsRoot: runsRootB,
        ...
        expect(canonicalJson(snapshotB?.projection)).toBe(canonicalJson(snapshotA?.projection));
    });
    it("replays a crash between branch completions bit-for-bit (AC-3/AC-11b)", async () => {
        ...
        const controller = new AbortController();
        const partialExecutor = new ScriptedExecutor((nodeId) => nodeId === "b2" ? new Promise(() => { }) : okOutput(nodeId));
        await expect(runGraph(sealed, {
            runsRoot: runsRootB,
            executors: [partialExecutor],
            prompter: { prompt: async () => "approved" },
        ...
        for (const [transitionId, transition] of Object.entries(transitionsA)) {
            ...
            if (transitionsB[transitionId] !== undefined) {
                expect(transitionsB[transitionId].request_fingerprint).toBe(transition.request_fingerprint);
            }
        }
        const journalB = new FileJournal(runsRootB, sealed.run_id);
        const recordsB = await journalB.readAll();
        expect(recordsB.some((r) => r.transition.outcome === "join_resolved")).toBe(true);
