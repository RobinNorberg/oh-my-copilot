...
import { createRemoteApprovalGate, listPendingApprovals, parseDecisionArtifact, writeApprovalDecision, } from "../../runtime/remote-approval.js";
const REQUEST = {
    run_id: "run-1",
    node_id: "gate-1",
    activation_id: "act-1",
...
function decisionAfter(runsRoot, record, afterPolls, raw = false) {
    let calls = 0;
    return async () => {
        calls += 1;
        if (calls === afterPolls) {
            ...
            writeFileSync(join(dir, "act-1.json"), raw ? String(record) : JSON.stringify(record));
        }
    };
}
describe("createRemoteApprovalGate", () => {
    ...
    it("ignores malformed decision files and keeps waiting", async () => {
        ...
        const gate = createRemoteApprovalGate({
            ...
            sleep: async () => {
                ...
                mkdirSync(dir, { recursive: true });
                if (calls === 1)
                    writeFileSync(join(dir, "act-1.json"), "not json");
                if (calls === 2)
                    writeFileSync(join(dir, "act-1.json"), JSON.stringify({ decision: "maybe" }));
                if (calls === 3)
                    writeFileSync(join(dir, "act-1.json"), JSON.stringify({ decision: "approved" }));
            },
        });
    ...
    it("rejects traversal-shaped activation ids", async () => {
        ...
        await expect(gate.prompt({ ...REQUEST, activation_id: "../escape" })).rejects.toThrow();
    });
});
describe("parseDecisionArtifact", () => {
    it("accepts well-formed decisions", () => {
        expect(parseDecisionArtifact('{"decision":"approved","decided_by":"alice"}')).toEqual({ decision: "approved", decided_at: expect.any(String), decided_by: "alice" });
    });
...
describe("listPendingApprovals / writeApprovalDecision", () => {
    it("round-trips a pending request through decide", async () => {
        ...
        const raw = JSON.parse(readFileSync(join(runsRoot, "run-1", "approvals", "decisions", "act-1.json"), "utf8"));
...
describe("writeApprovalDecision fail-closed", () => {
    it("refuses to write a decision for an unknown run directory", () => {
        const runsRoot = makeRunsRoot();
        expect(() => writeApprovalDecision(runsRoot, "typo-run", "act-1", "approved", "cli")).toThrow(/unknown run/);
...
describe("createRemoteApprovalGate abort", () => {
    it("resolves denied when the signal is already aborted", async () => {
        ...
        expect(await gate.prompt(REQUEST)).toBe("denied");
        expect(existsSync(join(runsRoot, "run-1", "approvals", "pending", "act-1.json"))).toBe(false);
    });
});
//# sourceMappingURL=remote-approval.test.js.map