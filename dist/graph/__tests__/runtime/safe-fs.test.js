import { existsSync, mkdtempSync, readFileSync, renameSync, rmSync, symlinkSync, unlinkSync, writeFileSync, linkSync, openSync, constants as fsConstants, closeSync, } from "fs";
...
describe("graph runtime safe filesystem", () => {
    ...
    it("reads, writes, renames, and deletes artifacts through a validated Linux directory FD", () => {
        ...
        withContainedPathForPlatform(handle, "artifact.txt", (path) => {
            writeFileSync(path, "after");
            renameSync(path, join(handle.path, "renamed.txt"));
        }, "linux");
        expect(readFileSync(join(handle.path, "renamed.txt"), "utf8")).toBe("after");
    ...
    it("rejects special files and hardlinks as contained artifacts", () => {
        ...
        expect(() => readContainedFileNoFollow(handle, "artifact.txt")).toThrow("private regular file");
    ...
    it("fails closed on Windows instead of using a raceable pathname fallback", () => {
        ...
        expect(() => withContainedPathForPlatform(handle, "artifact.txt", (path) => readFileSync(path, "utf8"), "win32")).toThrow("refusing pathname fallback");
        expect(() => withContainedDirectory(handle, () => undefined, "win32"))
            .toThrow("refusing pathname fallback");
    });
    it("preserves ordinary ENOENT behavior for missing artifacts", () => {
        const { handle } = makeRunDir();
        expect(() => readContainedFileNoFollow(handle, "missing.txt")).toThrow(expect.objectContaining({ code: "ENOENT" }));
    });
    it.each([
        "",
        ".",
    ...
    it("rejects Windows alternate data stream names even when simulating Windows", () => {
        expect(() => assertSafeContainedFileName("artifact:stream", "win32")).toThrow("invalid contained artifact");
    });
    it("accepts canonical NFC and rejects decomposed NFD basenames", () => {
        expect(() => assertSafeContainedFileName("café.txt")).not.toThrow();
        expect(() => assertSafeContainedFileName("café.txt")).toThrow("invalid contained artifact");
    });
    it.each(["CON", "CON.txt", "NUL.log", "COM1", "LPT9", "AUX.md"])("rejects Windows device basename %j", (fileName) => {
