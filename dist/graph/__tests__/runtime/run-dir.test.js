 ...
 */
import { existsSync, mkdirSync, mkdtempSync, readdirSync, realpathSync, renameSync, rmSync, symlinkSync, } from "fs";
...
describe("resolveRunDir containment [P1-3]", () => {
    ...
    it("rejects malformed run ids with RangeError", () => {
        ...
        for (const bad of ["../x", "a/b", "a\\b", "", ".", ".."]) {
            expect(() => resolveRunDir(runsRoot, bad), JSON.stringify(bad)).toThrow(RangeError);
            expect(() => resolveRunDir(runsRoot, bad), JSON.stringify(bad)).toThrow("invalid run_id");
        }
    });
    it("creates the run directory inside the runs root and returns the joined path", () => {
        ...
        expect(resolved.toLowerCase().startsWith(`${runsRootReal.toLowerCase()}\\`) ||
            resolved.startsWith(`${runsRootReal}/`)).toBe(true);
    });
    it("fails closed on a symlinked run directory and writes nothing outside the root", async () => {
        ...
        symlinkSync(join(outside, "escape-target"), join(runsRoot, "run-victim"), "dir");
        // Act / Assert — resolution itself refuses.
        expect(() => resolveRunDir(runsRoot, "run-victim")).toThrow("run directory must not be a symbolic link");
        // Persistence components route through the same helper: use-time failure,
        // nothing materialized outside the runs root.
        await expect(new FileJournal(runsRoot, "run-victim").append(makeRecord(0))).rejects.toThrow("symbolic link");
        await expect(new FileOwnershipFence(runsRoot, "run-victim").acquire()).rejects.toThrow("symbolic link");
        expect(() => new FileProjectionStore(runsRoot, "run-victim")).toThrow("symbolic link");
        expect(readdirSync(join(outside, "escape-target"))).toEqual([]);
        expect(existsSync(join(outside, "escape-target", "journal.jsonl"))).toBe(false);
        expect(existsSync(join(outside, "escape-target", "owner.lock"))).toBe(false);
        expect(existsSync(join(outside, "escape-target", "projection.json"))).toBe(false);
    });
    ...
    it("does not follow a target symlink installed during creation", () => {
        ...
        try {
            ...
            expect(() => resolveRunDir(runsRoot, runId)).toThrow("run directory must not be a symbolic link");
        }
        finally {
            mkdirInterlock.before = undefined;
        }
    ...
    it("keeps component writes inside the runs root for a legitimate run id", async () => {
        ...
        await fence.release(acquired.epoch);
    });
});
//# sourceMappingURL=run-dir.test.js.map