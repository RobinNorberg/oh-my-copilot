...
import { applyNodeResult, beginActivationAttempt, initializeGraphProjection, listReadyExecutableActivations, listReadyJoinActivations, } from "../../scheduler.js";
import { runGraph } from "../../runtime/runner.js";
import { computeJournalFingerprint, FileJournal, } from "../../runtime/journal.js";
...
function okOutput(nodeId, route) {
    return {
        outcome: "succeeded",
...
function runOptions(runsRoot, executor) {
    return {
        runsRoot,
        executors: [executor],
...
describe("regression matrix (AC-9/10/11/11b)", () => {
    it("terminal-fails a node after exactly its max_attempts budget (AC-9)", async () => {
        ...
        const sealed = sealGraphDescriptor({
            ...input,
            nodes: input.nodes.map((node) => node.id === "b1" ? { ...node, max_attempts: 2 } : node),
        });
        const runsRoot = makeRunsRoot();
        const executor = new ScriptedExecutor((context) => context.node.id === "b1"
        ...
        const b1Failures = records.filter((record) => record.transition.node_id === "b1" &&
    ...
    it("drains sibling work to a defined end state when one branch fails terminally (AC-10)", async () => {
        ...
        const sealed = sealGraphDescriptor({
            ...input,
            nodes: input.nodes.map((node) => node.id === "b2" ? { ...node, max_attempts: 1 } : node),
        });
        const runsRoot = makeRunsRoot();
        const executor = new ScriptedExecutor((context) => context.node.id === "b2"
            ? failedOutput(context.node.id)
            : okOutput(context.node.id));
        // Act
        const result = await runGraph(sealed, runOptions(runsRoot, executor));
    ...
    it("runs a bounded back-edge loop to terminal and replays the journal to the identical projection (AC-11)", async () => {
        ...
        const executor = new ScriptedExecutor((context) => {
            if (context.node.id !== "work")
                return okOutput(context.node.id);
            workRuns += 1;
        ...
        const workTransitions = records.filter((record) => record.transition.node_id === "work");
        ...
        const snapshot = await store.load();
        expect(snapshot).not.toBeNull();
        const liveProjection = snapshot?.projection;
        ...
        const resumedAgain = await runGraph(sealed, runOptions(runsRoot, idleExecutor));
    ...
    it("fails closed with CORRUPT_JOURNAL when a journaled transition is tampered (AC-11b)", async () => {
        ...
        const runsRoot = makeRunsRoot();
        const firstRunExecutor = new ScriptedExecutor((context) => okOutput(context.node.id));
    ...
    it("fails closed when a journal record is rebound to a foreign descriptor hash", async () => {
        ...
        const sealed = sealGraphDescriptor(loadFixture("simple-linear.json"));
        const runsRoot = makeRunsRoot();
        await runGraph(sealed, runOptions(runsRoot, new ScriptedExecutor((context) => okOutput(context.node.id))));
        const journalPath = join(runsRoot, sealed.run_id, "journal.jsonl");
        const lines = readFileSync(journalPath, "utf8").split("\n").filter(Boolean);
        const tampered = JSON.parse(lines[0]);
        tampered.descriptor_hash =
            "0".repeat(64);
        lines[0] = canonicalJson(tampered);
        writeFileSync(journalPath, `${lines.join("\n")}\n`, "utf8");
        // Act + Assert: descriptor binding drift maps to DESCRIPTOR_MISMATCH.
        const result = await runGraph(sealed, runOptions(runsRoot, new ScriptedExecutor((context) => okOutput(context.node.id))));
        expect(result.terminal).toBe("failed");
    ...
    ])("fails closed before executing work when replay metadata has a forged $label", async ({ mutate, exitCode }) => {
        const sealed = sealGraphDescriptor(loadFixture("simple-linear.json"));
        const runsRoot = makeRunsRoot();
        await runGraph(sealed, runOptions(runsRoot, new ScriptedExecutor((context) => okOutput(context.node.id))));
        const journalPath = join(runsRoot, sealed.run_id, "journal.jsonl");
    ...
    it("refuses to start while a live owner holds the lock and writes nothing (AC-7)", async () => {
        ...
        writeFileSync(join(runDir, "owner.lock"), `${JSON.stringify({
            pid: process.pid,
            epoch: 1,
            timestamp: Date.now(),
        })}\n`, "utf8");
        // Act: the fence surfaces busy as a RunResult (not a throw).
        const executor = new ScriptedExecutor((context) => okOutput(context.node.id));
        const result = await runGraph(sealed, runOptions(runsRoot, executor));
...
function entryActivationIds(descriptor) {
    return Object.fromEntries(descriptor.entry_node_ids.map((id) => [id, `${id}-act0`]));
}
/**
 ...
 */
function foldJournal(descriptor, records) {
    let projection = initializeGraphProjection(descriptor, entryActivationIds(descriptor));
    for (const record of records) {
        ...
        if (transition.outcome === "succeeded") {
            ...
            const identities = transition.selected_edge_ids.length > 0
                ? {
        ...
        throw new Error(`fold helper only covers succeeded transitions; got ${transition.outcome} at seq ${record.seq}`);
    }
    return projection;
}
//# sourceMappingURL=regression-matrix.test.js.map