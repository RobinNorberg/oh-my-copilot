...
function makeContext(overrides) {
    ...
    const node = descriptor.nodes.find((n) => n.id === "entry");
    return {
        descriptor,
        node,
        activation_id: "act-1",
...
describe("AgentNodeExecutor", () => {
    it("succeeds and summarizes final text from a fake stream", async () => {
        let receivedOptions;
        const executor = new AgentNodeExecutor(async function* (options) {
        ...
        expect(receivedOptions.options.env)
    ...
    it("fails with timeout noted when timeout fires before the impl settles", async () => {
        const executor = new AgentNodeExecutor((_options) => new Promise(() => { }));
    ...
    it("aborts and interrupts a streaming query at the timeout boundary", async () => {
        ...
        const executor = new AgentNodeExecutor((options) => {
            signal =
                options.options
    ...
    it("surfaces an idempotency key before the agent effect starts", async () => {
        ...
        expect(receivedOptions.options.env
            .GRAPH_IDEMPOTENCY_KEY).toBe("run-approval:act-1");
    });
});
