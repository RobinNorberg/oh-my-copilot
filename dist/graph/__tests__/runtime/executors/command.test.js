...
function commandNode(overrides = {}) {
    return {
        id: "cmd",
        kind: "command",
        title: "Command node",
...
function contextFor(node, overrides = {}) {
    const descriptor = {
...
describe("CommandNodeExecutor", () => {
    ...
    it("captures stdout excerpts into the summary and evidence", async () => {
        ...
        const stdoutEvidence = output.evidence_refs.find((ref) => ref.ref.startsWith("stdout:"));
    ...
    it("fails fast on timeout, kills the tree, and leaves no orphaned grandchild", async () => {
        const executor = new CommandNodeExecutor();
        const node = commandNode({
            command: "node -e \"console.log('GPID:' + process.pid); setTimeout(()=>{},10000)\"",
            timeout_ms: 300,
        });
        ...
        const gpid = Number(/\bGPID:(\d+)\b/.exec(output.output_summary ?? "")?.[1] ?? "0");
    ...
    it("substitutes idempotency key tokens for idempotent policies", async () => {
        ...
        const node = commandNode({
            ...
            command: "node -e \"process.stdout.write(process.env.GRAPH_IDEMPOTENCY_KEY ?? '')\"",
        });
        const output = (await executor.execute(contextFor(node)));
    ...
    it("fails closed for reconcile policy instead of executing an unreconciled effect", async () => {
        ...
        const output = await executor.execute(contextFor(node));
        expect(output.outcome).toBe("failed");
        expect(output.output_summary).toBe("reconcile policy requires a custom executor");
    });
    it("keeps only the tail when stdout exceeds the 2000-char cap", async () => {
        const executor = new CommandNodeExecutor();
        const node = commandNode({
            command: "node -e \"console.log('START-MARKER' + '#'.repeat(3000) + 'END-MARKER')\"",
        });
        const output = await executor.execute(contextFor(node));
        expect(output.outcome).toBe("succeeded");
    ...
    it("scrubs non-allowlisted env from the child; GRAPH_* passes through", async () => {
        ...
        try {
            const node = commandNode({
                command: "node -e \"process.stdout.write(String(process.env['PARENT_SECRET_TOKEN']===undefined)+'|'+String(process.env['GRAPH_TEST_MARKER']))\"",
            });
            const output = await executor.execute(contextFor(node));
            expect(output.outcome).toBe("succeeded");
