...
function descriptorInput(runId, goal) {
    return {
        ...
        edges: [{ id: 'e-start-term', kind: 'fixed', from: 'start', to: 'term' }],
...
describe('graphCommand run subcommand', () => {
    const createConsoleErrorSpy = () => vi.spyOn(console, 'error').mockImplementation(() => { });
    let workDir;
    ...
    beforeEach(() => {
        ...
        mocks.runGraph.mockImplementation(async (sealed) => ({
            terminal: 'succeeded',
    ...
    it('runs a fresh descriptor end to end with exit code 0', async () => {
        ...
        const [sealedArg, optionsArg] = mocks.runGraph.mock.calls[0];
    ...
    it('accepts resume when stored descriptor matches the requested revision', async () => {
        ...
        // Assert
        expect(process.exitCode).toBe(EXIT_CODES.OK);
        const [sealedArg] = mocks.runGraph.mock.calls[0];
