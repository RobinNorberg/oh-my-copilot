...
export async function ralphthonCommand(args) {
    ...
    if (!options.task) {
        console.error(chalk.red('Task description required. Usage: omg ralphthon "your task"'));
        console.log(RALPHTHON_HELP);
        process.exit(1);
    }
    // Must be inside tmux
    if (!isInsideTmux()) {
        console.error(chalk.red("Ralphthon requires tmux. Run inside a tmux session or use `omg` to launch one."));
        process.exit(1);
    }
