...
vi.mock("../dispatcher.js", () => ({
    ...
    shellEscapeArg: vi.fn((v) => v),
    interpolateInstruction: vi.fn((t) => t),
}));
vi.mock("../dedupe.js", () => ({
...
const RESOLVED_GW = {
    gatewayName: "test-gw",
    gateway: { url: "https://example.com/hook", method: "POST" },
    instruction: "Stopped: {{tmuxTail}}",
};
const PROJECT_PATH = "/home/user/project";
const STATE_DIR = join(PROJECT_PATH, ".omg", "state");
describe("dead-pane guard in wakeOpenClaw (issue #2562)", () => {
    ...
    it("skips capture when pane has no new lines — no stale tmuxTail in payload", async () => {
        ...
        const [, , payload] = vi.mocked(wakeGateway).mock.calls[0];
...
//# sourceMappingURL=dead-pane-guard.test.js.map