...
function createPluginRoot(dir, agentContents) {
...
describe('cleanupStaleAgents', () => {
    ...
    it('fails closed when explicit OMC and Claude plugin roots conflict', async () => {
        ...
        try {
            ...
            vi.resetModules();
            const { cleanupStaleAgents: cleanup, prunePluginDuplicateAgents: prune, getInstalledOmcPluginRoots, AGENTS_DIR: agentsDir, } = await import('../index.js');
            mkdirSync(agentsDir, { recursive: true });
            writeFileSync(join(agentsDir, 'build-fixer.md'), historicalAgent('build-fixer.md'));
            writeFileSync(join(agentsDir, 'architect.md'), historicalAgent('architect.md'));
            ...
            if (originalOmcRoot === undefined)
                delete process.env.OMC_PLUGIN_ROOT;
            else
                process.env.OMC_PLUGIN_ROOT = originalOmcRoot;
            if (originalClaudeRoot === undefined)
                delete process.env.CLAUDE_PLUGIN_ROOT;
            else
                process.env.CLAUDE_PLUGIN_ROOT = originalClaudeRoot;
        }
    });
...
describe('prunePluginDuplicateSkills', () => {
    ...
    it('preserves user-authored standalone alias skills like omc-plan when content differs from plugin copy', async () => {
        ...
        const aliasSkillDir = join(skillsDir, 'omc-plan');
        mkdirSync(aliasSkillDir, { recursive: true });
        writeFileSync(join(aliasSkillDir, 'SKILL.md'), '---\nname: plan\ndescription: My custom alias skill\n---\n\n# Custom omc-plan\nUser-authored content.\n');
        const removed = prune(log);
        expect(removed).not.toContain('omc-plan');
...
describe('prunePluginDuplicateAgents', () => {
    ...
    async function withActivePluginAgents(agentContents) {
        const pluginRoot = join(tempDir, 'active-plugin-root');
    ...
    it('removes an exact release-authenticated standalone copy that duplicates an active agent', async () => {
        ...
        const { prunePluginDuplicateAgents: prune, AGENTS_DIR: agentsDir } = await withActivePluginAgents({ 'architect.agent.md': bytes });
        mkdirSync(agentsDir, { recursive: true });
    ...
    it.each([
        ...
        const { prunePluginDuplicateAgents: prune, AGENTS_DIR: agentsDir } = await withActivePluginAgents({ [filename]: Buffer.from('active payload agent\n') });
        mkdirSync(agentsDir, { recursive: true });
    ...
    it('preserves a candidate that becomes non-regular during unlink revalidation', async () => {
        const { prunePluginDuplicateAgents: prune, AGENTS_DIR: agentsDir } = await withActivePluginAgents({ 'architect.agent.md': Buffer.from('active payload agent\n') });
        const actualFs = await vi.importActual('fs');
        mkdirSync(agentsDir, { recursive: true });
