...
describe('install() standalone hook reconciliation', () => {
    ...
    it('restores OMC settings hooks for standalone installs during forced reconciliation', async () => {
        ...
        expect(writtenSettings.statusLine?.command).toContain(`${join(testClaudeDir, 'hud', 'omg-hud.mjs').replace(/\\/g, '/')}`);
        expect(writtenSettings.statusLine?.command).toContain('omg-hud-cache.sh');
        expect(readFileSync(join(testClaudeDir, 'hud', 'omg-hud-cache.sh'), 'utf-8')).toContain('HUD cached statusLine launcher');
        expect(readFileSync(join(testClaudeDir, 'hud', 'omg-hud.mjs'), 'utf-8')).toContain('const { getCopilotConfigDir } = await import(pathToFileURL(join(__dirname, "lib", "config-dir.mjs")).href);');
...
describe('install() plugin-provided hook deduplication (#2252)', () => {
    ...
    it('drops empty hook containers and reports exact removals in verbose mode (#3638)', async () => {
        ...
        try {
            ...
            expect(result.success).toBe(true);
            expect(writtenSettings.hooks).toBeUndefined();
            expect(logSpy).toHaveBeenCalledWith('  Removed 1 stale plugin duplicate hook entry from settings.json (events: SessionStart)');
        }
        finally {
            logSpy.mockRestore();
        }
    ...
    it('fails closed with a root-aware diagnostic when the plugin registry is ambiguous (#3638)', async () => {
        ...
        try {
            ...
            expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(`Skipped plugin duplicate-hook cleanup: plugin root resolution is plugin (cleanupAllowed=false, roots=${fakePluginRoot})`));
        }
        finally {
            logSpy.mockRestore();
        }
