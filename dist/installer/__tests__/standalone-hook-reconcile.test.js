...
const originalPluginRoot = process.env.COPILOT_PLUGIN_ROOT;
...
describe.skip('install() standalone hook reconciliation', () => {
    ...
    afterEach(() => {
        ...
        if (originalPluginRoot !== undefined) {
            process.env.COPILOT_PLUGIN_ROOT = originalPluginRoot;
        }
        else {
            delete process.env.COPILOT_PLUGIN_ROOT;
        }
    ...
    it('restores OMC settings hooks for standalone installs during forced reconciliation', async () => {
        ...
        expect(readFileSync(join(testClaudeDir, 'hud', 'omc-hud.mjs'), 'utf-8')).toContain('const { getCopilotConfigDir } = await import(pathToFileURL(join(__dirname, "lib", "config-dir.mjs")).href);');
    ...
    it('preserves non-OMC ~/.copilot/hooks commands while adding standalone OMC hooks', async () => {
        const settingsPath = join(testClaudeDir, 'settings.json');
        mkdirSync(testClaudeDir, { recursive: true });
        ...
        expect(result.success).toBe(true);
        expect(commands).toContain('node $HOME/.copilot/hooks/other-plugin.mjs');
