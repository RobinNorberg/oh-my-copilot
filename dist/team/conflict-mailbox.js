...
// (out of scope here). Worker inbox: .omcp/state/team/{team}/workers/{worker}/inbox.md.
// Leader inbox: .omcp/state/team/{team}/leader/inbox.md (see leader-inbox.ts).
//
 ...
 */
export function formatMergeConflictForLeader(args) {
 ...
 * Pure: same input → same output.
 */
export function formatRebaseConflictForWorker(args) {
 ...
 */
export async function deliverMergeConflictToLeader(args) {
 ...
 */
export async function deliverRebaseConflictToWorker(args) {
    const { teamName, workerName, cwd, message } = args;
    await appendToInbox(teamName, workerName, message, cwd);
}
//# sourceMappingURL=conflict-mailbox.js.map