...
describe('runtime v2 startup inbox dispatch', () => {
    ...
    beforeEach(() => {
        ...
        mocks.deliverStartupInbox.mockImplementation(async (context, message) => {
            ...
            if (mocks.autoStartupEvidence) {
                ...
                const workerDir = join(cwd, '.omg', 'state', 'team', context.attempt.team_name, 'workers', context.attempt.worker_name);
    ...
    it('writes durable inbox dispatch evidence when startup worker notification succeeds', async () => {
        ...
        const config = JSON.parse(await readFile(join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'config.json'), 'utf-8'));
        const manifest = JSON.parse(await readFile(join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'manifest.json'), 'utf-8'));
    ...
    it('delivers trusted Cursor reviewer guidance in the default non-worktree inbox', async () => {
        ...
        const config = JSON.parse(await readFile(join(cwd, '.omg', 'state', 'team', 'cursor-bootstrap-team', 'config.json'), 'utf-8'));
        expect(config.workers[0].role).toBe('critic');
        const inbox = await readFile(join(cwd, '.omg', 'state', 'team', 'cursor-bootstrap-team', 'workers', 'worker-1', 'inbox.md'), 'utf-8');
    ...
    it('persists startup task delegation plans and gives executable result evidence instructions', async () => {
        ...
        const taskPath = join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'tasks', 'task-1.json');
    ...
    it('preserves startup failure evidence when a worker launch throws after scaffolding', async () => {
        ...
        const markerPath = join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'startup-failure.json');
    ...
    it('keeps dirty worktree preservation metadata when startup rollback records failure evidence', async () => {
        ...
        const markerPath = join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'startup-failure.json');
        const marker = JSON.parse(await readFile(markerPath, 'utf-8'));
        const backupPath = join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'workers', 'worker-1', 'worktree-root-agents.json');
        const worktreePath = join(cwd, '.omg', 'team', 'dispatch-team', 'worktrees', 'worker-1');
    ...
    it('persists runtime-v2 worktree contract fields for split-pane teams', async () => {
        ...
        expect(runtime.config.workers[0]).toMatchObject({
            working_dir: join(cwd, '.omg', 'team', 'dispatch-team', 'worktrees', 'worker-1'),
        ...
        expect(mocks.spawnOwnedWorkerInPane).toHaveBeenCalledWith('dispatch-session', expect.objectContaining({ paneId: '%2' }), expect.objectContaining({
            cwd: join(cwd, '.omg', 'team', 'dispatch-team', 'worktrees', 'worker-1'),
            launchStateCwd: cwd,
        }));
        const configPath = join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'config.json');
        const manifestPath = join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'manifest.json');
        ...
        expect(requests[0]?.trigger_message.replace('$OMC_TEAM_STATE_ROOT', teamStateRoot))
            .toContain(join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'workers', 'worker-1', 'inbox.md'));
        const overlay = await readFile(join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'workers', 'worker-1', 'AGENTS.md'), 'utf-8');
    ...
    it('wires auto-merge worker cadence and drains before unregistering on shutdown', async () => {
        ...
        expect(cadenceMocks.installCommitCadence).toHaveBeenCalledWith(expect.objectContaining({
            ...
            agentType: 'codex',
            enabled: true,
            worktreePath: join(cwd, '.omg', 'team', 'dispatch-team', 'worktrees', 'worker-1'),
        }));
        expect(cadenceMocks.startFallbackPoller).toHaveBeenCalledWith(join(cwd, '.omg', 'team', 'dispatch-team', 'worktrees', 'worker-1'), 'worker-1');
    ...
    it('kills the started team session and rolls back worktrees when manifest persistence fails', async () => {
        ...
        await mkdir(join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'manifest.json'), { recursive: true });
        const { startTeamV2 } = await import('../runtime-v2.js');
        await expect(startTeamV2({
            teamName: 'dispatch-team',
            workerCount: 1,
        ...
        await expect(readFile(join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'config.json'), 'utf-8'))
            .rejects.toMatchObject({ code: 'ENOENT' });
        await expect(readFile(join(cwd, '.omg', 'state', 'team', 'dispatch-team', 'worktrees.json'), 'utf-8'))
            .rejects.toMatchObject({ code: 'ENOENT' });
        await expect(readFile(join(cwd, '.omg', 'team', 'dispatch-team', 'worktrees', 'worker-1', 'AGENTS.md'), 'utf-8'))
            .rejects.toMatchObject({ code: 'ENOENT' });
    });
    ...
    it('routes an inferred reviewer task to a cursor worker carrying the verdict contract (issue #3880)', async () => {
        ...
        const persisted = JSON.parse(await readFile(join(cwd, '.omg', 'state', 'team', 'cursor-routing-team', 'config.json'), 'utf-8'));
        ...
        const inbox = await readFile(join(cwd, '.omg', 'state', 'team', 'cursor-routing-team', 'workers', 'worker-1', 'inbox.md'), 'utf-8');
        expect(inbox).toContain('REQUIRED: Structured Verdict Output');
        expect(inbox).toContain('do NOT edit, create, or delete any file');
    });
