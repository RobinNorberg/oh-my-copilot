...
import { CONTRACT_ROLES, cliWorkerOutputFilePath, parseCliWorkerVerdict, renderCliWorkerOutputContract, shouldInjectContract, } from '../cli-worker-contract.js';
describe('cli-worker-contract', () => {
    ...
    describe('cliWorkerOutputFilePath', () => {
        it('joins team state root + worker into the conventional path', () => {
            const p = cliWorkerOutputFilePath('/repo/.omcp/state/team/foo', 'worker-2');
            expect(p).toBe('/repo/.omcp/state/team/foo/workers/worker-2/verdict.json');
        });
        it('normalizes windows backslashes to forward slashes', () => {
            const p = cliWorkerOutputFilePath('C:\\proj\\.omcp\\state\\team\\foo', 'worker-1');
            expect(p).toBe('C:/proj/.omcp/state/team/foo/workers/worker-1/verdict.json');
        });
    });
...
//# sourceMappingURL=cli-worker-contract.test.js.map