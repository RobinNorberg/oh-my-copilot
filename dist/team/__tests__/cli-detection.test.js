...
describe('cli-detection', () => {
    ...
    it.each([
        ['finder timeout', spawnResult({ status: null, signal: 'SIGTERM', error: { code: 'ETIMEDOUT' } })],
        ['finder start error', spawnResult({ status: null, signal: null, error: { code: 'ENOENT' } })],
    ...
    it.each(['', 'codex/evil', 'codex\\evil', 'codex name', 'codex\tname', 'codex\nname', 'codex"name', 'codex&name', 'café'])('rejects unsafe bare binary name %j before spawning', (binary) => {
    ...
    it.each(['ENOENT', 'UNKNOWN', 'EINVAL'])('uses the closed batch fallback for direct %s', (code) => {
        ...
        mockSpawnSync
            ...
            .mockReturnValueOnce(spawnResult({ status: null, signal: null, error: { code } }))
        ...
        expect(mockSpawnSync).toHaveBeenNthCalledWith(3, 'C:\\Windows\\System32\\cmd.exe', ['/d', '/v:off', '/s', '/c', '""C:\\Program Files\\Provider\\provider.cmd" --version"'], {
            timeout: 3000,
            encoding: 'utf8',
            stdio: ['ignore', 'pipe', 'pipe'],
            shell: false,
    ...
    it('maps a blank batch wrapper result like any other zero-exit version result', () => {
        ...
        mockSpawnSync
            ...
            .mockReturnValueOnce(spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } }))
            .mockReturnValueOnce(spawnResult({ status: 0, stdout: '\r\n' }));
        expect(probeCli('provider')).toEqual({
            found: true,
    ...
    it.each([
        ['non-batch path', 'C:\\Tools\\provider.exe', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['percent path', 'C:\\Tools\\provider%inject.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['exclamation path', 'C:\\Tools\\provider!inject.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['caret path', 'C:\\Tools\\provider^inject.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['ampersand path', 'C:\\Tools\\provider&inject.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['pipe path', 'C:\\Tools\\provider|inject.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['less-than path', 'C:\\Tools\\provider<inject.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['greater-than path', 'C:\\Tools\\provider>inject.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['parentheses path', 'C:\\Tools\\provider(inject).cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['quoted path', 'C:\\Tools\\provider"inject.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['NUL path', `C:\\Tools\\provider${String.fromCharCode(0)}inject.cmd`, spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['UNC path', '\\\\server\\share\\provider.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['device path', '\\\\?\\C:\\Tools\\provider.cmd', spawnResult({ status: null, signal: null, error: { code: 'EINVAL' } })],
        ['completed zero-exit path', 'C:\\Tools\\provider.cmd', spawnResult({ status: 0, signal: null, stdout: '' })],
        ['completed nonzero path', 'C:\\Tools\\provider.cmd', spawnResult({ status: 1, signal: null, error: { code: 'EINVAL' } })],
        ['signaled path', 'C:\\Tools\\provider.cmd', spawnResult({ status: null, signal: 'SIGTERM', error: { code: 'EINVAL' } })],
        ['timed-out path', 'C:\\Tools\\provider.cmd', spawnResult({ status: null, signal: 'SIGTERM', error: { code: 'ETIMEDOUT' } })],
        ['missing direct error', 'C:\\Tools\\provider.cmd', spawnResult({ status: null, signal: null })],
        ['unadmitted direct error', 'C:\\Tools\\provider.cmd', spawnResult({ status: null, signal: null, error: { code: 'EACCES' } })],
        ...
        expect(mockSpawnSync).toHaveBeenCalledTimes(2);
        expect(mockSpawnSync.mock.calls.every((call) => call[2]?.shell !== true)).toBe(true);
    });
