...
describe('buildWorkerLaunchWrapper platform contract (issue #3931)', () => {
    describe('Windows wrapper (platform=win32)', () => {
        ...
        it('preserves Windows quoting for paths with spaces and percent', () => {
            const wrapper = buildWorkerLaunchWrapper(makeAttempt({ runtimeCliPath: 'C:\\Program Files\\omc\\runtime-cli.cjs' }), 'win32');
    ...
    describe('POSIX wrapper (platform=linux/darwin)', () => {
        ...
        it('single-quote-escapes runtimeCliPath with spaces', () => {
            const wrapper = buildWorkerLaunchWrapper(makeAttempt({ runtimeCliPath: '/opt/my project/runtime-cli.cjs' }), 'linux');
        ...
        it('escapes single quotes via close-open pattern', () => {
            const wrapper = buildWorkerLaunchWrapper(makeAttempt({ runtimeCliPath: "/opt/omc/runtime's/cli.cjs" }), 'linux');
        ...
        // POSIX-only: executes the generated sh wrapper directly, which Windows
        // cannot spawn (and the interpolated paths assume POSIX separators).
        it.skipIf(process.platform === 'win32')('executes correctly: bootstrap derivation, exit propagation, self-delete', async () => {
        ...
        it('handles bootstrap path with spaces via symlink-safe derivation', () => {
            const wrapper = buildWorkerLaunchWrapper(makeAttempt({ runtimeCliPath: '/opt/omc/runtime-cli.cjs' }), 'linux');
...
//# sourceMappingURL=worker-launch-wrapper-posix.test.js.map