...
import { readPlanningArtifacts, isPlanningComplete, readApprovedExecutionLaunchHint, readApprovedExecutionLaunchHintOutcome, } from "../artifacts.js";
import { planningArtifactTimestamp, selectMatchingTestSpecsForPrd, } from "../artifact-names.js";
describe("planning/artifacts", () => {
    ...
    describe("artifact names", () => {
        it("formats canonical artifact timestamps without milliseconds", () => {
            expect(planningArtifactTimestamp(new Date("2026-05-02T08:09:10.456Z"))).toBe("20260502T080910Z");
        });
    ...
    describe("isPlanningComplete", () => {
        ...
        it("requires timestamped PRDs to use matching timestamped test specs", () => {
            writeValidArtifacts("prd-20260502T080910Z-alpha.md", "test-spec-alpha.md");
            expect(isPlanningComplete(readPlanningArtifacts(testDir))).toBe(false);
            writeValidArtifacts("prd-20260502T080910Z-alpha.md", "test-spec-20260502T080910Z-alpha.md");
            expect(isPlanningComplete(readPlanningArtifacts(testDir))).toBe(true);
        });
        it("treats valid OMX planning artifacts as planning-complete", () => {
            const omxPlansDir = join(testDir, ".omx", "plans");
            mkdirSync(omxPlansDir, { recursive: true });
            writeFileSync(join(omxPlansDir, "prd-feature.md"), [
                "# PRD",
                "",
                "## Acceptance criteria",
                "- done",
            ...
            writeFileSync(join(omxPlansDir, "test-spec-feature.md"), [
                "# Test Spec",
                "",
                "## Unit coverage",
                "- unit",
    ...
    describe("readApprovedExecutionLaunchHint", () => {
        ...
        it("marks launch hints incomplete when planning-complete validation is required", () => {
            ...
            expect(readApprovedExecutionLaunchHint(testDir, "team").task).toBe("implement auth");
            expect(readApprovedExecutionLaunchHintOutcome(testDir, "team", {
        ...
        it("resolves launch hints when required planning artifacts are complete", () => {
            ...
            expect(outcome.status === "resolved" ? outcome.hint.task : null).toBe("implement auth");
        });
