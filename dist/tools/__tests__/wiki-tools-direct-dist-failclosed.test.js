...
import { mkdtempSync, mkdirSync, writeFileSync, readFileSync, rmSync, existsSync, chmodSync, copyFileSync, } from 'node:fs';
...
describe('tracked dist wiki runtime fail-closed (#3858 remaining P1)', () => {
    ...
    let srcDir;
    let secretPage;
    let clearWorktreeCache;
    let resolveWorkingDirectoryOrLinkedWorktree;
    let wikiTools;
    beforeEach(async () => {
        originalCwd = process.cwd();
        originalPath = process.env.PATH;
        const resolverMod = await import(pathToFileURL(DIST_RESOLVER).href);
        const wikiMod = await import(pathToFileURL(DIST_WIKI).href);
...
//# sourceMappingURL=wiki-tools-direct-dist-failclosed.test.js.map