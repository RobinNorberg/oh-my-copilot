...
describe('state-tools', () => {
    ...
    describe('state_write', () => {
        it('should write state to legacy path when no session_id provided', async () => {
            ...
            const legacyPath = join(TEST_DIR, '.omcp', 'state', 'ralph-state.json');
    ...
    describe('state_clear', () => {
        ...
        it('should clear only the requested session for every execution mode', async () => {
            ...
            for (const mode of modes) {
                ...
                const sessionAPath = join(TEST_DIR, '.omcp', 'state', 'sessions', sessionA, `${mode}-state.json`);
                const sessionBPath = join(TEST_DIR, '.omcp', 'state', 'sessions', sessionB, `${mode}-state.json`);
        ...
        it('should clear legacy and all sessions when session_id is omitted and show warning', async () => {
            ...
            const legacyPath = join(TEST_DIR, '.omcp', 'state', 'ultrawork-state.json');
            const sessionPath = join(TEST_DIR, '.omcp', 'state', 'sessions', sessionId, 'ultrawork-state.json');
        ...
        it('should only count actual deletions in broad clear count', async () => {
            ...
            mkdirSync(join(TEST_DIR, '.omcp', 'state', 'sessions', sessionWithState), { recursive: true });
            mkdirSync(join(TEST_DIR, '.omcp', 'state', 'sessions', sessionWithoutState), { recursive: true });
        ...
        it('clears completed-session orphan state when cancel runs from a fresh session id', async () => {
            ...
            for (const orphanSessionId of orphanSessionIds) {
                ...
                writeFileSync(join(TEST_DIR, '.omcp', 'sessions', `${orphanSessionId}.json`), JSON.stringify({ session_id: orphanSessionId, ended_at: '2026-05-04T00:00:00.000Z' }));
            }
            mkdirSync(join(TEST_DIR, '.omcp', 'state', 'sessions', liveSessionId), { recursive: true });
            for (const mode of modes) {
                for (const orphanSessionId of orphanSessionIds) {
                    writeFileSync(join(TEST_DIR, '.omcp', 'state', 'sessions', orphanSessionId, `${mode}-state.json`), JSON.stringify({
                        active: true,
                        session_id: orphanSessionId,
                        ...(mode === 'team' ? { team_name: `team-${orphanSessionId}` } : {}),
                    }));
                }
                writeFileSync(join(TEST_DIR, '.omcp', 'state', 'sessions', liveSessionId, `${mode}-state.json`), JSON.stringify({ active: true, session_id: liveSessionId }));
                const result = await stateClearTool.handler({
                    mode,
        ...
        it('reports completed-session orphan state on session-scoped read misses', async () => {
            ...
            mkdirSync(join(TEST_DIR, '.omcp', 'state', 'sessions', orphanSessionId), { recursive: true });
            writeFileSync(join(TEST_DIR, '.omcp', 'sessions', `${orphanSessionId}.json`), JSON.stringify({ session_id: orphanSessionId, ended_at: '2026-05-04T00:00:00.000Z' }));
            writeFileSync(join(TEST_DIR, '.omcp', 'state', 'sessions', orphanSessionId, 'ralph-state.json'), JSON.stringify({ active: true, session_id: orphanSessionId }));
            const result = await stateReadTool.handler({
                mode: 'ralph',
                session_id: freshSessionId,
                workingDirectory: TEST_DIR,
    ...
    describe('session_id parameter', () => {
        ...
        it('should clear session-specific state without affecting legacy owned by another session', async () => {
            ...
            writeFileSync(join(TEST_DIR, '.omcp', 'state', 'ralph-state.json'), JSON.stringify({ active: true, source: 'legacy', _meta: { sessionId: otherSessionId } }));
            const sessionDir = join(TEST_DIR, '.omcp', 'state', 'sessions', sessionId);
            mkdirSync(sessionDir, { recursive: true });
        ...
        it('reports no-op with checked paths when session clear finds no actual state file', async () => {
            ...
            expect(result.content[0].text).toContain('Checked paths');
            expect(result.content[0].text).toContain(join(TEST_DIR, '.omcp', 'state', 'sessions', sessionId, 'autopilot-state.json'));
        });
