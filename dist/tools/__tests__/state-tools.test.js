...
describe('state-tools', () => {
    ...
    describe('state_clear', () => {
        ...
        it('clears completed-session orphan state when cancel runs from a fresh session id', async () => {
            ...
            for (const orphanSessionId of orphanSessionIds) {
                ...
                writeFileSync(join(TEST_DIR, '.omg', 'sessions', `${orphanSessionId}.json`), JSON.stringify({ session_id: orphanSessionId, ended_at: '2026-05-04T00:00:00.000Z' }));
            }
            mkdirSync(join(TEST_DIR, '.omg', 'state', 'sessions', liveSessionId), { recursive: true });
            for (const mode of modes) {
                for (const orphanSessionId of orphanSessionIds) {
                    writeFileSync(join(TEST_DIR, '.omg', 'state', 'sessions', orphanSessionId, `${mode}-state.json`), JSON.stringify({
                        active: true,
                        session_id: orphanSessionId,
                        ...(mode === 'team' ? { team_name: `team-${orphanSessionId}` } : {}),
                    }));
                }
                writeFileSync(join(TEST_DIR, '.omg', 'state', 'sessions', liveSessionId, `${mode}-state.json`), JSON.stringify({ active: true, session_id: liveSessionId }));
                const result = await stateClearTool.handler({
                    mode,
        ...
        it('reports completed-session orphan state on session-scoped read misses', async () => {
            ...
            mkdirSync(join(TEST_DIR, '.omg', 'state', 'sessions', orphanSessionId), { recursive: true });
            writeFileSync(join(TEST_DIR, '.omg', 'sessions', `${orphanSessionId}.json`), JSON.stringify({ session_id: orphanSessionId, ended_at: '2026-05-04T00:00:00.000Z' }));
            writeFileSync(join(TEST_DIR, '.omg', 'state', 'sessions', orphanSessionId, 'ralph-state.json'), JSON.stringify({ active: true, session_id: orphanSessionId }));
            const result = await stateReadTool.handler({
                mode: 'ralph',
                session_id: freshSessionId,
                workingDirectory: TEST_DIR,
    ...
    describe('session_id parameter', () => {
        ...
        it('should clear session-specific state without affecting legacy owned by another session', async () => {
            ...
            writeFileSync(join(TEST_DIR, '.omg', 'state', 'ralph-state.json'), JSON.stringify({ active: true, source: 'legacy', _meta: { sessionId: otherSessionId } }));
            const sessionDir = join(TEST_DIR, '.omg', 'state', 'sessions', sessionId);
            mkdirSync(sessionDir, { recursive: true });
