...
import { mkdtempSync, mkdirSync, writeFileSync, readFileSync, rmSync, existsSync, chmodSync, realpathSync, copyFileSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { join, delimiter } from 'node:path';
import { clearWorktreeCache, setGitShowToplevelProbeForTests, } from '../../lib/worktree-paths.js';
import { wikiIngestTool, wikiAddTool, wikiQueryTool, wikiReadTool, wikiListTool, wikiLintTool, wikiDeleteTool, } from '../wiki-tools.js';
function git(cwd, command) {
    execSync(`git ${command}`, { cwd, stdio: 'pipe' });
}
async function invokeAllWikiTools(workingDirectory) {
    return [
...
function gitExit(status, stderr = '') {
    const err = new Error('Command failed: git rev-parse --show-toplevel');
    err.status = status;
    err.stderr = stderr;
    return err;
}
function installFakeGit(dir, unixBody, winBody) {
    ...
    if (process.platform === 'win32') {
        ...
        if (!windowsRoot)
            throw new Error('SystemRoot or WINDIR is required on Windows');
...
function seedSessionWiki(sessionRepo) {
...
describe('wiki tools fail closed on generic git probe failure (#3858 remaining P1)', () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(join(foreignRepo, '.omg', 'wiki', 'aapanel-setup.md'), '---\ntitle: aaPanel Setup\ncategory: reference\nconfidence: high\ntags: [aapanel]\nupdated: 2026-08-24\n---\n\nforeign-only\n');
    ...
    function assertNoWikiIo(secretBefore) {
    ...
    async function assertAllToolsRejectWithoutIo(workingDirectory) {
    ...
    it('seven wiki tools reject a trusted subdirectory when fake git exits 1 and perform no reads/writes/deletes', async () => {
        ...
        await assertAllToolsRejectWithoutIo(undefined);
    ...
    it('seven wiki tools reject omitted workingDirectory when fake git exits 1', async () => {
        ...
        clearWorktreeCache();
        const secretBefore = readFileSync(secretPage, 'utf8');
        const results = await invokeAllWikiTools(undefined);
        expect(results).toHaveLength(7);
        for (const { name, result } of results) {
            expect(result.isError, name).toBe(true);
    ...
    it('seven wiki tools reject injectable EACCES/ETIMEDOUT/signal/malformed probes with no wiki IO', async () => {
        const probes = [
            () => {
                const err = new Error('spawnSync git EACCES');
                err.code = 'EACCES';
                err.path = 'git';
                err.syscall = 'spawnSync git';
                throw err;
            ...
            () => {
                const err = new Error('git killed');
                err.signal = 'SIGTERM';
                err.status = null;
                throw err;
            },
...
//# sourceMappingURL=wiki-tools-git-probe-failclosed.test.js.map