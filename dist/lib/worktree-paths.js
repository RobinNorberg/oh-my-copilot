...
const omgMigrationWarnings = new Set();
/**
 ...
 */
export function migrateOmgToOmcp(worktreeRoot) {
    ...
    if (existsSync(newPath)) {
        ...
        if (!omgMigrationWarnings.has(worktreeRoot)) {
            ...
            console.warn(`[omg] Both .omg/ and .omcp/ exist in ${worktreeRoot}. ` +
    ...
    try {
        ...
        console.warn(`[omg] Failed to migrate .omg/ → .omcp/ in ${worktreeRoot}: ${err instanceof Error ? err.message : err}. ` +
...
export function getOmcRoot(worktreeRoot) {
    ...
    if (customDir) {
        ...
        if (!dualDirWarnings.has(warningKey) && existsSync(legacyPath) && existsSync(centralizedPath)) {
            ...
            console.warn(`[omg] Both legacy state dir (${legacyPath}) and centralized state dir (${centralizedPath}) exist. ` +
 ...
 */
export function getSharedOmcRoot(worktreeRoot) {
    const customDir = process.env.OMC_STATE_DIR;
    const root = worktreeRoot || getWorktreeRoot() || process.cwd();
    if (customDir) {
        const projectId = getProjectIdentifier(root);
 ...
 */
export function migrateOmcpContentToOmc(worktreeRoot) {
    ...
    for (const [src, dst] of fileMoves) {
        ...
        if (existsSync(dst)) {
            ...
            if (!omcpContentWarnings.has(warningKey)) {
                ...
                console.warn(`[omc-share] Both ${src} and ${dst} exist. Using ${dst}; remove the ` +
        ...
        try {
            ...
            console.warn(`[omc-share] Failed to relocate ${src} → ${dst}: ` +
    ...
    for (const sub of ['plans', 'research', 'notepads']) {
        ...
        for (const entry of entries) {
            ...
            if (existsSync(d)) {
                ...
                if (!omcpContentWarnings.has(warningKey)) {
                    omcpContentWarnings.add(warningKey);
                    console.warn(`[omc-share] Both ${s} and ${d} exist. Using ${d}; remove the ` +
            ...
            try {
                ...
                console.warn(`[omc-share] Failed to relocate ${s} → ${d}: ` +
                    `${err instanceof Error ? err.message : err}`);
            }
        }
    }
...
const omcpContentWarnings = new Set();
/**
 * Clear the omcp→omc content migration warning cache (testing only).
 * @internal
 */
export function clearOmcpContentWarnings() {
