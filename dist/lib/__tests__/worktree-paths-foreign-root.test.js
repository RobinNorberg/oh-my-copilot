...
import { clearWorktreeCache, resolveWorkingDirectoryOrLinkedWorktree, validateWorkingDirectoryOrLinkedWorktree, ForeignWorkingDirectoryError, getCanonicalWorkingDirectoryRoots, } from '../../lib/worktree-paths.js';
...
function assertOpaqueCanonicalSerialization(value, opts) {
    ...
    expect(() => getCanonicalWorkingDirectoryRoots(cloned)).toThrow();
    const wrappedError = new Error('logger wrap', { cause: value });
...
describe('shared resolver #3858: foreign repo, linked worktree, non-git, same-root', () => {
    ...
    it('ForeignWorkingDirectoryError requires callerLabel and never renders canonical provided/trusted roots', () => {
        const error = new ForeignWorkingDirectoryError('/canonical/foreign-vault', '/canonical/session-project', '../foreign-vault');
        ...
        const overlapping = new ForeignWorkingDirectoryError(sourceFile, sourceRoot, '../foreign-vault');
    ...
    it('redacts percent-encoded file-URL stack frames for roots with spaces', () => {
        ...
        writeFileSync(probe, `import { ForeignWorkingDirectoryError } from ${JSON.stringify(moduleHref)};
import { inspect } from 'node:util';
...
`);
    ...
    it('still throws for a non-git path outside the trusted root without leaking the full trusted root', () => {
        expect(() => validateWorkingDirectoryOrLinkedWorktree(plainDir)).toThrow('is outside the trusted worktree root');
        try {
...
//# sourceMappingURL=worktree-paths-foreign-root.test.js.map