...
function showToplevelCalls() {
    return mockedExecFileSync.mock.calls.filter(([, args]) => Array.isArray(args) &&
        args[0] === "rev-parse" &&
        args[1] === "--show-toplevel").length;
}
describe("git top-level memoization", () => {
    ...
    it("memoizes project identifiers by canonical root within a render scope", () => {
        ...
        const remoteCalls = () => mockedExecFileSync.mock.calls.filter(([command, args]) => command === "git" &&
            Array.isArray(args) &&
            args[0] === "remote" &&
            args[1] === "get-url").length;
        const commonDirCalls = () => mockedExecFileSync.mock.calls.filter(([command, args]) => command === "git" &&
            Array.isArray(args) &&
            args[0] === "rev-parse" &&
            args.includes("--git-common-dir")).length;
        withWorktreePathRenderScope(() => {
...
//# sourceMappingURL=worktree-paths-toplevel-cache.test.js.map