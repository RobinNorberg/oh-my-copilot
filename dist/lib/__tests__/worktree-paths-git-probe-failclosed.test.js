...
import { mkdtempSync, mkdirSync, writeFileSync, rmSync, chmodSync, realpathSync, copyFileSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { join, delimiter } from 'node:path';
import { clearWorktreeCache, resolveWorkingDirectoryOrLinkedWorktree, validateWorkingDirectoryOrLinkedWorktree, setGitShowToplevelProbeForTests, } from '../../lib/worktree-paths.js';
...
function spawnError(code) {
    const err = new Error(`spawnSync git ${code}`);
    err.code = code;
    err.path = 'git';
    err.syscall = 'spawnSync git';
    return err;
...
describe('git probe fail-closed classification (#3858 remaining P1)', () => {
    ...
    it('injectable EACCES, ETIMEDOUT, killed timeout, and signal fail closed', () => {
        const cases = [
            ...
            () => {
                const err = spawnError('ETIMEDOUT');
                err.killed = true;
                throw err;
            },
...
//# sourceMappingURL=worktree-paths-git-probe-failclosed.test.js.map